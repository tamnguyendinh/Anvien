...
This audit used Anvien as the primary codebase graph source, then cross-checked candidates with source search to reduce false positives.
...
graph: nodes=89034 relationships=122030 path=E:\Anvien\.anvien\graph.json
```

Graph schema checks:

```powershell
.\anvien\bin\anvien.exe cypher "MATCH (n) RETURN labels(n) AS labels, count(n) AS count ORDER BY count DESC LIMIT 50" --repo Anvien
.\anvien\bin\anvien.exe cypher "MATCH ()-[r]->() RETURN r.type AS relType, count(r) AS count ORDER BY count DESC LIMIT 80" --repo Anvien
```
...
MATCH (f)
WHERE labels(f) = 'File'
  AND f.filePath STARTS WITH 'anvien-web/src/'
...
Source cross-checks used `rg` for candidate names/imports across `anvien-web/src`, `anvien-web/test`, `cmd`, `internal`, and `anvien-launcher`.

## Classification

| Level | Meaning |
|---|---|
| High | Anvien reports no incoming imports/refs and `rg` found no external source/test usage. |
...
These files had no incoming `IMPORTS` in the Anvien graph and source search did not find product/test usage outside their own file.

| Candidate | Lines | Evidence | Notes |
|---|---:|---|---|
| `anvien-web/src/components/HelpPanel.the-press.tsx` | 395 | Anvien: no incoming imports. `context HelpPanelThePress`: `incoming: {}`. `rg HelpPanelThePress` only matches this file. | Looked like an older/alternate Help panel implementation. The separate `HelpPanel.tsx` candidate was also removed later as product-dead code. |
| `anvien-web/src/components/RightPanel.tsx` | 66 | Anvien: no incoming imports. `context RightPanel`: `incoming: {}`. `rg RightPanel` shows product code uses `RightPanelResizable`, not `RightPanel`. | Likely superseded by `RightPanel.resizable.tsx`. |
| `anvien-web/src/components/settings/ProviderConfigCard.tsx` | 108 | Anvien: no incoming imports. `rg ProviderConfigCard` only matches this file. | Settings UI appears to use local-runtime settings paths directly. |
| `anvien-web/src/config/ignore-service.ts` | 312 | Anvien: no incoming imports. `context shouldIgnorePath`: `incoming: {}`. `rg shouldIgnorePath` only matches this file. | Frontend ignore matcher appears unused; backend has current ignore handling under `internal/ignore`. |
| `anvien-web/src/core/ingestion/cluster-enricher.ts` | 243 | Anvien: no incoming imports. `context enrichClusters`: `incoming: {}`. `rg enrichClusters` only matches this file. | Old browser-side LLM cluster enrichment path; current runtime graph/search is backend/local-runtime driven. |
| `anvien-web/src/core/llm/index.ts` | 28 | Anvien: no incoming imports. `rg` found no imports from bare `core/llm` or `core/llm/index`. | Barrel file is unused; consumers import `session-client`, `types.local-runtime`, or settings modules directly. |
| `anvien-web/src/hooks/useSettings.ts` | 10 | Anvien: no incoming imports. `rg useSettings` only matches this file. | Wrapper around `useChatRuntime`; no current caller. |

Detailed symbols reported by Anvien in these files:
...
| Candidate | Lines | Evidence | Notes |
|---|---:|---|---|
| `anvien-web/src/components/HelpPanel.tsx` | 711 | Anvien source-only import query reports no incoming imports from `anvien-web/src`. `rg HelpPanel` shows only `anvien-web/test/unit/Branding.local-only.test.tsx` imports/renders it. | Removed on 2026-05-27 with the test-only branding assertion. |
| `anvien-web/src/lib/utils.ts` | 3 | Anvien source-only import query reports no incoming imports from `anvien-web/src`. `rg generateId` shows test-only usage through `anvien-web/test/unit/utils.test.ts`. | Removed on 2026-05-27 with `utils.test.ts`. |

## Unused Exports In Live Files

`anvien-web/src/services/backend-client.ts` is a live file, but these exported functions have no incoming Anvien `CALLS/USES/ACCESSES` and `rg` found no call sites outside their own declarations.

| Function | Location | Endpoint/path touched | Evidence |
|---|---|---|---|
| `fetchServerInfo` | `backend-client.ts:308` | `/api/info` | No call sites found by Anvien or `rg fetchServerInfo(`. |
| `grep` | `backend-client.ts:560` | `/api/grep` | No call sites found by Anvien or `rg grep(`. The word `grep` appears in comments/UI labels only. |
...
Anvien found all `cmd/*/main.go` files have zero incoming file imports, which is expected for standalone binaries. Source/workflow search separates true entrypoints from review candidates.

Not candidates:

- `cmd/anvien/main.go`: primary CLI/runtime entrypoint.
...
go list -deps ./cmd/anvien
go list -deps ./cmd/generate-web-contracts
```

Comparing `./internal/...` to production dependencies from `cmd/anvien` plus `cmd/generate-web-contracts` left only:

- `github.com/tamnguyendinh/anvien/internal/providers`
- `github.com/tamnguyendinh/anvien/internal/testutil`
...
| Item | Why excluded |
|---|---|
| `anvien-web/src/main.tsx` | Web entrypoint; no incoming imports is expected. |
| `anvien-web/src/lib/lucide-icons.tsx` | Anvien source-only import query missed alias imports, but `rg lucide-icons` shows many product imports via `@/lib/lucide-icons`. |
...
| Generated files under `anvien-web/src/generated/` | Generated contract artifacts; update generator/source contracts first. |
