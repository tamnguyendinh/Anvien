...
describe("edge types", () => {
  ...
  it("EDGE_SIZE_MULTIPLIERS covers every generated relationship type", () => {
    expect(Object.keys(EDGE_SIZE_MULTIPLIERS).sort()).toEqual(
      [...GRAPH_RELATIONSHIP_TYPES].sort(),
    );
  });
...
describe("graph display inventory helpers", () => {
  ...
  it("groups duplicate INHERITS compatibility edges for display counts", () => {
    const relationships = [
      {
        id: "extends",
        sourceId: "source",
      ...
      {
        id: "inherits-duplicate",
        sourceId: "source",
        targetId: "target",
        type: "INHERITS",
      },
      {
        id: "inherits-standalone",
        sourceId: "other-source",
        targetId: "other-target",
        type: "INHERITS",
      },
    ] as Array<
      Pick<GraphRelationship, "id" | "sourceId" | "targetId" | "type">
    >;

    expect(
      getDisplayGraphRelationships(relationships).map((rel) => rel.id),
    ).toEqual(["extends", "inherits-standalone"]);
    expect(
      getDisplayRelationshipTypeCounts(relationships).get("INHERITS"),
    ).toBe(1);
    expect(
