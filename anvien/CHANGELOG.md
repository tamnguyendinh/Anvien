...
- **Global-install upgrade `ENOTEMPTY`** — devendored `tree-sitter-proto` install lifecycle + preinstall cleanup so `npm i -g anvien@latest` succeeds on top of an older install (#843, #846)
...
- anvien-web dependency bumps: `mermaid` 11.12.2 → 11.14.0 (#860), `tailwindcss` (#861), `jsdom` 29.0.0 → 29.0.2 (#863), `wait-on` 8.0.5 → 9.0.5 (#859), `@vitest/coverage-v8` (#864)
...
- **Runtime dependency on `file:../anvien-shared`** removed from the published package (#803)
...
- **`ANVIEN_HOME` env var** to customize global directory (#746)
...
- **`.anvienignore` negation patterns** now respected (#654)
- **MCP setup** prefers global anvien binary over npx (#653)
...
### Fixed
- **`anvien-shared` module not found** — `anvien-shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `anvien` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `anvien-shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `anvien-shared` package for cross-package type unification (#536)
...
- **Agent context refresh** — `anvien analyze` refreshes Anvien-managed sections in AGENTS.md and CLAUDE.md during normal analysis (#517)
...
### Docs
- Added `anvien-shared` build step to web UI quick start instructions (#585)
- Added enterprise offering section to README (#579)

## [1.4.10] - 2026-03-27

### Fixed
- **MCP server install via npx** — resolve tree-sitter peer dependency conflicts that broke `npx -y anvien@latest mcp` (#537, #538)
...
- `anvien-stable-ops` added to community integrations
...
- `.gitignore` and `.anvienignore` support during file discovery (#231)
