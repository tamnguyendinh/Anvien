...
func TestRuntimeProcessSweepIncludesRepoViteServer(t *testing.T) {
	paths := launcherPaths{
		exePath:    filepath.Join("C:", "repo", "anvien-launcher", "AnvienLauncher.exe"),
		rootDir:    filepath.Join("C:", "repo"),
		homeDir:    filepath.Join("C:", "repo", "anvien-launcher"),
		serverExe:  filepath.Join("C:", "repo", "anvien-launcher", "server-bundle", "anvien-server.exe"),
		backendExe: filepath.Join("C:", "repo", "anvien", "bin", "anvien.exe"),
	}

	script := buildStopRuntimeProcessesScript(paths, 1234)
	for _, want := range []string{"node.exe", "anvien-web", "vite", "--port 5228", "anvien\\bin\\anvien.exe", " serve", "--port 4848"} {
...
func TestConflictingWebRuntimeSweepTargetsOnlyRepoViteServer(t *testing.T) {
	...
	for _, want := range []string{"node.exe", "anvien-web", "vite", "--port 5228"} {
		if !strings.Contains(script, want) {
