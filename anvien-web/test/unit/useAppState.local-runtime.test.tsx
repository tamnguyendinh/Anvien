...
describe('useAppState.local-runtime', () => {
  ...
  it('exposes a stable chat runtime bridge with current repo identity', async () => {
    ...
    await act(async () => {
      appState!.setProjectName('anvien');
    });
    expect(appState!.chatRuntimeBridge.getRepoName()).toBe('anvien');
  ...
  it('defaults graph links to visible and persists toggle state', async () => {
    ...
    expect(localStorage.getItem('anvien.graphLinksVisible')).toBe('false');
    ...
    expect(appState!.areGraphLinksVisible).toBe(true);
    expect(localStorage.getItem('anvien.graphLinksVisible')).toBe('true');
  });

  it('hydrates graph links visibility from localStorage', async () => {
    localStorage.setItem('anvien.graphLinksVisible', 'false');
