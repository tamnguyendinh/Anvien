...
describe('SettingsPanel.local-runtime', () => {
  ...
  it('renders local runtime UI without API key fields', async () => {
    render(<SettingsPanel isOpen={true} onClose={() => {}} repoName="anvien" />);
    ...
    await waitFor(() =>
      expect(fetchSessionStatusMock).toHaveBeenCalledWith({ repoName: 'anvien' }),
    );
  ...
  it('closes the runtime panel', async () => {
    ...
    render(<SettingsPanel isOpen={true} onClose={onClose} repoName="anvien" />);
