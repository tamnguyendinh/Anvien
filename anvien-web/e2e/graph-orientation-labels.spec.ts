...
const createOrientationGraph = () => {
  const nodes = [
    ...
    ...Array.from({ length: 6 }, (_item, index) =>
      makeNode("File", index, "frontend", `anvien-web/src/file-${index}.tsx`),
    ),
    ...Array.from({ length: 4 }, (_item, index) =>
      makeNode("Class", index, "frontend", `anvien-web/src/class-${index}.tsx`),
    ),
    ...Array.from({ length: 4 }, (_item, index) =>
  ...
  const nodes = [
    ...Array.from({ length: 1677 }, (_item, index) =>
      makeNode(
        ...
        index,
        "frontend",
        `anvien-web/src/dense/function-${index}.tsx`,
      ),
    ),
    ...Array.from({ length: 300 }, (_item, index) =>
      makeNode(
        ...
        index,
        "frontend",
        `anvien-web/src/dense/method-${index}.tsx`,
      ),
    ),
...
const getLayoutNodeSpacingDiagnostics = async (page: Page) =>
  page.evaluate(() => {
    ...
    return win.__ANVIEN_WEB_DIAGNOSTICS__?.layoutNodeSpacing ?? null;
  });
...
const getLayoutRingDiagnostics = async (page: Page) =>
  page.evaluate(() => {
    ...
    return win.__ANVIEN_WEB_DIAGNOSTICS__?.layoutRings ?? null;
  });
...
const getScreenNodeSpacingDiagnostics = async (page: Page) =>
  page.evaluate(() => {
    ...
    return win.__ANVIEN_WEB_DIAGNOSTICS__?.screenNodeSpacing ?? null;
  });
...
const getGraphOverviewDiagnostics = async (page: Page) =>
  page.evaluate(() => {
    ...
    return win.__ANVIEN_WEB_DIAGNOSTICS__?.graphOverview ?? null;
  });
...
const getGraphInteractionDiagnostics = async (page: Page) =>
  page.evaluate(() => {
    ...
    return win.__ANVIEN_WEB_DIAGNOSTICS__?.graphInteraction ?? null;
  });
...
const installBrowserPerformanceProbe = async (page: Page) =>
  page.addInitScript(() => {
    ...
    (
      window as typeof window & {
        __ANVIEN_PHASE8_PROBE__?: typeof probe;
      }
    ).__ANVIEN_PHASE8_PROBE__ = probe;
    try {
...
const resetBrowserPerformanceProbe = async (page: Page) =>
  page.evaluate(() => {
    (
      window as typeof window & {
        __ANVIEN_PHASE8_PROBE__?: { reset: () => void };
      }
    ).__ANVIEN_PHASE8_PROBE__?.reset();
  });
...
const getBrowserPerformanceProbe = async (page: Page) =>
  page.evaluate(() => {
    return (
      window as typeof window & {
        __ANVIEN_PHASE8_PROBE__?: {
          snapshot: () => BrowserPerformanceProbeSnapshot;
        };
      }
    ).__ANVIEN_PHASE8_PROBE__?.snapshot() ?? null;
  });
