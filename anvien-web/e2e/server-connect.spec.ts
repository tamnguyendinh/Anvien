...
/**
 * E2E tests for the Anvien web UI — exploring view features.
 *
 * Requires:
 *   - anvien serve running on 127.0.0.1:4848 with at least one indexed repo
 *   - anvien-web dev server running on 127.0.0.1:5228
 *
...
test.beforeAll(async () => {
  if (process.env.E2E) {
    ...
    if (!firstRepoName) test.skip(true, 'No indexed repos - run anvien analyze first');
    return;
  }
  try {
    ...
    if (
      ...
      test.skip(true, 'anvien serve not available on :4848');
      return;
    }
    if (
    ...
    if (backendRes.status === 'fulfilled') {
      ...
      if (!repos.length) {
        test.skip(true, 'No indexed repos — run anvien analyze first');
        return;
      }
      firstRepoName = repos[0].name;
    }
...
async function getRuntimeDiagnostics(
  ...
  return page.evaluate(() => {
    const win = window as typeof window & {
      __ANVIEN_WEB_DIAGNOSTICS__?: RuntimeDiagnostics;
    };
    return win.__ANVIEN_WEB_DIAGNOSTICS__ ?? null;
  });
}
