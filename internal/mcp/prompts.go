...
func promptDefinitions() []promptDefinition {
	return []promptDefinition{
		{
			Name:        "detect_impact",
			Description: "Analyze current changes before committing using AVmatrix change detection, context, impact, and repository freshness rules.",
		...
		{
			Name:        "generate_map",
			Description: "Generate evidence-backed architecture documentation from AVmatrix resources and graph facts.",
			Arguments: []promptArgument{
	...
	return promptMessage{
		...
		Content: promptContent{
			...
			Text: "Analyze the impact of my current code changes before committing.\n\n" +
				"Follow these steps:\n" +
				"1. Obey the active repository rules first. Before graph-based work, verify the AVmatrix index is fresh; if it is stale, run `avmatrix analyze --force` from the repository root before trusting graph facts.\n" +
				"2. Run MCP `detect_changes(" + string(raw) + ")` to find changed symbols, affected processes, App Layers, Functional Areas, and resolution-health impact. If MCP is unavailable, use CLI `avmatrix detect-changes --scope " + scope + cliBaseRefSuffix(baseRef) + " --repo <repo>`.\n" +
				"3. For changed symbols in important or risky flows, run MCP `context({name: \"<symbol>\"})` or CLI `avmatrix context \"<symbol>\" --repo <repo>` and inspect direct callers/callees before drawing conclusions.\n" +
				"4. For high-risk items, run MCP `impact({target: \"<symbol>\", direction: \"upstream\"})` or CLI `avmatrix impact \"<symbol>\" --repo <repo> --direction upstream` for blast radius. HIGH or CRITICAL is a blast-radius warning to report and account for; it is not an edit ban by itself.\n" +
...
func generateMapPrompt(args map[string]any) promptMessage {
	...
	text := "Generate architecture documentation for this codebase using AVmatrix knowledge graph evidence.\n\n"
	if repo == "" {
		text += "Repo selection:\n" +
			"1. Because no `repo` argument was supplied, first READ `avmatrix://repos`.\n" +
		...
		text += "Repo selection:\n" +
			...
			"2. Use URL-escaped resource URI segment `" + repoURI + "` for AVmatrix resources.\n\n" +
...
func generateMapSteps(repoURI string, repoName string) string {
	return "Evidence workflow:\n" +
		...
		"2. If the context resource or active repository rules indicate stale graph data, run `avmatrix analyze --force` from the repository root, then reread the context resource. If you cannot refresh, state that limitation in the output.\n" +
		...
		"7. Use `context`, `impact`, `avmatrix api route-map`, `avmatrix api tool-map`, `avmatrix api shape-check`, `graph-health`, `query-health`, `resolution-inventory`, or `cypher` only when the architecture question needs evidence beyond the resources already read.\n\n" +
		"Output rules:\n" +
