...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("python extract: missing root node")
	}
	if request.Language != scanner.Python {
		return scopeir.ScopeIR{}, fmt.Errorf("python extract: unsupported language %q", request.Language)
	}
...
type collector struct {
	...
	classDefIDsByName map[string]string
...
func newCollector(request Request) *collector {
	return &collector{
		...
		classDefIDsByName: make(map[string]string),
	}
}
