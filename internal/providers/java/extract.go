...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("java extract: missing root node")
	}
	if request.Language != scanner.Java {
		return scopeir.ScopeIR{}, fmt.Errorf("java extract: unsupported language %q", request.Language)
	}
