...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("rust extract: missing root node")
	}
	if request.Language != scanner.Rust {
		return scopeir.ScopeIR{}, fmt.Errorf("rust extract: unsupported language %q", request.Language)
	}
...
type collector struct {
	...
	typeScopeIDsByName    map[string]string
...
func newCollector(request Request) *collector {
	return &collector{
		...
		typeScopeIDsByName:    make(map[string]string),
