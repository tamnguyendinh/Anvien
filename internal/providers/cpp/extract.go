...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("cpp extract: missing root node")
	}
	if request.Language != scanner.CPlusPlus {
		return scopeir.ScopeIR{}, fmt.Errorf("cpp extract: unsupported language %q", request.Language)
	}
