...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("csharp extract: missing root node")
	}
	if request.Language != scanner.CSharp {
		return scopeir.ScopeIR{}, fmt.Errorf("csharp extract: unsupported language %q", request.Language)
	}
