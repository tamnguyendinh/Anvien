...
const csharpParityFixture = `using System;
using App.Util;
...
public class Service : Base, INamed
{
...
func TestExtractCSharpScopeIR(t *testing.T) {
	ir := parseAndExtract(t, "src/Service.cs", "hash-csharp", []byte(csharpParityFixture))

	for _, def := range ir.Definitions {
		if def.FileHash != "hash-csharp" {
			t.Fatalf("definition %s missing file hash: %#v", def.Name, def)
		}
	}
	for _, call := range ir.Calls {
		if call.FileHash != "hash-csharp" {
			t.Fatalf("call %s missing file hash: %#v", call.Name, call)
		}
	}

	requireDefinition(t, ir, "App.Service", scopeir.NodePackage)
	requireDefinition(t, ir, "INamed", scopeir.NodeInterface)
	requireDefinition(t, ir, "Base", scopeir.NodeClass)
	...
	requireQualifiedDefinition(t, ir, "INamed.Save", scopeir.NodeMethod)
	...
	if constructor.OwnerID != service.ID || save.OwnerID != service.ID || normalize.OwnerID != service.ID || repo.OwnerID != service.ID {
		t.Fatalf("owner mismatch: service=%s constructor=%#v save=%#v normalize=%#v repo=%#v", service.ID, constructor, save, normalize, repo)
	}
	if id.DeclaredType != "string" || save.ReturnType != "bool" {
		t.Fatalf("type extraction mismatch: id=%#v save=%#v", id, save)
	}

	requireImport(t, ir, scopeir.ImportNamed, "System", "System", "System")
	requireImport(t, ir, scopeir.ImportNamed, "Util", "Util", "App.Util")
	...
	requireHeritage(t, ir, "INamed", scopeir.HeritageImplements)
...
func TestExtractCSharpAliasImportsCoverNamedBindingLegacyCases(t *testing.T) {
	const source = `using MyApp.Models;
using System;
using Mod = MyApp.Models;
using Svc = MyApp.Services.UserService;

class App {}
`

	ir := parseAndExtract(t, "src/App.cs", "hash-csharp-alias", []byte(source))

	requireImport(t, ir, scopeir.ImportNamed, "Models", "Models", "MyApp.Models")
	requireImport(t, ir, scopeir.ImportNamed, "System", "System", "System")
	requireAliasImport(t, ir, "Mod", "Models", "Mod", "MyApp.Models")
	requireAliasImport(t, ir, "Svc", "UserService", "Svc", "MyApp.Services.UserService")
}

func TestExtractCSharpScopeIRParityFixture(t *testing.T) {
	ir := parseAndExtract(t, "src/Service.cs", "hash-csharp", []byte(csharpParityFixture))
	...
	raw := buffer.Bytes()
	golden, err := os.ReadFile("testdata/csharp_scopeir_signature.golden.json")
	if err != nil {
...
func TestResolveCSharpGraphParityCounts(t *testing.T) {
	ir := parseAndExtract(t, "src/Service.cs", "hash-csharp", []byte(csharpParityFixture))
...
func BenchmarkExtractCSharpScopeIR(b *testing.B) {
	source := []byte(csharpParityFixture)
...
func BenchmarkParseAndExtractCSharpScopeIR(b *testing.B) {
	source := []byte(csharpParityFixture)
	pool := parser.NewPool(nil, parser.PoolOptions{ParseTimeout: time.Second})
	defer pool.Close()
...
func requireAliasImport(t *testing.T, ir scopeir.ScopeIR, local string, imported string, alias string, target string) {
	t.Helper()
	for _, item := range ir.Imports {
		if item.Kind == scopeir.ImportAlias && item.LocalName == local && item.ImportedName == imported && item.Alias == alias && item.TargetRaw != nil && *item.TargetRaw == target {
			return
		}
	}
	t.Fatalf("missing alias import local=%s imported=%s alias=%s target=%s in %#v", local, imported, alias, target, ir.Imports)
}
