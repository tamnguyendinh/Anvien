...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("ruby extract: missing root node")
	}
	if request.Language != scanner.Ruby {
		return scopeir.ScopeIR{}, fmt.Errorf("ruby extract: unsupported language %q", request.Language)
	}
...
func newCollector(request Request) *collector {
	return &collector{
		...
		propertiesSeen:   make(map[string]struct{}),
	}
}
