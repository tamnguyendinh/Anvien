...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("php extract: missing root node")
	}
	if request.Language != scanner.PHP {
		return scopeir.ScopeIR{}, fmt.Errorf("php extract: unsupported language %q", request.Language)
	}
