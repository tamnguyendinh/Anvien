...
func (c *collector) collectScopes(root *sitter.Node) {
	...
	walkKind(root, func(node *sitter.Node, kind string) {
		c.collectScopeCandidateForKind(node, kind)
	})
}

func (c *collector) collectScopesAndContext(root *sitter.Node) {
	c.addScopeCandidate(scopeir.ScopeModule, root)
	walkKind(root, func(node *sitter.Node, kind string) {
		c.collectScopeCandidateForKind(node, kind)
...
func (c *collector) collectScopeCandidateForKind(node *sitter.Node, kind string) {
	switch kind {
	case "type_spec":
...
func (c *collector) innermostScopeID(rng scopeir.Range) string {
	...
	for _, scope := range c.scopes {
		if scope.FilePath != c.filePath {
			continue
		}
		if !rangeContains(scope.Range, rng) {
			continue
		}
