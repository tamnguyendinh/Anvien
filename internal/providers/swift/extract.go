...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("swift extract: missing root node")
	}
	if request.Language != scanner.Swift {
		return scopeir.ScopeIR{}, fmt.Errorf("swift extract: unsupported language %q", request.Language)
	}
...
type collector struct {
	...
	typeLabelsByName      map[string]scopeir.NodeLabel
	returnTypesByCallable map[string]string
...
func newCollector(request Request) *collector {
	return &collector{
		...
		typeLabelsByName:      make(map[string]scopeir.NodeLabel),
		returnTypesByCallable: make(map[string]string),
	}
}
