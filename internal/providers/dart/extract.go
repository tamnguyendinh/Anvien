...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("dart extract: missing root node")
	}
	if request.Language != scanner.Dart {
		return scopeir.ScopeIR{}, fmt.Errorf("dart extract: unsupported language %q", request.Language)
	}
