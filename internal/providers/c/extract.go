...
func Extract(request Request) (scopeir.ScopeIR, error) {
	if request.Root == nil {
		return scopeir.ScopeIR{}, fmt.Errorf("c extract: missing root node")
	}
	if request.Language != scanner.C {
		return scopeir.ScopeIR{}, fmt.Errorf("c extract: unsupported language %q", request.Language)
	}
