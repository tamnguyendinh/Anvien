...
const kotlinParityFixture = `package app.service

import java.util.UUID
import app.util.Names.format
...
class Service(private val repo: Repository) : Base(), Named {
    override fun save(user: User): Boolean {
        val id: String = format(normalize(user))
        this.repo.write(id)
        return true
    }

    private fun normalize(user: User): String {
        return user.id()
    }
}
`

func TestExtractKotlinScopeIR(t *testing.T) {
	ir := parseAndExtract(t, "src/app/service/Service.kt", "hash-kotlin", []byte(kotlinParityFixture))

	for _, def := range ir.Definitions {
		if def.FileHash != "hash-kotlin" {
			t.Fatalf("definition %s missing file hash: %#v", def.Name, def)
		}
	}
	for _, call := range ir.Calls {
		if call.FileHash != "hash-kotlin" {
...
func TestExtractKotlinScopeIRParityFixture(t *testing.T) {
	ir := parseAndExtract(t, "src/app/service/Service.kt", "hash-kotlin", []byte(kotlinParityFixture))
	...
	raw := buffer.Bytes()
	golden, err := os.ReadFile("testdata/kotlin_scopeir_signature.golden.json")
	if err != nil {
...
func TestResolveKotlinGraphParityCounts(t *testing.T) {
	ir := parseAndExtract(t, "src/app/service/Service.kt", "hash-kotlin", []byte(kotlinParityFixture))
...
func BenchmarkExtractKotlinScopeIR(b *testing.B) {
	source := []byte(kotlinParityFixture)
	...
	parsed, err := pool.Parse(context.Background(), parser.Request{
		FilePath: "src/app/service/Service.kt",
...
func BenchmarkParseAndExtractKotlinScopeIR(b *testing.B) {
	source := []byte(kotlinParityFixture)
	pool := parser.NewPool(nil, parser.PoolOptions{ParseTimeout: time.Second})
	defer pool.Close()
