...
func containsRoute(routes []APIRouteContract, method string, path string, responseType string) bool {
	for _, route := range routes {
		if route.Method == method && route.Path == path && route.ResponseType == responseType {
			return true
		}
	}
	return false
