...
func TestCopyQueriesMatchLadybugCSVContract(t *testing.T) {
	...
	want := `COPY Method(id, name, filePath, qualifiedName, startLine, startCol, endLine, endCol, selectionStartLine, selectionStartCol, selectionEndLine, selectionEndCol, isExported, content, description, parameterCount, returnType, appLayer, functionalArea) FROM "C:/tmp/method.csv" (HEADER=true, ESCAPE='"', DELIM=',', QUOTE='"', PARALLEL=false, auto_detect=false)`
	if query != want {
