...
func TestP6DResolvePackageBuildRootsUsesDistinctLaneChildren(t *testing.T) {
	repoRoot := t.TempDir()
	laneRoot := filepath.Join(repoRoot, ".tmp", "lane-a")
	cacheRoot := filepath.Join(laneRoot, "cache-a")
	runtimeRoot := filepath.Join(laneRoot, "runtime-a")
	t.Setenv("ANVIEN_BUILD_REPO_ROOT", repoRoot)
	t.Setenv("ANVIEN_BUILD_LANE_ROOT", laneRoot)
	t.Setenv("ANVIEN_BUILD_CACHE_ROOT", cacheRoot)
	t.Setenv("ANVIEN_BUILD_RUNTIME_ROOT", runtimeRoot)

	roots, err := resolvePackageBuildRoots(repoRoot)
	if err != nil {
		t.Fatalf("resolvePackageBuildRoots returned error: %v", err)
	}
	if roots.LaneRoot != laneRoot || roots.CacheRoot != cacheRoot || roots.RuntimeRoot != runtimeRoot {
		t.Fatalf("resolved roots = %#v, want lane=%q cache=%q runtime=%q", roots, laneRoot, cacheRoot, runtimeRoot)
	}
	if roots.CacheRoot == roots.RuntimeRoot {
		t.Fatalf("cache and runtime roots share one path: %#v", roots)
	}
	for _, path := range []string{roots.CacheRoot, roots.RuntimeRoot} {
		if stat, err := os.Stat(path); err != nil || !stat.IsDir() {
			t.Fatalf("resolved root was not created at %s: %v", path, err)
		}
	}

	t.Setenv("ANVIEN_BUILD_RUNTIME_ROOT", cacheRoot)
	if _, err := resolvePackageBuildRoots(repoRoot); err == nil || !strings.Contains(err.Error(), "must be distinct") {
		t.Fatalf("shared cache/runtime root error = %v, want explicit distinct-root failure", err)
	}
}

func TestP6DResolvePackageBuildRootsGeneratesUniqueLanes(t *testing.T) {
	repoRoot := t.TempDir()
	t.Setenv("ANVIEN_BUILD_REPO_ROOT", repoRoot)
	t.Setenv("ANVIEN_BUILD_LANE_ROOT", "")
	t.Setenv("ANVIEN_BUILD_CACHE_ROOT", "")
	t.Setenv("ANVIEN_BUILD_RUNTIME_ROOT", "")

	first, err := resolvePackageBuildRoots(repoRoot)
	if err != nil {
		t.Fatalf("first resolvePackageBuildRoots returned error: %v", err)
	}
	second, err := resolvePackageBuildRoots(repoRoot)
	if err != nil {
		t.Fatalf("second resolvePackageBuildRoots returned error: %v", err)
	}
	if first.LaneRoot == second.LaneRoot {
		t.Fatalf("generated build lanes unexpectedly share %s", first.LaneRoot)
	}
	for _, roots := range []packageBuildRoots{first, second} {
		if err := assertPackageChild(filepath.Join(repoRoot, ".tmp"), roots.LaneRoot); err != nil {
			t.Fatalf("generated lane escaped repository temp root: %v", err)
		}
		if err := assertPackageChild(roots.LaneRoot, roots.CacheRoot); err != nil {
			t.Fatalf("generated cache escaped lane: %v", err)
		}
		if err := assertPackageChild(roots.LaneRoot, roots.RuntimeRoot); err != nil {
			t.Fatalf("generated runtime escaped lane: %v", err)
		}
	}
}

func TestP6DResolvePackageBuildRootsRejectsOutsideLane(t *testing.T) {
	repoRoot := t.TempDir()
	laneRoot := filepath.Join(repoRoot, ".tmp", "lane-a")
	t.Setenv("ANVIEN_BUILD_REPO_ROOT", repoRoot)
	t.Setenv("ANVIEN_BUILD_LANE_ROOT", laneRoot)
	t.Setenv("ANVIEN_BUILD_CACHE_ROOT", filepath.Join(repoRoot, ".tmp", "other-cache"))
	t.Setenv("ANVIEN_BUILD_RUNTIME_ROOT", filepath.Join(laneRoot, "runtime"))

	if _, err := resolvePackageBuildRoots(repoRoot); err == nil || !strings.Contains(err.Error(), "invalid package build cache root") {
		t.Fatalf("outside-lane cache error = %v, want explicit containment failure", err)
	}
}
