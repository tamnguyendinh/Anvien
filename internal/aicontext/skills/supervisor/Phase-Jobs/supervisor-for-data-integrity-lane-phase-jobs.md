...
- Supervisor must read the 7 most recent Data Integrity reports in `reports/Data-Integrity/`.
...
- If SPEC files conflict, stop and escalate by report to the architect-review lane instead of guessing.
...
2. Check evidence for the Job ID and all unresolved reports.
   ...
   . Read the 7 most recent Data Integrity reports in `reports/Data-Integrity/`.
...
6. If PASS:
   . Update `Docs/execution/progress.md` fully for the reviewed scope so both review-bookkeeping dimensions are correct:
     + tick state
     + final status state such as `APPROVED`
...
7. If FAIL:
   . Update `Docs/execution/progress.md` fully for the reviewed scope so both review-bookkeeping dimensions are correct:
     + tick state
     + final status state such as `REJECTED`
...
- Before ticking `Supervisor`, supervisor must read the 7 most recent Data Integrity reports in `reports/Data-Integrity/`.
...
- After every completed supervisor review, `Docs/execution/progress.md` must be updated fully for the reviewed scope so both review-bookkeeping dimensions match the final verdict:
  . tick state
  . final status state such as `APPROVED` or `REJECTED`
