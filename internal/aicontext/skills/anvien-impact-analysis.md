...
2. Identify the exact symbol, file, route, or tool. If impact returns candidates, use `context symbol`, `context file`, or `--uid` to disambiguate.
3. Run upstream impact before editing. For file edits, prefer `impact file`; for single owner edits, prefer `impact symbol`.
4. Report affected files, App Layers, Functional Areas, execution flows, linked tests, unresolved deltas, and risk.
