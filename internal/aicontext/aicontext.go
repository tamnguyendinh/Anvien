...
func renderAVmatrixBlock(projectName string, stats Stats, skills []GeneratedSkillInfo, noStats bool) string {
	...
	builder.WriteString("|------|---------|\n")
	builder.WriteString("| `query` | Find execution flows and symbols with App Layer, Functional Area, and ResolutionGap summaries when available |\n")
	builder.WriteString("| `context` | 360-degree symbol view with callers, callees, process participation, source-site proof/status, and related ResolutionGap rows |\n")
	builder.WriteString("| `impact` | Symbol blast radius before edits, including affected App Layers, Functional Areas, and resolution-health risks |\n")
	builder.WriteString("| `detect_changes` | Git-diff impact before committing, including changed semantic layers, ResolutionGap changes, and resolution-health impact |\n")
	builder.WriteString("| `rename` | Graph-guided symbol rename |\n")
	builder.WriteString("| `cypher` | Read-only graph queries |\n")
	builder.WriteString("| `route_map` | API route handler/consumer inspection with semantic route, consumer, and flow fields |\n")
	builder.WriteString("| `shape_check` | API response shape drift checks with semantic route and consumer fields |\n")
	builder.WriteString("| `api_impact` | API route pre-change impact with consumer/flow layer summaries and resolution-health impact |\n\n")
	builder.WriteString("## Semantic Graph Fields\n\n")
	builder.WriteString("- `appLayer` and `functionalArea` identify where a node belongs in the product and feature map.\n")
	builder.WriteString("- Proven relationships can include `sourceSiteId`, `sourceSiteStatus`, `proofKind`, `targetRole`, `targetText`, file/range, confidence, and resolution source.\n")
	builder.WriteString("- `ResolutionGap` nodes are persisted diagnostic facts for unresolved, external, ambiguous, unsupported, or non-actionable references; they are not fake resolved in-repo symbols.\n")
	builder.WriteString("- Resolution Health separates resolved references, in-repo analyzer gaps, external unresolved references, non-actionable builtin/standard-library/test-framework references, and unclassified unknowns.\n\n")
