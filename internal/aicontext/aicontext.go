...
func renderAVmatrixBlock(projectName string, stats Stats, noStats bool) string {
	...
	builder.WriteString("| `generate_map` | Evidence-backed architecture map workflow; resolves the repo through `avmatrix://repos` when needed and uses only resources/tools/command output actually read. |\n\n")
