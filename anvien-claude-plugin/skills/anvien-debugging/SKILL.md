...
3. READ anvien://repo/{name}/process/{name}                → Trace execution flow
4. cypher({query: "MATCH path..."})                 → Custom traces if needed
```

> If "Index is stale" → run `anvien analyze` in terminal.
...
3. READ anvien://repo/my-app/process/CheckoutFlow
