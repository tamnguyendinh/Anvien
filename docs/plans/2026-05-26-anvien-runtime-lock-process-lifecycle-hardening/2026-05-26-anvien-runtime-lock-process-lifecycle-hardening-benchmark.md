# Anvien Runtime Lock And Process Lifecycle Hardening Benchmark Ledger
...
| Metric | Unit | Baseline | Latest | Delta | Final target |
|---|---:|---:|---:|---:|---:|
| Live `anvien.exe` processes observed during baseline trace | processes | 1 | 1 | 0 | record |
| Live editor-owned `anvien mcp` processes observed | processes | 1 | 1 | 0 | record |
| Live `anvien analyze` processes observed after analyze completed | processes | 0 | 0 | 0 | 0 |
| Live launcher-owned `serve --port 4848` processes observed | processes | 0 | 0 | 0 | record |
| Process classes discoverable through Anvien diagnostics | classes | 0 | 7 | +7 | >= 5 |
...
| Commands that report Anvien process status | commands | 0 | 1 | +1 | >= 1 |
