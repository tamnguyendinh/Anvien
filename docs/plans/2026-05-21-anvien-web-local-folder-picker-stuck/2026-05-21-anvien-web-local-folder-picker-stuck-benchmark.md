# Anvien Web Local Folder Picker Stuck Benchmark Ledger
...
Evidence: [2026-05-21-anvien-web-local-folder-picker-stuck-evidence.md](2026-05-21-anvien-web-local-folder-picker-stuck-evidence.md)
...
- current `Start-Anvien.html` artifact location and whether dist copies exist;
...
| `Start-Anvien.html` root entry exists | yes | post-build artifact gate |
| `Start-Anvien.html` absent from `anvien-web\dist` | yes | post-build artifact gate |
| `Start-Anvien.html` absent from `anvien-launcher\web-dist` | yes | post-build artifact gate |
| Current protocol command | `E:\Anvien\anvien-launcher\AnvienLauncher.exe "%1"` | registry inspection |
...
| Command | Expected |
| --- | --- |
| `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` | pass |
| Packaged artifact location gate | pass; root present and dist copies absent |
| `npm --prefix anvien-web test -- test/unit/RepoAnalyzer.local-only.test.tsx test/unit/analyze-contract.local-only.test.tsx` | pass; 11 tests |
| `go test ./internal/httpapi -run TestLocalFolderPicker -count=1` | pass |
| `Push-Location anvien-launcher\src; go test . -count=1; Pop-Location; Push-Location anvien-launcher\server-wrapper; go test . -count=1; Pop-Location` | pass |
| `npm --prefix anvien-web test` | pass; 43 files, 349 tests |
| `go test ./cmd/... ./internal/... -count=1` | pass |
| `npm --prefix anvien-web run test:e2e` | pass; 36 passed, 7 skipped |
...
| Metric | Expected |
| --- | --- |
| `Start-Anvien.html` reset protocol still triggers reset | yes |
...
| `Start-Anvien.html` remains root-only user-facing entry | yes |
