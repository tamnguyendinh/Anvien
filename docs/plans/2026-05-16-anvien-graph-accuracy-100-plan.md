# Anvien Go Graph Accuracy 100% Plan
...
- Benchmark ledger: [2026-05-16-anvien-graph-accuracy-100-benchmark.md](2026-05-16-anvien-graph-accuracy-100-benchmark.md)
- Evidence ledger: [2026-05-16-anvien-graph-accuracy-100-evidence.md](2026-05-16-anvien-graph-accuracy-100-evidence.md)
...
Make the Go local Anvien graph for `E:\Anvien` reach `100%` on the measured graph-accuracy gates for this repo.

Canonical target: local Go binary `anvien\bin\anvien.exe`.

Node/MCP Anvien is kept only as a comparison baseline. Do not spend implementation work on Node/MCP unless a later plan explicitly asks for parity.
...
6. A cluster that changes graph extraction or resolution must include focused tests and one full local analyze on `E:\Anvien`.
...
- [x] [P1-B] Make the tracked gate regenerate or consume the Go local API graph deterministically. The report command runs `anvien\bin\anvien.exe analyze --force`, copies the fresh graph snapshot to `.tmp\p1-go-api-graph-20260516.json`, and compares that artifact instead of reading `.anvien\graph.json` as authority.
...
- [x] [P5-A] Run the final Go local graph accuracy gate on `E:\Anvien`. Final result is `100.00%` for `TypeAlias`, `Variable`, direct `CALLS` subset, local `IMPORTS` recall, local `IMPORTS` precision, and all core definition labels.
...
| P1-B | Accuracy gate | API graph input | no stale `.anvien\graph.json` dependency | recorded | recorded | `288c093` | done |
...
- Go local graph accuracy on `E:\Anvien` is `100.00%` for every measured gate in this plan.
