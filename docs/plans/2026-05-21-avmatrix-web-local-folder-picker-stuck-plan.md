...
7. For implementation validation, run the full packaged build before any tests. For this plan the full build gate is `powershell -ExecutionPolicy Bypass -File avmatrix-launcher\build.ps1`, which rebuilds Web dist, backend CLI, launcher, server wrapper, packaged Web dist, and protocol registration.
...
- `TESTING.md` and `RUNBOOK.md` define `Start-AVmatrix.html` as the user-facing launcher entry at repository root. It must not be emitted into `avmatrix-web\dist\` or copied into `avmatrix-launcher\web-dist\`.
...
- Existing frontend unit coverage lives in `avmatrix-web/test/unit/RepoAnalyzer.local-only.test.tsx`; it currently covers local input/submission/completion, but not pending picker cancellation or manual analysis while picker is pending.
...
- Verify `Start-AVmatrix.html` remains only at repository root and is not copied into `avmatrix-web\dist\` or `avmatrix-launcher\web-dist\`.
...
- [x] [P0-E] Record current `Start-AVmatrix.html` artifact locations and protocol command before implementation.
...
- [x] [P3-A] Add `avmatrix-web/test/unit/RepoAnalyzer.local-only.test.tsx` coverage for pending picker state showing a cancel action.
...
- [x] [P3-G] Add or update `avmatrix-web/e2e/onboarding.spec.ts` coverage for chooser success and pending/cancel recovery.

### P4 - Runtime Reset Hidden Execution

- [x] [P4-A] Record the reset-runtime window flashing report in the evidence ledger.
- [x] [P4-B] Audit `Start-AVmatrix.html` reset protocol flow and packaged launcher protocol registration.
- [x] [P4-C] Audit `avmatrix-launcher/src` reset, stop, process sweep, taskkill, and browser-launch paths for visible windows.
- [x] [P4-D] Audit `avmatrix-launcher/server-wrapper` backend launch path for visible windows.
...
- [x] [P4-J] Preserve reset behavior: stale runtime processes are stopped, state is cleaned, and the user can start AVmatrix again.
- [x] [P4-K] Verify `Start-AVmatrix.html` remains a root user-facing file and is absent from `avmatrix-web\dist\` and `avmatrix-launcher\web-dist\` after the packaged build.

### P5 - Validation

- [x] [P5-A] Run the full packaged build before tests: `powershell -ExecutionPolicy Bypass -File avmatrix-launcher\build.ps1`.
- [x] [P5-B] Verify packaged artifact locations after the build: `Start-AVmatrix.html` exists at repository root, `avmatrix-web\dist\Start-AVmatrix.html` is absent, `avmatrix-launcher\web-dist\Start-AVmatrix.html` is absent, and protocol registration points to the intended launcher.
- [x] [P5-C] Run focused frontend tests: `npm --prefix avmatrix-web test -- test/unit/RepoAnalyzer.local-only.test.tsx test/unit/analyze-contract.local-only.test.tsx`.
- [x] [P5-D] Run focused backend tests: `go test ./internal/httpapi -run TestLocalFolderPicker -count=1`.
- [x] [P5-E] Run focused launcher/server-wrapper tests from their module directories: `Push-Location avmatrix-launcher\src; go test . -count=1; Pop-Location; Push-Location avmatrix-launcher\server-wrapper; go test . -count=1; Pop-Location`.
- [x] [P5-F] Run the Web unit suite: `npm --prefix avmatrix-web test`.
- [x] [P5-G] Run the Go runtime suite: `go test ./cmd/... ./internal/... -count=1`.
- [x] [P5-H] Run the Web e2e suite with chooser coverage: `npm --prefix avmatrix-web run test:e2e`.
- [x] [P5-I] Run packaged reset validation from the user-facing path (`Start-AVmatrix.html` and/or `.\avmatrix-launcher\AVmatrixLauncher.exe reset`) and record whether any terminal/helper window flashes. On this Windows workspace, this validation is required unless build/runtime startup is blocked by an external error that is recorded in evidence.
...
- [x] Reset still stops stale runtime processes and allows AVmatrix to start again.
- [x] Reset logging goes to files/status UI, not visible terminal windows.
- [x] `Start-AVmatrix.html` remains at repository root as the user-facing entry and is absent from `avmatrix-web\dist\` and `avmatrix-launcher\web-dist\`.
