# Anvien Web Graph Node Spacing And Overlap Hardening Evidence Ledger
...
- Plan: [2026-05-26-anvien-web-graph-node-spacing-overlap-hardening-plan.md](2026-05-26-anvien-web-graph-node-spacing-overlap-hardening-plan.md)
...
- `docs/plans/2026-05-26-anvien-web-graph-node-spacing-overlap-hardening-plan.md`
- `docs/plans/2026-05-26-anvien-web-graph-node-spacing-overlap-hardening-evidence.md`
- `docs/plans/2026-05-26-anvien-web-graph-node-spacing-overlap-hardening-benchmark.md`
...
- This plan creation is documentation-only, so Anvien was not used for this commit slice.

## E1 - Initial Source Trace From Prior Investigation

Date: 2026-05-26

Status: preliminary; implementation must re-verify with fresh Anvien graph before code edits

Relevant source owners observed:

| Area | Path | Observed responsibility |
|---|---|---|
| Web graph conversion and layout | `anvien-web/src/lib/graph-adapter.ts` | Computes rendered node size caps, cluster node spacing, island radius, deterministic node offsets, island placement, and ring placement. |
| Sigma rendering integration | `anvien-web/src/hooks/useSigma.ts` | Applies rendered node size caps and camera/rendering behavior. |
| Graph canvas | `anvien-web/src/components/GraphCanvas.tsx` | Hosts graph UI and may expose diagnostics or validation hooks if needed. |
| Geometry tests | `anvien-web/test/unit/graph-adapter.edge-geometry.test.ts` | Contains existing island/ring geometry tests, but must be extended with pairwise same-island spacing checks. |
| Label tests | `anvien-web/test/unit/graph-orientation-labels.test.ts` | Protects graph orientation label metadata and overlap guardrails. |
| Browser/e2e tests | `anvien-web/e2e/graph-orientation-labels.spec.ts` | Validates label visibility and overlap behavior in browser. |

Important source search command used during plan creation:

```powershell
rg -n "getClusterNodeSpacing|getClusterIslandRadius|getIslandOffset|MAX_RENDERED_NODE_SIZE|capRenderedNodeSize|golden|GOLDEN_ANGLE|GraphCanvas|useSigma|edge-geometry" anvien-web -g "*.ts" -g "*.tsx"
```
...
```powershell
rg -n "node spacing|island radius|edge gap|overlap|cluster island|graph label|ring label" docs\plans anvien-web -g "*.md" -g "*.ts" -g "*.tsx"
```
...
- fresh Anvien graph counts before graph-based implementation work;
...
graph: nodes=88619 relationships=121516 path=E:\Anvien\.anvien\graph.json
```

Conclusion:

- The Anvien graph was refreshed before graph-based implementation work.
- Subsequent Anvien query/context/impact/detect-changes commands in this plan use this fresh graph unless another stale warning requires a new refresh.
...
```powershell
.\anvien\bin\anvien.exe query "Web graph layout node spacing island radius sigma camera dense node overlap" --repo Anvien
```
...
|---:|---|---|---|
| 1 | `knowledgeGraphToGraphology` | `anvien-web/src/lib/graph-adapter.ts` | Converts backend graph into Sigma graph and applies layout. |
| 2 | `useSigma` | `anvien-web/src/hooks/useSigma.ts` | Sigma rendering, node reducers, camera/rendering integration. |
| 4 | `getClusterIslandRadius` | `anvien-web/src/lib/graph-adapter.ts` | Computes island footprint from node count and node spacing. |
| 5 | `getClusterNodeSpacing` | `anvien-web/src/lib/graph-adapter.ts` | Computes current graph-level spacing bucket. |

Source search:

```powershell
rg -n "getClusterNodeSpacing|getClusterIslandRadius|getIslandOffset|place|ringGap|islandGap|largestAdjacentClusterSpan|largestAdjacentRingSpan|capRenderedNodeSize|MAX_RENDERED_NODE_SIZE|fit|camera|graph.order|Sigma" anvien-web\src anvien-web\test anvien-web\e2e -g "*.ts" -g "*.tsx"
```
...
|---|---|---|
| Rendered size cap | `anvien-web/src/lib/graph-adapter.ts` `MAX_RENDERED_NODE_SIZE`, `MAX_DENSE_RENDERED_NODE_SIZE`, `getMaxRenderedNodeSize`, `capRenderedNodeSize` | Current maximum rendered node size is capped at `3`. |
...
| Sigma rendering | `anvien-web/src/hooks/useSigma.ts` `capNodeReducerSize`, `useSigma` | Node reducer caps `size`; hover/label rendering receives scaled `data.size`. |
| Graph canvas labels | `anvien-web/src/components/GraphCanvas.tsx` `createLayoutRingBounds`, orientation label refresh | Graph orientation label overlay depends on layout coordinates and camera state. |
| Unit geometry tests | `anvien-web/test/unit/graph-adapter.edge-geometry.test.ts` | Existing tests cover rendered size caps, island separation, ring expansion, and non-rail shape, but not pairwise same-island node gap. |
| Browser label tests | `anvien-web/e2e/graph-orientation-labels.spec.ts` | Existing e2e validates labels and label overlap, not dense node-node spacing. |
...
- Used the current `GOLDEN_ANGLE`, `getStableLabelSeed`, `getClusterNodeSpacing`, and `getIslandOffset` formulas from `anvien-web/src/lib/graph-adapter.ts`.
...
| Source | Finding |
|---|---|
| `anvien-web/src/lib/graph-adapter.ts` | `MAX_RENDERED_NODE_SIZE = 3`; `capRenderedNodeSize` clamps graph node `size` to that maximum. |
| `anvien-web/src/hooks/useSigma.ts` | `capNodeReducerSize` applies `capRenderedNodeSize` in Sigma reducers. |
| `anvien-web/src/hooks/useSigma.ts` hover renderer | `const nodeSize = capRenderedNodeSize(data.size || 8, graph.order)` then uses `context.arc(data.x, data.y, nodeSize + 4, ...)`, which treats `nodeSize` as radius. |
| `anvien-web/node_modules/sigma/dist/index-16136237.cjs.prod.js` | Sigma label/hover paths position labels with `data.x + data.size + 3` and draw hover circles using `data.size + PADDING`, which treats display `size` as a radius. |
| `anvien-web/node_modules/sigma/dist/index-16136237.cjs.prod.js` | Edge rendering uses target node `size` as a node radius when offsetting edge/arrow geometry. |
...
```powershell
.\anvien\bin\anvien.exe impact "getIslandOffset" --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact "getClusterIslandRadius" --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact "applyFilterBasedClusteredLayout" --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact "capRenderedNodeSize" --repo Anvien --direction upstream
```
...
| `capRenderedNodeSize` | UNKNOWN / ambiguous | Anvien found the real function plus ResolutionGap candidates; no edit to this symbol is currently planned. |
...
| Path | Change |
|---|---|
| `anvien-web/src/lib/graph-adapter.ts` | Added rendered node radius/diameter/edge-gap/center-distance helpers; added deterministic same-island offset generation with spatial-grid pairwise clearance; updated island radius to use actual centered offsets plus spacing margin; reused expanded island radii for existing island/ring placement. |
| `anvien-web/src/components/GraphCanvas.tsx` | Added layout node-spacing diagnostics derived from the rendered Sigma graph and recorded them into runtime diagnostics for browser validation. |
| `anvien-web/src/lib/runtime-diagnostics.ts` | Added `layoutNodeSpacing` diagnostics and `recordLayoutNodeSpacing`. |
...
```powershell
.\anvien\bin\anvien.exe analyze --force
.\anvien\bin\anvien.exe impact "createLayoutRingBounds" --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact "WebRuntimeDiagnostics" --repo Anvien --direction upstream
```
...
| Path | Change |
|---|---|
| `anvien-web/test/unit/graph-adapter.edge-geometry.test.ts` | Added spacing contract test, dense same-island pairwise spacing test, and regression coverage for the previous dense spiral close-pair condition. |
| `anvien-web/test/unit/runtime-diagnostics.test.ts` | Added runtime diagnostics unit coverage for `layoutNodeSpacing`. |
| `anvien-web/e2e/graph-orientation-labels.spec.ts` | Added dense spacing fixture, runtime diagnostics polling, overlap/gap assertions, and desktop/smaller-viewport screenshot capture for graph node spacing. |
...
- Native runtime file `anvien\bin\lbug_shared.dll` was already up to date.
...
- Updated `anvien-web/test/unit/GraphCanvas.selection-performance.test.tsx` mock for `../../src/lib/graph-adapter` to include `getMinimumNodeEdgeGap` and `getMinimumNodeCenterDistance`.
...
- `anvien-web/test-results/graph-orientation-labels-G-1d271-ted-by-the-default-node-gap-chromium/graph-node-spacing-dense-desktop.png`
- `anvien-web/test-results/graph-orientation-labels-G-1d271-ted-by-the-default-node-gap-chromium/graph-node-spacing-dense-small.png`
...
analyzed E:\Anvien
files: scanned=777 parsed=578 unsupported=199 failed=0
graph: nodes=89032 relationships=122028 path=E:\Anvien\.anvien\graph.json
```
...
Anvien refresh before graph-based impact checks:
...
graph: nodes=89426 relationships=122567 path=E:\Anvien\.anvien\graph.json
```
...
```text
files: scanned=771 parsed=572 unsupported=199 failed=0
graph: nodes=89442 relationships=122583 path=E:\Anvien\.anvien\graph.json
```
...
| Source | Finding |
|---|---|
| `anvien-web/src/lib/graph-adapter.ts` | `getMinimumNodeCenterDistance` returns `12` layout units for max rendered radius `3`; same-island placement rejects candidates below this threshold in graph/layout coordinates. |
| `anvien-web/src/components/GraphCanvas.tsx` | `buildLayoutNodeSpacingDiagnostics` reads raw Sigma graph `x`/`y` attributes and records spacing in layout coordinates before `setSigmaGraph`; it does not use `sigma.graphToViewport` or rendered node display data. |
| `anvien-web/e2e/graph-orientation-labels.spec.ts` | The dense spacing e2e asserts `layoutNodeSpacing` diagnostics from layout coordinates, then captures screenshots; it does not assert screen-projected center distances or rendered pixel gaps. |
| `anvien-web/node_modules/sigma/dist/index-16136237.cjs.prod.js` | Sigma computes viewport positions through normalization/camera matrix paths such as `graphToViewport`, `framedGraphToViewport`, `graphToViewportRatio`, and scales node display size through `scaleSize`. |
...
graph: nodes=88570 relationships=121510 path=E:\Anvien\.anvien\graph.json
```
...
| Source | Finding |
|---|---|
| `anvien-web/node_modules/sigma/settings/dist/sigma-settings.esm.js` | Default `itemSizesReference` is `"screen"`, default `zoomToSizeRatioFunction` is `Math.sqrt`, and `autoRescale` is `true`. |
| `anvien-web/node_modules/sigma/dist/declarations/src/sigma.d.ts` | `scaleSize`, `graphToViewport`, `framedGraphToViewport`, `getGraphToViewportRatio`, `setCustomBBox`, and camera accessors are public Sigma APIs. |
| `anvien-web/node_modules/sigma/dist/sigma.cjs.prod.js` | `scaleSize(size, cameraRatio)` returns a screen size and changes behavior when `itemSizesReference === "positions"`. |

Impact analysis before code edits:

```powershell
.\anvien\bin\anvien.exe impact "buildLayoutNodeSpacingDiagnostics" --repo Anvien --direction upstream --include-tests
.\anvien\bin\anvien.exe impact "useSigma" --repo Anvien --direction upstream --include-tests
.\anvien\bin\anvien.exe impact "recordLayoutNodeSpacing" --repo Anvien --direction upstream --include-tests
.\anvien\bin\anvien.exe impact "applyFilterBasedClusteredLayout" --repo Anvien --direction upstream --include-tests
```
...
| Path | Change |
|---|---|
| `anvien-web/src/lib/graph-screen-spacing.ts` | Added viewport-pixel node spacing diagnostics using Sigma projection APIs and rendered radius data; diagnostics now track coordinate space, viewport size, camera ratio, rendered radius/diameter, minimum center/edge gap, overlap count, and target-gap violations. |
| `anvien-web/src/lib/graph-readable-camera.ts` | Added dense-graph initial camera helper. Large graphs open at a readable zoom around the densest visible island instead of compressing rendered nodes into a fit-to-screen blob. |
| `anvien-web/src/hooks/useSigma.ts` | Switched Sigma node size reference to position-scaled rendering and applies the readable camera after graph load. |
| `anvien-web/src/components/GraphCanvas.tsx` | Records screen-space node spacing diagnostics after graph load, render/resize, and filter/depth changes. |
| `anvien-web/src/lib/runtime-diagnostics.ts` | Added `screenNodeSpacing` runtime diagnostics and `recordScreenNodeSpacing`. |
| `anvien-web/src/lib/graph-orientation-labels.ts` | Prevented ring-label fallback placement from introducing overlaps when a collision-free placement is unavailable. |
| `anvien-web/test/unit/graph-screen-spacing.test.ts` | Added screen-space diagnostics regression coverage. |
| `anvien-web/test/unit/graph-readable-camera.test.ts` | Added dense readable-camera contract coverage. |
| `anvien-web/test/unit/runtime-diagnostics.test.ts` | Added `screenNodeSpacing` diagnostics coverage. |
| `anvien-web/test/unit/GraphCanvas.selection-performance.test.tsx` | Updated Sigma mock for screen-spacing diagnostics. |
| `anvien-web/e2e/graph-orientation-labels.spec.ts` | Added `Function 1677` dense fixture, viewport-pixel spacing assertions, readable camera assertions, and desktop/small screenshots. |

Additional impact analysis:

```powershell
.\anvien\bin\anvien.exe analyze --force
.\anvien\bin\anvien.exe impact --repo Anvien --direction upstream --include-tests --uid "Function:anvien-web/src/lib/graph-orientation-labels.ts:placeGraphOrientationLabels"
```
...
```text
files: scanned=771 parsed=572 unsupported=199 failed=0
graph: nodes=89166 relationships=122218 path=E:\Anvien\.anvien\graph.json
```
