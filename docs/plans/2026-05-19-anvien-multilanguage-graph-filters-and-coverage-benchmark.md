# Anvien Multi-Language Graph Filters and Coverage Benchmark Ledger
...
- graph path: `E:\Restaurant_manager\.anvien\graph.json`
- nodes: `78,350`
- relationships: `130,497`

### Graph Filter Contract Baseline

Generated contract counts come from `internal/contracts/web_ui.go` and the generated Web contract consumed by `anvien-web/src/lib/constants.ts`.
...
- include: `*.ts`, `*.tsx`
- exclude: `node_modules`, `.git`, `.anvien`, `reports`, `Docs`, `dist`, `build`
...
| Metric | Final |
|---|---|
| Top bar Back navigation target | `/Start-Anvien.html` on the current origin |
...
| Resize persistence | `localStorage` key `anvien.leftPanelWidth` |
...
| Anvien full graph filter inventory | pending | recorded |
...
| Top bar Back navigation to `Start-Anvien.html` | not implemented in current product | visible and tested |
...
| `npm --prefix anvien-web test -- --run` | passed, `41` files / `325` tests |
| `npm --prefix anvien-web run test:e2e -- shell-interactions.spec.ts -g "back button|resizes the left dashboard|displays graph filters" --workers=1 --timeout=120000` | passed, `3/3` |
...
| Command | Result |
|---|---|
| `$env:ANVIEN_RESTAURANT_MANAGER_ROOT='E:\Restaurant_manager'; go test ./internal/providers/tsjs -run TestExtractRestaurantManagerTypeScriptHeritageSites -count=1 -v` | passed |
| `Remove-Item Env:\ANVIEN_RESTAURANT_MANAGER_ROOT -ErrorAction SilentlyContinue; go test ./internal/providers/tsjs -run TestExtractRestaurantManagerTypeScriptHeritageSites -count=1 -v` | skipped by design |
...
| Top bar Back navigation target | `/Start-Anvien.html` on current origin |
| Back navigation false reconnect banner | `0` banners in focused e2e |
| Left dashboard resize bounds | `192px` to `480px` |
| Left dashboard persisted width key | `anvien.leftPanelWidth` |
...
| `npm --prefix anvien-web test -- --run test/unit/constants.test.ts` | passed, `26` tests |
| `go build ./cmd/... ./internal/...` | passed |
| `go test ./cmd/... ./internal/...` | passed |
| `npm --prefix anvien-web run build` | passed |
| `npm --prefix anvien-web test -- --run` | passed, `41` files / `325` tests |
