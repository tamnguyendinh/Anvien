...
The Phase 2 layout work must also improve visual spacing, not only add labels. Node islands are not acceptable if nodes look stacked, marched into rails, or compressed into a dense mass. Island radius and internal spiral spacing must expand with visible node count so individual nodes remain distinguishable. The distance between island spiral bands, neighboring islands, and macro rings must leave readable whitespace at normal zoom and remain understandable when zoomed out. The goal is that a user can visually see each island as a separate group, see the ring structure clearly, and inspect node positions without nodes sitting on top of each other.
...
- Web graph node islands use adaptive spacing so nodes do not visually stack, island spiral bands are readable, neighboring islands have clear gutters, and island/ring radius grows with visible node count.
...
- [ ] [P2-F2] Improve adaptive island and ring spacing in the Web graph layout. Update the layout geometry so node spacing, island radius, island spiral band distance, island-to-island gutters, and macro-ring gutters expand from the visible node count and island size. The implementation must avoid nodes visually sitting on top of each other, avoid rail-like/marched-node shapes, keep each island readable at normal zoom, preserve a clear macro-ring structure when zoomed out, and record deterministic geometry tests plus browser screenshot evidence for the spacing result.
