# Anvien GO Agent Context Generation Parity Plan
...
- Evidence ledger: [2026-05-20-anvien-agent-context-generation-parity-evidence.md](2026-05-20-anvien-agent-context-generation-parity-evidence.md)

## Rules

1. Use Anvien for codebase analysis and impact checks before implementation slices that edit functions, classes, methods, or shared generation behavior.
2. For doc-only commits, do not use Anvien.
3. Compare Anvien behavior against the original TypeScript implementation in `E:\anvien-main` before declaring parity.
...
5. Treat `AGENTS.md`, `CLAUDE.md`, `.claude/`, and `.anvien/` as generated or ignored local artifacts unless a phase explicitly changes tracked packaging inputs.
...
Anvien did not generate useful root agent context files by default. In the reported repository state, `AGENTS.md` and `CLAUDE.md` existed but were empty, while the original TypeScript project under `E:\anvien-main` generated full Anvien context during analyze.

The conversion gap is serious because agents rely on these root files to learn:

- when to use Anvien tools;
- when impact analysis and `detect_changes` are required;
- which MCP tools and resources exist;
- which Anvien skills should be consulted;
...
- packaged or embedded base skill sources if Anvien needs rich skill content instead of placeholder skill files;
...
- broad redesign of all agent policies outside the Anvien managed block.

## Parity Contract

The accepted Go behavior must match the original intent, not only the current port:

Superseded policy update, 2026-05-21: analyze must always refresh the managed Anvien sections in
...
|---|---|
| Default `anvien analyze` | Generates or updates `AGENTS.md`, `CLAUDE.md`, and base Anvien skill files after a successful analyze. |
...
| Empty root files | Replaced with a clean Anvien managed block, not left empty and not converted into malformed whitespace. |
| Existing Anvien block | Replaced in place between `<!-- anvien:start -->` and `<!-- anvien:end -->`. |
| Legacy managed block | Replaced when a prior managed Code Intelligence block exists under another marker name. |
| Manual root files without managed block | Preserve manual content and append one Anvien managed block. |
| Base skills | Install rich Anvien skill content, not thin fallback placeholders, unless the rich package source is genuinely unavailable and evidence records the fallback. |
| Generated instructions | Refer only to MCP tools, resources, skills, and CLI commands that exist in Anvien. |
...
- CLI fallback commands must be present if the block tells agents to use Anvien when MCP tools are unavailable.
...
- A default analyze run creates non-empty `AGENTS.md` and `CLAUDE.md` with one valid Anvien managed block.
- A default analyze run installs base skill files under `.claude/skills/anvien/`.
...
- Final smoke on `E:\Anvien` confirms generated root files are non-empty, base skills are installed, generated artifacts remain ignored by git, and analyze still completes successfully.
...
- The original TypeScript implementation imports and calls `generateAIContextFiles` from `E:\anvien-main\anvien\src\cli\ai-context.ts` during analyze in `E:\anvien-main\anvien\src\core\run-analyze.ts`.
...
- The original TypeScript generator reads rich bundled skill Markdown from `E:\anvien-main\anvien\skills\anvien-*.md` before falling back to minimal generated skill content.
- Anvien has AI context generation in `internal/aicontext/aicontext.go`.
- Anvien's base skill installation currently emits thin generated fallback content instead of copying or embedding the rich source skill Markdown.
- Anvien's MCP resources include more surfaces than the current generated root resource table shows, including `anvien://repos`, `anvien://setup`, `anvien://repo/{name}/schema`, and `anvien://repo/{name}/cluster/{clusterName}`.
- Anvien's MCP tools include the core tool names used in the generated block: `query`, `cypher`, `context`, `detect_changes`, `rename`, and `impact`.
...
- [x] [P3-B] Replace existing Anvien managed blocks in place.
...
- [x] [P4-E] Add a CLI fallback section with real `anvien` commands if generated content references terminal fallback behavior.
...
- [x] [P5-A] Decide where rich base skill Markdown should live in Anvien: tracked `skills/`, embedded assets, or another package-local source.
...
- [x] [P6-D] Run default analyze smoke on `E:\Anvien`.
...
The plan is complete only when default Anvien analyze reliably generates accurate, non-empty, useful agent context files and skill files; all relevant flags behave intentionally; generated content matches actual Go MCP and CLI capabilities; focused tests pass; final smoke evidence is recorded; benchmark rows are updated; and the final commit excludes unrelated workspace changes.
