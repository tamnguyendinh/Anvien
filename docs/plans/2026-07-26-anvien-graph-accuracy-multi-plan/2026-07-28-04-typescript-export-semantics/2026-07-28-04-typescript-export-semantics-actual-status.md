...
Date: 2026-07-28
Status: P0-A current-state complete by isolated documentation boundary / Child 03 Pn-C commit `3e25f9ca75c9cb7d59bf228e6cc9aa0b81d738b4` accepted / P4-A opens after Git commit success
...
| Current production owner inventory | Four ScopeIR contract owners, TSJS extraction owners, graph/persistence owners, semantic consumers, and preserve-only carriers are current and impact-classified | exact editable/inspect-only/preserve-only owners with current counts | `correct` | `12` file-detail rows | `E0-P0A-SRC1`, `E0-P0A-FD1`, `E0-P0A-IMPACT1` | open exact four-file P4-A boundary after this docs commit |
...
|-----------------------|----------------------------|-----------|------------|----------|------------|
| `internal/scopeir/{facts.go,kinds.go,ir.go,sort_keys.go}` | source of deterministic module-export syntax truth | P4-A | exact production edit boundary | `E0-P0A-SRC1`, `E0-P0A-FD1`, `E0-P0A-IMPACT1` | access visibility remains independent; tests follow code |
...
Allowed next action: after this exact docs-only P0 commit, implement P4-A in the four ScopeIR contract owners only.
...
| P4-A | first-class contract is absent; four deterministic ScopeIR owners and two test-after-code files are exact | open after this P0 docs commit; edit only `facts.go`, `kinds.go`, `ir.go`, and `sort_keys.go` |
