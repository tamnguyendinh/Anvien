...
| 03 | [TypeScript binding-pattern extraction](2026-07-28-03-typescript-binding-pattern-extraction/2026-07-28-03-typescript-binding-pattern-extraction-plan.md) | binding-pattern defect only | all seven slices, aggregate review, cleanup, Pn-B commit, and Pn-C handoff/detect are closed; isolated Pn-C commit hash is reported externally after Git success |
| 04 | [TypeScript export semantics](2026-07-28-04-typescript-export-semantics/2026-07-28-04-typescript-export-semantics-plan.md) | export-fact defect only | predecessor handoff recorded; P0-A becomes the sole eligible slice after Pn-C commit, while implementation remains blocked pending fresh current-source/file-detail/impact evidence |
...
- Child 04 actual status is refreshed from that handoff: its bounded `21`-export finding remains historical evidence, current P0 source/file-detail/impact/syntax/consumer evidence is still pending, and only Child 04 P0-A may open after the Pn-C closure commit; no P4 implementation or Supervisor loop is opened here.
...
- `E3-PNC-COMMIT1` is the exact eight-path docs/handoff boundary; its final hash is reported externally after Git success because the closure documents are contained in that commit. No push is authorized.
