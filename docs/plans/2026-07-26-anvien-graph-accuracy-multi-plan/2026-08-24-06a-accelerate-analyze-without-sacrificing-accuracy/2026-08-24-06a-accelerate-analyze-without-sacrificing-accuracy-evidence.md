...
- Exact binding `EVIDENCE_EXHAUSTED` is the only exception: the last accepted baseline is preserved, a fresh attempt-local Architect proves no further bounded measurement is justified and releases no production direction, Main Supervisor-protocol verification passes, no candidate/attempt/`NO_KEEP`/streak event is manufactured, and exact unavailable evidence plus strict resume condition are recorded. It terminalizes only that measured child; it is not `SYSTEM_CHARACTERISTIC`, a third attempt, a speedup, an accuracy waiver, or an ordinary `BLOCKED_OPEN` shortcut.
...
### `E2-P2A-A005CHEAPAPP1/RESTAURANT1` — Two Target-Separated Candidate Measurements Recorded

Status: `A005_TWO_TARGET_MEASUREMENTS_RECORDED / A005_SUPERVISOR_PENDING / D001_STREAK_1`.

- Cheapapp task/report: `01a03ffa-d63b-79f3-980e-d5e38256f550`; `E:\Anvien\reports\Investigation\rp_child06a_a005_cheapapp_benchmark_frozen_17child.md`; report SHA-256 `D5BDA2B8B44D92AAEF224FE86A5B8F4C52E651287BE2328ED4037A80EE70DCD7`. Corrected packet root `E:\Anvien\.tmp\child06a_a005_cheapapp_frozen_17child_20260827_retry1`; PID `17616`; exit `0`; D001/parent/analyzer/process `3.036901000 / 18.160962900 / 95.376559900 / 203.598831900 s`; A003 deltas `-0.410945300 / -2.311639400 / +1.844585000 / +107.968183700 s`; denominator `calls=27890; files=887`; `30/30`, `17/17`, `2675` conserved intervals, zero overlap.
- Restaurant task/report: `01a03ffb-7d5a-7ac0-ae93-75dcb8cd2fa9`; `E:\Anvien\reports\Investigation\rp_child06a_a005_restaurant_manager_benchmark_frozen_17child.md`; report SHA-256 `0C3C0D6D4E3CE1C400FB9A3B832F96743AB3D5047DEEC2EA02E25FE7FD5E407C`. Corrected packet root `E:\Anvien\.tmp\child06a_a005_restaurant_manager_frozen_17child_20260827_retry1`; PID `17228`; exit `0`; D001/parent/analyzer/process `9.142619400 / 19.678482100 / 122.900035100 / 162.290202200 s`; A003 deltas `-0.258965900 / -1.172310700 / +24.879488400 / +61.193290300 s`; denominator `calls=86030; files=1234`; `30/30`, `17/17`, `3716` conserved intervals, zero overlap.
- Both corrected packets use frozen executable `0178DAC396E0DD4D0BECCE7B803378A3400013F0B508EC95F9221A0020CF96F3`, include the required `ANVIEN_OP001_RESOLUTION_CPU_PROFILE`, and pass exact ordered evidence, canonical Graph JSON, stdout/stderr, graph/DB, file/parser, resolution-counter and non-timing semantic parity against their own A003 bases. Resource totals are exposed; the private `O(U+B)` canonical-payload lifecycle counter is `NOT EXPOSED` and is not inferred.
- The initial launches exited `1` because Main omitted the required profile environment input. They produced no benchmark and remain recorded in the same reports as measurement-support failures only. The corrected `retry1` launches were explicitly authorized under distinct roots; neither failure nor recovery is a production attempt, no-KEEP result, retry of accepted A003 evidence, or streak/checklist/baseline change.
- The corrected Cheapapp interval `2026-08-26T21:41:20.947306Z..21:44:44.5461379Z` contains the corrected Restaurant interval `2026-08-26T21:41:56.7304748Z..21:44:39.0222374Z`; this overlap was explicitly allowed by the target-separated parallel measurement contract and is recorded for Supervisor/Main interpretation of analyzer/process wall comparability. It does not alter the measured packet facts or authorize a disposition.
- Next owner: exactly one fresh visible A005 Supervisor reviews the exact candidate plus both target packets for correctness/equivalence/output/lifecycle/resource. Main alone decides `KEEP`, `REWORK`, or `ROLLBACK` afterward.

### `E2-P2A-A005REVIEW1/DECISION1` — Supervisor PASS, Main NO_KEEP, Rollback Pending

- Supervisor task/report: `01a04017-7e78-7392-b09c-1487b2a404f7`; `E:\Anvien\reports\Supervisor\rp_supervisor_260827_by_gpt-5_child06a_a005_outcome_serialization.md`; SHA-256 `AE6863BF27815894F9526704244F06D861A4E46914A8FCC65A1B01DC8F373C82`; exact verdict `SUPERVISOR_A005_PASS`.
- Review closes all five correctness/equivalence/output/lifecycle/resource questions. It records both production files HIGH and every edited production symbol CRITICAL as blast-radius scope, exact four-path hashes, no public/persisted/fallback/second-encoder drift, production-first test coverage, unchanged known golden truth, exact two-target parity, structural `O(U+B)` proof, runtime lifecycle field `NOT EXPOSED`, and corrected-run overlap `162.292 s` as a timing limitation only.
- Main disposition is `A005_NO_KEEP`: binding plan rules require lower D001, parent, and process elapsed time on both targets plus Supervisor PASS. D001 and parent are lower on both targets, but process wall is higher by `+107.968183700 s` on Cheapapp and `+61.193290300 s` on Restaurant. The A003-only Owner exception does not apply to A005; no candidate value is promoted.
- D001 unsuccessful streak changes `1 -> 2`. Parent/D001 remain unchecked; D002-D017 remain queued; A003 remains the accepted baseline. A005 is not terminal and does not open another child, P3, or Child 07.
...
### `E2-P2A-A006M1DIRECT1` — Target-Separated Direct-Callee Attribution Ready

Status: `A006_M1_DIRECT_CALLEE_ATTRIBUTION_READY / A006_ARCHITECT_PENDING / D001_STREAK_2`.

- Measurement task/report: `01a04061-02fe-7f02-9c4c-f9caa6c0413c`; `E:\Anvien\reports\Investigation\rp_child06a_a006_m1_direct_callee_attribution.md`; SHA-256 `9CAAD9F8754D8AAB2948C1B1A9DB6C5F6532B7ECE1B516612FF6FE0F935087D2`; exact terminal marker `A006_M1_DIRECT_CALLEE_ATTRIBUTION_READY`.
- Main handoff review: `E:\Anvien\reports\Supervisor\rp_supervisor_260827_071409_by_gpt-5_child06a_a006_m1_handoff.md`; SHA-256 `D187443C4DA28A62DFA7B5DDBAF5A1BBCE78D83FB706C03CB91D53D3E12276F7`; verdict `PASS`. The review corrected one report-only cross-target gap value from rounded `261,000,000,000` to exact timestamp delta `261,011,279,700 ns`; no build or target reran.
- Recovery build: unchanged overlay, manifest, script, production, and tests; one invocation with provenance `AttemptId=A006` at `frozen-build-retry1`; build/root exit `0/0`. Executable version/bytes/SHA-256 `1.2.8 / 73,836,032 / 0362FE211C2E072988EF61B662FC4F0D2160437F520F4525CDA59DDE52FB57A7`; DLL SHA-256 `20CBD87840483A2053CFF3FC2DB23A86DD802B8915D86509D41A4B709624CDB7`; provenance SHA-256 `80BCC23E20518815CB5B7A94C21B2524B7984F6B5C32475CFD45444AA746D887`; schema/attempt/exits `1/A006/0/0`; exact overlay/candidate/native identities `2/4/3`.
- Cheapapp: exactly one launch, PID `19288`, exit `0`, interval `2026-08-27T06:51:47.8464038+07:00..06:53:53.5496640+07:00`; `30/30` operations, `17/17` children, `2675` exclusive intervals, overlap `0`; parent/child-sum/residual `19,018,779,500 / 19,001,125,700 / 17,653,800 ns`; D001 `3,048,999,300 ns`, denominator `calls=27,890; files=887`.
- Cheapapp ten ordered groups, duration/invocations: `source_context 43,637,500/27,890`; `binding_receiver 78,596,800/26,079`; `scoped_same_file 53,094,400/29,686`; `member_import 186,935,000/47,123`; `go_same_package 972,300/8,501`; `global_lookup 14,767,200/4,118`; `typescript_lookup_record 686,469,300/23,430`; `evidence_emission 1,954,984,600/44,488`; `direct_site_identity 9,858,900/40,360`; `resolve_call_residual 19,683,300/27,890`. Sum equals D001 exactly; residual is nonnegative.
- Restaurant Manager: exactly one launch, PID `17244`, exit `0`, interval `2026-08-27T06:58:14.5609437+07:00..07:00:35.3795083+07:00`; `30/30` operations, `17/17` children, `3716` exclusive intervals, overlap `0`; parent/child-sum/residual `18,605,489,700 / 18,582,446,900 / 23,042,800 ns`; D001 `8,297,125,100 ns`, denominator `calls=86,030; files=1,234`.
- Restaurant ten ordered groups, duration/invocations: `source_context 204,263,300/86,030`; `binding_receiver 185,310,600/105,656`; `scoped_same_file 266,276,000/65,948`; `member_import 308,333,200/167,208`; `go_same_package 3,946,043,200/16,011`; `global_lookup 11,392,500/13,781`; `typescript_lookup_record 435,161,600/44,775`; `evidence_emission 2,774,855,300/164,333`; `direct_site_identity 120,948,600/156,976`; `resolve_call_residual 44,540,800/86,030`. Sum equals D001 exactly; residual is nonnegative.
- Sequentiality: Restaurant started `261,011,279,700 ns` after Cheapapp ended; cross-target capture overlap `0`; both immediate competitor lists are empty. Process packets record one launch each, identical Anvien/target HEAD and status before/after, and staged set `0 -> 0`.
- Equivalence: independent normalized comparison against each accepted A003 benchmark is exact after excluding timing/resource fields and the authorized A006-M1 measurement field. Both target Graph JSON, stdout, and stderr hashes exactly equal A003; graph/DB rows, files/parser, resolution counters, diagnostics/outcomes, and ordered Evidence are unchanged.
- Resource values are attribution-era observations only. Cheapapp A003 -> M1 start/end/max-Sys deltas are `+1,768 / +252,303,448 / +27,164,672` bytes; Restaurant `-3,896 / -36,632,576 / +364,048,384` bytes.
- State effect: accepted A003 remains authoritative; A006-M1 is not a production attempt, candidate, Supervisor result, no-KEEP event, baseline promotion, or terminal disposition. D001 streak remains `2`; every parent/child checkbox and queue remains unchanged. Next owner is one fresh visible A006 Architect; Planner/Coder remain locked.

### `E2-P2A-A006ARCH2` — Post-M1 Architect Finds No Safe Production Direction

Status: `ARCHITECT_A006_NO_SAFE_DIRECTION / A006_M2_OR_STOP_PENDING / D001_STREAK_2`.

- Fresh Architect task/report: `01a0409e-6403-7412-9d8b-b9c6f5496464`; `E:\Anvien\reports\system-architect\rp_system-architect_260827_by_gpt-5_child06a_a006_residual_direction.md`. The report preserves `ARCHITECT_A006_NEEDS_MEASUREMENT_INPUT` as pre-M1 history and appends one superseding post-M1 decision.
- Fresh Architect graph: HEAD `80ced4d7191fc70eb97187f626b89f64e2fd779e`; `2264` scanned / `766` parsed / `0` failed; graph `124418 / 171284`; `resolve.go` HIGH and `resolveCall` CRITICAL (`31` impacted symbols, `13` files, `7` modules, `32` processes). No source/test/plan/ledger/script/target edit occurred.
- Exact verdict: `ARCHITECT_A006_NO_SAFE_DIRECTION`.
- Selection result: all ten M1 groups conserve D001 but none proves one materially new two-target removable mechanism plus exact owner and synchronous cost-removal design. `go_same_package` has a concrete whole-`defsByFile` scan but is Restaurant-dominant (`3,946,043,200 ns`) and near-zero on Cheapapp (`972,300 ns`). `typescript_lookup_record` and `evidence_emission` are material on both targets but combine mandatory per-site/carrier work with accepted A002/A003 and rejected A004/A005 descendants. The remaining groups are small or composite and lack per-callee/hop/key/candidate evidence needed to justify a new cache/index rather than an A001 variant.
- No production direction, owner, test owner, algorithm, gain, Planner/Coder/Supervisor transition, new measurement, streak increment, terminalization, or checklist/queue change is released. A006-M1 remains attribution-only and is not a failed production attempt.
- Report-only boundary: exact report diff `+101/-2`; scoped `git diff --check` exit `0`; staged count `0`.

### `E2-P2A-A006MAINVERIFY1` — Main Architect-Handoff Verification PASS

Status: `PASS / NO PRODUCTION RELEASE`.

- Main Supervisor-protocol report: `E:\Anvien\reports\Supervisor\rp_supervisor_260827_075505_by_gpt-5_child06a_a006_architect_handoff.md`.
- Independent current-source inspection confirmed: `ResolveBoundInto` remains sequential; `resolveCall` owns the reported composite lookup/emission path; `resolveGlobalCallName` already consumes `defsByName`; `resolveSameFileName` scans a file-local slice; `resolveGoSamePackageFunction` scans `defsByFile` by Go package; TypeScript lookup still feeds mandatory per-site authority/outcome/diagnostic recording.
- Current Git boundary at verification: HEAD `80ced4d7191fc70eb97187f626b89f64e2fd779e`; only the Architect report was modified before the Main review report; staged set empty; scoped diff-check PASS. No accepted gate was rerun.
- Planner authoring freshness for this state sync: `anvien --help` PASS; one fresh `anvien analyze --force` exit `0`, `2265` scanned / `766` parsed / `0` failed, graph `124434 / 171300`; all four ledgers are current/non-stale LOW-risk documentation with `2` inbound / `1` outbound / `0` unresolved relationships.
- State effect: accept only the `NO_SAFE_DIRECTION` architecture handoff. Accepted A003, D001 streak `2`, every checkbox, and every queue stay unchanged. Main permits one bounded existing-Architect `M2_OR_STOP` decision; no evidence capture or Planner/Coder opens before that decision.

### `E2-P2A-A006M2ARCH1` — One Bounded M2 Measurement Contract Ready

Status: `ARCHITECT_A006_M2_READY / A006_M2_MEASUREMENT_PENDING / D001_STREAK_2`.

- Existing visible Architect task/turn: `01a0409e-6403-7412-9d8b-b9c6f5496464` / `01a040ca-02fa-7d71-84de-4dc3aac6acc9`; report `E:\Anvien\reports\system-architect\rp_system-architect_260827_by_gpt-5_child06a_a006_residual_direction.md`; exact terminal verdict `ARCHITECT_A006_M2_READY`.
- Fresh Architect graph at HEAD `d6b5b954d02ea2f908e7319bebdfe5a29c6a9fd7`: `2265` scanned / `766` parsed / `0` failed, graph `124436 / 171302`; `resolve.go` remains HIGH and `resolveCall` CRITICAL. Report-only diff is `+196/-1`, scoped diff-check PASS, staged count `0`.
- M2 selects only M1 `typescript_lookup_record`, measured at `686,469,300 ns / 23,430` invocations on Cheapapp and `435,161,600 ns / 44,775` on Restaurant Manager. It asks whether the D001-only `resolveCall -> lookupTypeScriptMember -> repositoryReceiverClaimed` recheck has positive exclusive elapsed beyond a matched adjacent timer control on both targets.
- Current source proves the control-flow question: `resolveCall` computes `repositoryReceiverClaimed`, assigns it to `externalBlocked`, and enters the TypeScript member fallback only when false; `lookupTypeScriptMember` recomputes the same predicate before lookup, with no intervening mutation of the claim inputs. D002/D003 helper callers are preserve-only.
- Allowed measurement surface is exactly new overlay copies of `internal/resolution/resolve.go` and `internal/resolution/types.go`, seeded from the accepted M1 overlay hashes. Canonical production, tests, scripts, ledgers, and target source remain unchanged.
- The new metric is one fixed seven-row TypeScript-family split plus scalar counters. It must prove subgroup-to-parent conservation, parent-to-M1-family identity, all exact count predicates, nonnegative remainders, existing ten-group-to-D001 conservation, and overlap `0`; no per-site list, map, cache, global/cross-run state, concurrency, I/O, or M1-family expansion is allowed.
- Build/run contract: one new root `E:\Anvien\.tmp\child06a_a006_m2_typescript_member_recheck_20260827`; unchanged `scripts/build-a00x-benchmark.ps1` invoked exactly once with provenance `AttemptId=A006`; then Cheapapp and Restaurant Manager run sequentially, exactly once each, after zero-competitor preflight, preserving `30/30`, `17/17`, denominators, graph/DB/semantic/order/output parity and cross-target overlap `0`.
- Completion is two valid target packets plus `E:\Anvien\reports\Investigation\rp_child06a_a006_m2_typescript_member_receiver_recheck.md`. The exact owner is exposed only if both targets have rechecks, positive recheck-minus-control elapsed, all rechecks false, and every count/conservation gate passes. Otherwise the owner is falsified. M3, another family split, cache-key packet, broad instrumentation, production release, and direct Planner/Coder transition are forbidden.

### `E2-P2A-A006M2MAINVERIFY1` — Main M2 Architect-Handoff PASS

Status: `PASS / MEASUREMENT RELEASE ONLY`.

- Main review report: `E:\Anvien\reports\Supervisor\rp_supervisor_260827_085245_by_gpt-5_child06a_a006_m2_architect_handoff.md`; verdict `PASS`.
...
### `E2-P2A-A006EXHAUST1` — D001 Evidence-Exhausted Terminal Transition

Status: `D001_EVIDENCE_EXHAUSTED_TERMINAL / D001_STREAK_2 / D002_CURRENT_BASIS_RECORDED / D002_RESIDUAL_ATTRIBUTION_PENDING / ARCHITECT_LOCKED`.

- Source evidence is exactly `E2-P2A-A006ARCH3/A006MAINVERIFY2/A006BLOCK1`: accepted A003 is preserved; the fresh attempt-local Architect proves no further bounded measurement is justified and releases no production direction; Main Supervisor-protocol verification passes; and the exact unavailable evidence is no evidence-proven two-target synchronously removable D001 owner outside A001-A005 after the sole eligible final split was falsified.
- No A006 production candidate, production attempt, `NO_KEEP`, third unsuccessful event, streak change, measurement value, or production commit is created. M1/M2 remain attribution-only. D001's accepted A003 values, denominators, parent/process basis, and streak remain unchanged; streak is exactly `2`.
- This is not `KEEP`, `NO_KEEP`, `ROLLBACK`, `SYSTEM_CHARACTERISTIC`, a speedup, a third attempt, or an accuracy waiver. It is not a generic size/convenience skip and cannot apply to an ordinary unresolved `BLOCKED_OPEN` row.
- Strict D001 resume condition: genuinely new external evidence or an explicit Owner boundary change that invalidates the recorded exhaustion basis. Repeating M1/M2, opening M3, or manufacturing an A001-A005 variant is insufficient.
