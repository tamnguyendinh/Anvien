...
| Field | Current value |
|-------|---------------|
| Attempt state | `A004 SUPERVISOR_PASS / NO_KEEP / ROLLBACK_COMPLETE / A005_CURRENT_BASIS_RECORDED / A005_ATTRIBUTION_COMPLETE / A005_ARCHITECT_COMPLETE / A005_PLAN_COMPLETE / A005_MAIN_VERIFIED / CODER_A005_READY_FOR_MEASUREMENT_HANDOFF / MAIN_HANDOFF_PASS / A005_FROZEN_BUILD_FAILED / OVERLAY_BUNDLE_ADAPTATION_RECOVERY_READY / MEASUREMENT_LOCKED / D001_STREAK_1`; Architect task `01a03f5e-b844-7ac3-b23b-b9c9cdc0374d`; Planner task `01a03f86-cd96-7573-97b1-52d9548768fb`; Coder task `01a03fa8-680f-7400-9ba6-f0d1db5c59dd`; accepted A003 checkpoint remains baseline; all checkboxes unchanged |
| Attempt ID | `A005` |
| Attempt goal | reduce D001 `resolve_calls`, parent `resolution`, and process elapsed time on each target independently by making the existing record-time marshal the sole canonical encoding event per retained `SourceSiteID`, while preserving exact semantic/error/byte/order/output/lifecycle behavior |
...
| Planner refresh evidence | `E2-P2A-A005PLAN1`; report `reports/planner/rp_planner_260827_by_gpt-5_child06a_a005_outcome_serialization.md`; exact architecture-to-plan translation is complete and accepted by `E2-P2A-A005MAINVERIFY1` |
| Transition authority | `E2-P2A-A004REVIEW1/DECISION1/ROLLBACK1`, `E2-P2A-A005CURRENT1/ATTRIB1/ARCH1/PLAN1/MAINVERIFY1`; Main verification report `reports/Supervisor/rp_supervisor_260827_025255_by_gpt-5_child06a_a005_planner_handoff.md` is `PASS`; exactly one visible Coder may be released next |
| Future pre-edit Anvien gate | after Main verification and separate Coder release: `anvien --help` -> exactly one fresh `anvien analyze --force` -> file-detail for `internal/resolution/outcome.go` and `internal/resolution/resolve.go` -> exact upstream impact for every symbol to be edited; record HIGH/CRITICAL warnings without treating them as bans |
| Allowed production/test surfaces | production only private collector state/init, `record`, one private finalized bundle, `finalize`, and `projectResolutionOutcomes` in `internal/resolution/outcome.go`, plus only the existing `finalize -> project -> error/result ResolutionOutcomes` wiring block in `internal/resolution/resolve.go`; tests only after production in new `internal/resolution/outcome_serialization_test.go` plus minimal mechanical adaptation of three direct private `finalize` calls and one direct `projectResolutionOutcomes` call in `internal/resolution/p6c3_structured_outcome_test.go` |
| Production algorithm | keep the existing cloned semantic map and add one private run-scoped `SourceSiteID -> canonical JSON string` sidecar; unique valid `record` retains the clone then calls unchanged `marshalResolutionOutcome` once at the same boundary and stores bytes only on success; equal duplicates reuse the prior immutable clone/string with `added=false`; conflicts/marshal failures retain current first-error behavior; `finalize` returns a private SourceSiteID-sorted semantic-values-plus-sealed-sidecar bundle and fails closed on non-bijective coverage; `projectResolutionOutcomes` consumes/validates that bundle with no marshal or fallback; `ResolveBoundInto` passes the bundle to projection and unwraps only `values` into the public result |
| Test contract | after production is correct, prove one canonical string across every status family, immediate diagnostic/final relationship/reference byte parity, duplicate/conflict/first-error/`added` behavior, deep clone immutability, record-time non-finite-weight marshal failure, missing/extra encoding coverage fail-closed, duplicate finalized sites, resolved/unresolved overlap, diagnostic/carrier drift rejection, SourceSiteID order, complete evidence order, and repeated-run determinism; test-local expected-byte logic never becomes a production encoder/fallback |
| Build/test contract | after actual build-holder proof and `anvien doctor locks/processes`, require canonical full build exit `0` before tests; then run the A005 focused tests and the nine named P6B/P6C3/P6D/Graph JSON/Ladybug regressions, followed by `internal/resolution`, `internal/graphhealth`, `internal/analyze`, `internal/lbugload`, and `internal/lbugnative`; record the unchanged preserve-only golden truthfully and block on any new/changed failure |
| Measurement contract | reuse unchanged `scripts/build-a00x-benchmark.ps1`; mechanically regenerate/verify the overlay-mapped `resolve.go` from exact A005 production wiring plus accepted 17-child instrumentation, keep `types.go` mapping unchanged absent exact compile evidence, exclude all A004 bytes, and record independent one-launch Cheapapp and Restaurant packets against each target's own accepted A003 basis with D001/parent/analyzer/process, `30/30`, `17/17`, denominators, conservation/zero overlap, full ordered evidence, graph/DB/output/semantic/resource parity, and `O(U+B)` one-payload proof; never rerun, average, or combine accepted bases |
...
| Exact rollback | remove only the A005 collector sidecar/init, `record` reuse hunk, private finalized bundle/`finalize` return, strict projection-consumer change, narrow `ResolveBoundInto` wiring, new focused test file, mechanical existing-test adaptations, and rejected A005 frozen/overlay packet; restore projection-time marshal/local map only as the exact A005 rollback; preserve A001-A003, WAL, P1 instrumentation, reports/ledgers/targets/protected work and the already-restored A004 hashes `4BFE1976766FBF2E2257102070FF43CAC6D757E32E71DD583F8824781EAB6A8E` / `AD3DCA9E82EACFB31137560636B59D426A063AEB967613E724D5EE3017AD5812` |
| Mandatory STOP | no source/test edit until the sole released Coder completes its own fresh Anvien/file-detail/impact gate; STOP on any owner/symbol outside the exact list, edit to `marshalResolutionOutcome` or immediate diagnostic callers, moved validation/initial marshal timing, changed clone/conflict/first-error/`added`/SourceSiteID behavior, non-identical immediate bytes, re-encoding fallback/second encoder/missing-byte tolerance, A004-order drift, resource bound beyond `O(U+B)`, build/new-test/golden change, stale/mixed overlay, missing/incomparable target packet, unexplained sibling/resource regression, or D002-D017/P3/Child 07 opening; one unsuccessful A005 would move streak `1 -> 2`, never terminal `3` |
| Coder status | `CODER_A005_IMPLEMENTATION_VALIDATED_READY_FOR_MAIN_HANDOFF`; Main source/test/report review `PASS`; exact four-path candidate remains uncommitted and unstaged |
| Next action | preserve failed build root; patch only the copied A005 overlay’s two projection-denominator calls and one result-count call from the finalized bundle to `outcomes.values`, require recovered overlay SHA-256 `304F65E40629AE9B32803BA3D61ECD093022481381C948D8CF0B09AD75BFF788`, then run one recovery build into a new root; no script edit or target analyze |
...
- [ ] P2-A: Optimize Every Measured Parent And Child In Largest-First Order.
  ...
  - Work Steps:
    ...
    2. Select the largest unchecked child, bind its exact goal/current child-parent-total basis/denominator/cause/owner/complete call path, then open a new Visible Architect and obtain the exact attempt-scoped decision.
       ...
       - Current A005 cursor: `E2-P2A-A005CURRENT1/ATTRIB1/ARCH1` records the accepted A003 target-separated basis, A004 `NO_KEEP / ROLLBACK_COMPLETE`, eager record-time outcome encoding plus projection re-encoding cause, exact two-file private owner boundary, HIGH/CRITICAL blast radius, and one canonical record-time byte-owner direction. The finalized report returns `ARCHITECT_A005_READY_FOR_PLANNER`; no architecture reinterpretation or A001-A004/WAL rerun is required.
    3. Planner refreshes the Current P2-A Attempt card and these execution steps to the Architect decision before Coder begins.
       ...
       - Current cursor: A003 remains closed at checkpoint `b6bf45bce95323aa6b53b182edfea8628bd8b463`; WAL checkpoint `0f3a572331dd23d17688886fcbfebeb7d37ee35d` is complete; A004 is `SUPERVISOR_PASS / NO_KEEP / ROLLBACK_COMPLETE`. A005 is `A005_FROZEN_BUILD_FAILED / OVERLAY_BUNDLE_ADAPTATION_RECOVERY_READY / MEASUREMENT_LOCKED / D001_STREAK_1`; parent/D001 remain unchecked and D002-D017 remain queued.
    4. The sole Coder completes fresh required Anvien graph/file-detail/impact, implements only the refreshed production direction, edits authorized tests only after production is correct, performs the exact build-holder preflight, obtains canonical full-build `PASS`, and only then executes focused and `internal/resolution` package tests.
       ...
       - Current A005 cursor: no source/test edit or Coder exists. Main must verify PLAN1 and separately release a future Coder before this step can start; P2-A/parent/children remain unchecked.
    5. After A005 production/build/test validation, record two independent candidate measurements without combining them, using each target's accepted A003 artifact as that target's `before` and one identifiable A005 candidate containing only the authorized PLAN1 production bytes plus the accepted measurement overlay as `after`.
       ...
       - Current A005 cursor: both future A005 packets are missing and locked behind Main verification plus Coder/build/test completion. Accepted A003 packets are the unchanged separate before bases and must not be rerun or reinterpreted. Only after both A005 packets exist may one fresh visible A005 Supervisor open.
    6. Record disposition, promote only an eligible child+parent+end-to-end `KEEP`, and keep the same child active until terminal.
       ...
       - Current cursor: A001/A002/A003 are `KEEP`; A004 is `SUPERVISOR_PASS / NO_KEEP / ROLLBACK_COMPLETE`; `B1-P1A-OP001` and D001 remain unchecked, D001 streak is `1`, and D002-D017 remain queued. A005 is `PLAN_READY_FOR_MAIN_VERIFY / CODER_LOCKED`; one future unsuccessful A005 can move streak only to `2`, not terminal `3`.
  ...
  - Implementation Gate:
    ...
    - A003 identity/checkpoint and WAL checkpoint `0f3a572331dd23d17688886fcbfebeb7d37ee35d` are complete; A004 is `SUPERVISOR_PASS / NO_KEEP / ROLLBACK_COMPLETE`. `E2-P2A-A005ARCH1/PLAN1/MAINVERIFY1/IMPACT1/SRC1/BUILD1/TEST1/BUILDFAIL1` are recorded and state is `A005_FROZEN_BUILD_FAILED / OVERLAY_BUNDLE_ADAPTATION_RECOVERY_READY / MEASUREMENT_LOCKED / D001_STREAK_1`; only the exact generated-overlay recovery build may proceed.
