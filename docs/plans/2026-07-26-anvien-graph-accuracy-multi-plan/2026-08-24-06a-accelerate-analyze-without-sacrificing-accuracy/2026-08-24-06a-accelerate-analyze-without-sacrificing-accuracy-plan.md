...
| Field | Current value |
|-------|---------------|
| Attempt state | `A003_CHECKPOINT_COMPLETE / WAL_FIX_CHECKPOINT_COMPLETE / A004_ARCHITECT_OWNER_APPROVED / A004_PLAN_COMPLETE / A004_MAIN_VERIFIED / A004_CODER_COMPLETE / A004_BOTH_TARGET_MEASUREMENTS_RECORDED / A004_SUPERVISOR_ACTIVE / D001_STREAK_0`; Supervisor task `01a03f21-63a6-7c53-a793-bb7cd459b858`; A003 checkpoint and WAL checkpoint remain accepted; all checkboxes remain unchanged; A004 values are measured/unpromoted |
| Attempt ID | `A004` |
...
| Current benchmark authority | `benchmark.md` remains the sole numeric authority. The accepted A003 Cheapapp and Restaurant Manager bases and denominators remain target-separated and unchanged; no A004 number exists and no target may be averaged, combined, or subsidized by the other |
| Consecutive unsuccessful attempts at current child baseline | `0`; planning changes neither the accepted baseline nor the streak |
| Selected child exact cause / owner / complete call path | repeated ordering work in `internal/resolution/export_binding_proof.go`: `appendExportBindingEvidence` pre-sorts projected evidence, `mergeExportBindingEvidence` exact-tuple deduplicates/partitions and sorts again, and the stable-sort comparator repeatedly calls `exportBindingEvidenceOrderFor`, which decodes serialized JSON `Evidence.Note`; complete D001 path is `analyze.Run -> runPhase(PhaseResolution) -> ResolveBoundInto -> w.files/ir.Calls -> resolveCall -> appendExportBindingEvidence -> mergeExportBindingEvidence -> sortExportBindingEvidence -> exportBindingEvidenceOrderFor -> json.Unmarshal`, with resolved relationship coalescing and final outcome/reference projection as shared preserve/run-only consumers |
| Fresh Architect decision | `E2-P2A-A004ARCH1`; report `E:\Anvien\reports\system-architect\rp_system-architect_260826_by_gpt-5_child06a_a004_residual_direction.md`; verdict `ARCHITECT_A004_OWNER_APPROVED_READY_FOR_PLANNER`; direction is exact-tuple dedupe -> one canonical key extraction from final serialized `Evidence.Note` per unique projected record -> one final stable sort on cached keys -> discard transient keys -> unchanged `[]graph.Evidence` |
| Planner refresh evidence | `E2-P2A-A004PLAN1`; report `E:\Anvien\reports\planner\rp_planner_260826_by_gpt-5_child06a_a004_export_evidence_ordering.md`; translation state `A004_PLAN_READY_FOR_MAIN_VERIFY / CODER_LOCKED` |
| Transition authority | Owner approval releases only this Planner translation. Main must independently verify the five owned artifacts and separately release one future Coder; planning itself does not authorize source/test edits, measurement, Supervisor, disposition, staging, or commit |
| Future pre-edit Anvien gate | future Coder runs `anvien --help`, one fresh `anvien analyze --force`, file-detail for `internal/resolution/export_binding_proof.go`, and exact upstream impact for every helper it will edit. Current architecture evidence classifies the file HIGH and `appendExportBindingEvidence`, `mergeExportBindingEvidence`, `sortExportBindingEvidence`, and `exportBindingEvidenceOrderFor` CRITICAL; these are blast-radius warnings, not edit bans |
| Allowed production/test surfaces | production only `internal/resolution/export_binding_proof.go`, limited to `appendExportBindingEvidence`, `mergeExportBindingEvidence`, `sortExportBindingEvidence`, `exportBindingEvidenceOrderFor`, existing private key types, and at most one private transient decorated representation/helper; all signatures unchanged. Tests only after production is correct: append A004-specific cases in `internal/resolution/export_binding_proof_test.go` |
| Production algorithm | remove the projection pre-sort; exact-tuple deduplicate and partition first; extract the current canonical comparison tuple once from final serialized `Evidence.Note` for each unique projected record; perform exactly one final stable sort using cached `kindRank/proofOrdinal/hopOrdinal/note`; undecorate and return unchanged evidence. Comparator JSON decode, producer-carried alternate keys, a second final sort, public/persisted order fields, and cross-call/global caches are forbidden |
| Test contract | use a test-local pre-A004 oracle, never a production fallback, and require full ordered `[]graph.Evidence` equality. Cover terminal/hop/failure combinations, mixed generic/projected records, input permutations/repeated coalescing, exact duplicates, same ordinals/different Notes, multiple SourceSiteIDs, malformed JSON per export kind, unknown/non-export kinds, equal-key stability, empty/no-proof paths, caller-input non-mutation, and idempotency |
| Build/test contract | clear every proven build-output holder first; canonical `scripts/full-build.ps1` must PASS before any tests. Then run the differential test, all export-binding proof tests, focused call/access/outcome/Graph JSON regressions, full `internal/resolution`, and any needed A003 graphhealth parity checks. Preserve the known `TestProofBasedCallAccessGoldenCorpus` failure truthfully; never call that package PASS or edit its owner |
| Measurement contract | reuse unchanged `scripts/build-a00x-benchmark.ps1` and the accepted overlay/native/runtime/provenance contract without script audit/edit. Build one identifiable A004 candidate and compare Cheapapp and Restaurant Manager separately against their own accepted A003 bases, recording D001/parent/analyzer/process, `30/30` operations, `17/17` children, denominators, interval conservation/zero overlap, workload, graph/DB, semantics, exact evidence ordering, Graph JSON, stdout/stderr, and resource fields |
| Post-measurement Supervisor | only after both complete packets exist, open one fresh visible A004 Supervisor for the exact candidate and affected ordering/output/lifecycle/resource boundary. Main/Owner alone decides `KEEP`, rollback, or other governed disposition; the A003-only exception creates no A004 tolerance or precedent |
| Resource/invariant boundary | transient decorated state is private `O(P)` per merge, may copy evidence values/string headers but not retained Note bytes, and dies on return. Preserve byte-for-byte `Kind/Weight/Note`, full evidence length/content/order, generic first-seen order, exact-tuple dedupe, terminal/hop/failure/default rank, ordinal/lexical tie-breaks, malformed-note fallback, stable equal keys, SourceSiteIDs, idempotency, non-mutation/aliasing, graph/outcome/reference/diagnostic/output/persistence/determinism/failure/lifecycle behavior |
...
| Next action | monitor sole A004 Supervisor task `01a03f21-63a6-7c53-a793-bb7cd459b858`; after its exact correctness/equivalence verdict, Main alone records measured disposition |
...
- [ ] P2-A: Optimize Every Measured Parent And Child In Largest-First Order.
  ...
  - Work Steps:
    ...
    2. Select the largest unchecked child, bind its exact goal/current child-parent-total basis/denominator/cause/owner/complete call path, then open a new Visible Architect and obtain the exact attempt-scoped decision.
       ...
       - Current A004 cursor: `E2-P2A-A004ARCH1` records the accepted A003 target-separated basis, repeated export-binding evidence order-key JSON decode/double-sort cause, exact one-file owner/shared consumer paths, HIGH/CRITICAL blast radius, and Owner-approved single-key/single-final-sort direction. The finalized report returns `ARCHITECT_A004_OWNER_APPROVED_READY_FOR_PLANNER`; no architecture reinterpretation or A001-A003/WAL rerun is required.
    3. Planner refreshes the Current P2-A Attempt card and these execution steps to the Architect decision before Coder begins.
       ...
       - Mini QA: `E2-P2A-A004PLAN1` translates `E2-P2A-A004ARCH1` without redesign. It binds the one-file production boundary, exact-tuple dedupe before key extraction, one canonical extraction from final serialized Note per unique projected record, exactly one cached-key stable sort, unchanged ordered evidence, production-first/test-second order, fresh Coder Anvien/build/test/two-target/Supervisor sequence, exact rollback, and STOP. It changes no benchmark number, checkbox, streak, or queue and cannot authorize Coder by itself.
       - Evidence target: `E2-P2A-AnnnPLAN1`.
       - Current cursor: A003 remains closed at checkpoint `b6bf45bce95323aa6b53b182edfea8628bd8b463`; WAL checkpoint `0f3a572331dd23d17688886fcbfebeb7d37ee35d` is complete. Main verification passed and sole Coder task `01a03ee4-3c2a-72d0-afe7-08be98fe982e` is active under `A004_PLAN_COMPLETE / A004_MAIN_VERIFIED / A004_CODER_ACTIVE / D001_STREAK_0`; parent/D001 remain unchecked and D002-D017 remain queued.
    4. The sole Coder completes fresh required Anvien graph/file-detail/impact, implements only the refreshed production direction, edits authorized tests only after production is correct, performs the exact build-holder preflight, obtains canonical full-build `PASS`, and only then executes focused and `internal/resolution` package tests.
       ...
       - Mini QA: future Coder runs `anvien --help` -> one fresh `anvien analyze --force` -> file-detail `internal/resolution/export_binding_proof.go` -> exact upstream impact for every helper it will edit, recording the containing-file HIGH and helper CRITICAL warnings without treating them as bans. Production first removes the projection pre-sort, exact-tuple deduplicates/partitions, decorates each unique projected record once through the canonical final-Note extractor, performs one stable cached-key sort, and undecorates unchanged output. Only after production is correct may it append A004 bytes to `internal/resolution/export_binding_proof_test.go`. Any other production file, signature/public shape, alternate key authority, comparator JSON decode, or second final sort forces STOP.
       ...
       - Current A004 cursor: no source/test edit or Coder exists. Main must verify PLAN1 and separately release a future Coder before this step can start; P2-A/parent/children remain unchecked.
    5. After A004 production/build/test validation, record two independent candidate measurements without combining them, using each target's accepted A003 artifact as that target's `before` and one identifiable A004 candidate containing only the authorized PLAN1 production bytes as `after`.
       - UI flow check: N/A unless activated by impact.
       - DB/data flow check: reuse unchanged `scripts/build-a00x-benchmark.ps1` and the accepted 17-child overlay/native/runtime/provenance contract; no script edit or build-interface audit. Cheapapp retains `--force --skip-git --json --progress`; Restaurant retains `--force --json --progress` and exactly one `--exclude electron/renderer/src/api/userApi.ts`. Both retain target-local graphs, exact denominators, interval conservation/zero overlap, evidence ordering, graph/DB/output/semantic equivalence, and resource proof.
       - Render location check: benchmark records separate A004 pending/returned numeric blocks against each target's own accepted A003 baseline; evidence records exact executable/command/target/artifact identity and equivalence; actual status records which reports are pending/received. Planner owns neither future measurement lane.
       - Mini QA: each repo separately records D001, parent, analyzer, process wall, all `30/30` operations and `17/17` children, diagnostics/outcomes, full ordered evidence equality, canonical Graph JSON/public stdout/stderr, `startAllocBytes`, `endAllocBytes`, `maxObservedSys`, and transient `O(P)` lifecycle proof. Never average/combine targets or use build/test/profile duration as elapsed proof.
       - Evidence target: `E2-P2A-AnnnDIRECT1`, `E2-P2A-AnnnPARENT1`, `E2-P2A-AnnnE2E1`, `E2-P2A-AnnnEQUIV1`, `E2-P2A-AnnnREVIEW1`.
       - Current A004 cursor: both future A004 packets are missing and locked behind Main verification plus Coder/build/test completion. Accepted A003 packets are the unchanged separate before bases and must not be rerun or reinterpreted. Only after both A004 packets exist may one fresh visible A004 Supervisor open.
    6. Record disposition, promote only an eligible child+parent+end-to-end `KEEP`, and keep the same child active until terminal.
       ...
       - Current cursor: A001/A002/A003 are `KEEP`; `B1-P1A-OP001` and D001 remain unchecked, D001 streak is `0`, and D002-D017 remain queued. A004 is `MAIN_VERIFIED / CODER_ACTIVE`; no checkbox, baseline, benchmark-number, streak, or parent transition occurs.
  ...
  - Implementation Gate:
    ...
    - A003 identity/checkpoint and WAL checkpoint `0f3a572331dd23d17688886fcbfebeb7d37ee35d` are complete. `E2-P2A-A004ARCH1/PLAN1` are recorded and state is `A004_PLAN_READY_FOR_MAIN_VERIFY / CODER_LOCKED`; only Main independent verification and a separate future Coder release can open implementation.
  ...
  - Acceptance:
    ...
    - Behavior test: production implementation, A004 differential tests only after production is correct, canonical full build, post-build export-binding/call/access/outcome/Graph JSON regressions plus truthful full `internal/resolution` execution and any needed A003 graphhealth parity checks, later same-work two-target remeasurement, per-attempt review, and final equivalence pass in that order.
