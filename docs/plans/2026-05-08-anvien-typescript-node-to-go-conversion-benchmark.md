# anvien-GO TypeScript/Node To Go Conversion Benchmarks
...
currently used Anvien-main where legacy weaknesses are identified. If a benchmark exposes a
...
- `powershell -ExecutionPolicy Bypass -File .tmp\pre_phase5_benchmark.ps1` passed on
  ...
  `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` as the Phase 4A
  benchmark step.
- Full launcher build time: 34,302.598 ms, exit code `0`.
- Package contents: `AnvienLauncher.exe` exists, 6,974,464 bytes;
  `server-bundle\anvien-server.exe` exists, 2,056,192 bytes;
  `server-bundle\anvien.exe` exists, 9,242,624 bytes; `server-bundle\node.exe`
  absent.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before native and
...
- Added `TestAnalyzeCommandGeneratesAIContextAndSkills` and extended
  `TestAnalyzeCommandRunsGoPipelineAndWritesBenchmark` to verify `.anvien/meta.json` output.
...
- Ran Phase 11 Go analyze benchmark snapshots into `.tmp/`:
  ...
  `phase11-benchmark-anvien.json` (`1047` scanned, `747` parsed, `21567` graph nodes,
...
- Re-ran tagged native LadybugDB `v0.16.1` fixture smoke with:
  `go run -tags ladybugdb ./cmd/anvien analyze .tmp\resolution-baseline-fixture --force --benchmark-json .tmp\phase11-benchmark-native-dbload-smoke.json`.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before the Phase
  13 benchmark gate.
...
- Group fixture benchmark with `ANVIEN_HOME=.tmp/mcp-group-sync-smoke/home` covered
...
- Benchmark/smoke, 1 run on this machine:
  - Full launcher package build:
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` -> `47.70s`;
    build log: `.tmp\phase16-launcher-build.log`.
  - Packaged launcher start readiness:
    `AnvienLauncher.exe` -> backend `127.0.0.1:4747` and Web UI `127.0.0.1:5173` ready in
    `1.64s`.
  - Protocol registration command:
    `AnvienLauncher.exe register` -> `0.16s`.
  - Stop command plus readiness-down wait:
    `AnvienLauncher.exe stop` -> `11.92s`.
  - Reset command on stopped runtime:
    `AnvienLauncher.exe reset` -> `0.63s`.
  - Package sizes:
    `AnvienLauncher.exe=6,974,464 bytes`,
    `server-bundle/anvien-server.exe=2,056,192 bytes`,
    `server-bundle/anvien.exe=48,830,976 bytes`.
...
- Full launcher build gate before this batch: `powershell -ExecutionPolicy Bypass -File
  anvien-launcher\build.ps1` passed in approximately `41s`.
- Docker CLI image first full build: `docker build -f Dockerfile.cli -t anvien-cli-cutover .`
...
- TypeScript baseline artifact: `.tmp\phase17-cutover-ts-anvien.json`.
  - Files: `1205`.
  - Nodes: `28731`.
  - CLI-reported edges: `51603`.
  - Graph-snapshot relationships: `52686`.
  - Total wall time: `150292.7ms`.
- Go packaged artifact: `.tmp\phase17-cutover-go-anvien.json`.
  - Files scanned: `1205`.
  - Files parsed: `1036`.
  - Unsupported: `169`.
  - Nodes: `31829`.
...
- Latest-release bootstrap:
  ...
  - `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` refreshed the cache to
...
- Packaged Go native artifact: `.tmp\phase17-cutover-go-native-anvien.json`.
  - Files scanned: `1208`.
  - Files parsed: `1036`.
  - Unsupported: `172`.
  - Nodes: `31838`.
...
- High-signal IMPORTS classification:
  - Missing TS-only imports include Go files linked to TypeScript/shared helper files such as
    `anvien-shared/src/scope-resolution/registries/context.ts`,
    `anvien/src/core/group/sync.ts`, and `internal/lbugruntime/errors.go` through path/name
    collisions.
...
- Build/test ordering for this slice:
  ...
  Playwright E2E `33/33` against `anvien/bin/anvien.exe serve`, npm-link PATH verification, and
  `npm publish --dry-run`.
...
| Before cutover | Packaged Go binary `anvien-launcher/server-bundle/anvien.exe --help` | `27.4`, `25.0`, `25.7`, `25.0`, `26.5`, `25.7`, `25.7` | `25.9` | `25.7` | `0` |
| After local cutover | PATH npm shim -> `node_modules/anvien/bin/anvien.exe` | `139.0`, `42.4`, `36.5`, `37.4`, `50.6`, `49.9`, `46.5` | `57.5` | `46.5` | `0` |
| After local cutover | Direct package binary `anvien/bin/anvien.exe --help` | `32.5`, `33.2`, `26.9`, `35.4`, `29.6`, `28.6`, `29.3` | `30.8` | `29.6` | `0` |

- Interpretation:
  PATH `anvien --help` median improved from `133.7ms` to `46.5ms`, approximately `2.9x` faster,
...
- Caveat:
  the Windows `npm publish --dry-run` tarball included `bin/anvien.exe`, `bin/lbug_shared.dll`,
  and `bin/anvien-runtime.json`, proving the local Windows package build shape. Portable npm
...
| Installed `anvien --help` elapsed | `1,481.1ms` |
| Installed runtime platform/arch/source | `win32` / `x64` / `go-src` |
| Installed help first line | `Anvien local CLI and MCP server` |
...
- Interpretation:
  ...
  native bootstrap, and Go runtime build from `node_modules/anvien/go-src`; it is intentionally
  not compared to CLI startup or graph-analyze speed.
- Follow-up gate:
  portable package selection is closed, but Phase 17 CLI cutover remains open because `anvien
...
| --- | --- | ---: | ---: | ---: |
| Root help | `anvien\bin\anvien.exe --help` | `1,939.3` | `1,305` | `0` |
| Query tool | `query "CLI command surface" --repo F:\Anvien --limit 1` | `4,237.1` | `2,254` | `0` |
| Context tool | `context NewRootCommand --repo F:\Anvien --file internal/cli/command.go` | `950.9` | `12,206` | `0` |
| Impact tool | `impact NewRootCommand --repo F:\Anvien --direction upstream --depth 1` | `884.2` | `3,416` | `0` |
| Cypher tool | `cypher "MATCH (n:Function) RETURN n.id AS id, n.name AS name, n.filePath AS path LIMIT 1" --repo F:\Anvien` | `420.5` | `553` | `0` |
| Detect changes | `detect-changes --scope unstaged --repo F:\Anvien` | `961.5` | `11,080` | `0` |
| Augment | `augment NewRootCommand` | `4,399.9` | `5,151` | `0` |

- E2E validation:
  Go backend `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747` plus Vite on
...
- Purpose: verify the Phase 17 local admin/analyze command-surface batch after the direct graph-tool
  commands. This benchmark uses temp repos and a temp `ANVIEN_HOME` so the normal repo registry and
  `.anvien` data are not mutated.
- Build/test ordering for this slice:
...
| Check | Command | Elapsed ms | Output chars | Exit |
| --- | --- | ---: | ---: | ---: |
| Root help | `anvien\bin\anvien.exe --help` | `58.4` | `1,565` | `0` |
...
| Index existing | `index <temp git repo with .anvien/lbug>` | `164.0` | `108` | `0` |
...
- Purpose: verify the Phase 17 group command-surface batch after admin/analyze command parity. This
  benchmark uses temp repos and a temp `ANVIEN_HOME`.
- Build/test ordering for this slice:
...
- Interpretation:
  ...
  the TypeScript `anvien/src/cli/group.ts` command file. The benchmark validates command
...
- Purpose: verify the final Phase 17 command-surface batch after group command parity. This
  benchmark uses a temp `HOME`/`USERPROFILE`/`ANVIEN_HOME` and empty `PATH` for setup execution so
...
- Setup artifact checks:
  ...
  `anvien-cli` skills, `.claude/hooks/anvien/anvien-hook.cjs`, and `.claude/settings.json`.
  Codex fallback contained exactly one `[mcp_servers.anvien]` section after two setup runs.
- E2E validation:
  Go backend `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747` plus Vite on
  `127.0.0.1:5173`; `cd anvien-web && E2E=1 npx playwright test --workers=1` exited `0` in
  ...
  `npx playwright test --list` listed `33` tests in `6` files.
- Interpretation:
  `anvien setup` now executes through the Go CLI and writes local MCP config/skills/hooks without
...
| Check | Command | Elapsed ms | Stdout chars | Stderr chars | Exit |
| --- | --- | ---: | ---: | ---: | ---: |
| PATH shim help | `anvien --help` | `75.7` | `1,726` | `0` | `0` |
| Direct package binary help | `anvien\bin\anvien.exe --help` | `36.6` | `1,726` | `0` | `0` |
| npm serve help | `cd anvien && npm run --silent serve -- --help` | `369.2` | `359` | `0` | `0` |
| npm dev help | `cd anvien && npm run --silent dev -- --help` | `411.9` | `359` | `0` | `0` |

- Audit checks:
  the PowerShell PATH shim contains `node_modules/anvien/bin/anvien.exe`. The old blocker
  patterns for `tsx src/cli/index.ts serve`, `"serve": "tsx`, `"dev": "tsx watch
  src/cli/index.ts`, and the Web onboarding command `cd anvien && npm run serve` no longer appear
  in runtime files; the only remaining literal `cd anvien && npm run serve` is a negative unit
  test assertion proving the old command is absent from onboarding.
- Test validation:
  `cd anvien && npm run build` passed in `16,488.3ms`;
...
- Purpose: verify the Phase 17 contract-authority cutover after the normal runtime/CLI cutover. This
  ...
  removes `anvien-shared` from the Web package dependency/import/alias path.
- Build/test ordering for this slice:
  ...
  build after install, Playwright E2E, and `cd anvien && npm test`.

| Check | Command | Elapsed ms | Result |
| --- | --- | ---: | --- |
| Full launcher build | `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` | `38,426.7` | passed |
| Go runtime/contract tests | `go test ./cmd/... ./internal/... -count=1` | `29,795.3` | passed |
| Web contract unit tests | `cd anvien-web && npm test -- session-client.test.ts security-guards.test.ts useAppState.local-runtime.test.tsx GraphCanvas.selection-performance.test.tsx ChatPanel.grounding-links.test.tsx SettingsPanel.local-runtime.test.tsx` | `13,013.4` | `6` files / `130` tests passed |
| Contract generator check | `go run ./cmd/generate-web-contracts --check` | `559.9` | passed |
| Web contract dependency audit | `rg -n "anvien-shared" anvien-web` | `55.8` | `0` hits |
| Fresh Web install | `cd anvien-web && npm ci` | `50,004.7` | passed |
| Post-install full launcher build | `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` | `33,223.8` | passed |
| Browser E2E | `cd anvien-web && E2E=1 npx playwright test --workers=1` with packaged Go backend | `205,536.5` | `33/33` passed |
| Legacy baseline/test suite | `cd anvien && npm test` | `338,908.6` | passed |

- E2E validation:
  the backend was the packaged Go runtime from `anvien-launcher/server-bundle/anvien.exe serve
  --host 127.0.0.1 --port 4747`; Vite ran on `127.0.0.1:5173`. The final accepted Playwright run
  exited `0` with `33` tests passed.
- Audit result:
  `anvien-web` has no remaining `anvien-shared` package dependency, Vite/Vitest alias, source
  import, test import, lockfile entry, or Vercel install requirement. The Web contract TypeScript is
  generated from Go under `anvien-web/src/generated/anvien-contracts.ts`, with the JSON manifest
  stored at `contracts/web-ui/anvien-web-contract.schema.json`.
- Notes:
...
- Interpretation:
  ...
  generated browser glue instead of the independent `anvien-shared` TypeScript package. Legacy
  `anvien-shared` and `anvien/src` TypeScript remain baseline/dev/test material, not normal
...
- Build/test ordering for this slice:
  ...
  Go tests, packaged analyze graph-parity smoke, browser E2E, and `cd anvien && npm test`.
...
- Graph parity/analyze smoke:
  `anvien-launcher\server-bundle\anvien.exe analyze --force [redacted removed argument] --benchmark-json
...
- Test validation:
  ...
  isolated `ANVIEN_HOME` containing only `Anvien`, the packaged Go backend on
  `127.0.0.1:4747`, and Vite on `127.0.0.1:5173`; `cd anvien-web && E2E=1 npx playwright test
  --workers=1 --reporter=line` passed `33/33` in `227,307.3ms`. `cd anvien && npm test` passed
  in `373,303.6ms`.
- Notes:
  ...
  and failed. The accepted E2E rerun used isolated `ANVIEN_HOME=.tmp\phase15-route-cache-e2e-home`
  and registered only `Anvien`, matching the plan benchmark target.
...
| --- | --- | --- |
| Full launcher build | `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` | `39,240.9ms`, passed |
...
| Isolated frontend/mobile fixture analyze | `anvien-launcher\server-bundle\anvien.exe analyze .tmp\phase14-frontend-mobile-fixture --skip-git --force [redacted removed argument] --no-stats --name phase14-frontend-mobile --benchmark-json .tmp\phase14-frontend-mobile-analyze.json` | wall `7,646.2ms`, benchmark total `7,549.2ms` |
| Current-repo packaged analyze | `anvien-launcher\server-bundle\anvien.exe analyze --force [redacted removed argument] --no-stats --benchmark-json .tmp\phase14-frontend-mobile-current-repo.json` | wall `61,853.6ms`, benchmark total `61,521.2ms` |
| Full Go tests | `go test ./cmd/... ./internal/... -count=1` | passed |
| Browser E2E | `cd anvien-web && E2E=1 npx playwright test --workers=1 --reporter=line` with isolated packaged Go backend | `494,207.2ms`, `32` passed / `1` skipped |
| Legacy baseline/test suite | `cd anvien && npm test` | `335,723.4ms`, passed |
...
- E2E notes:
  ...
  isolated backend whose `/api/repos` returned only `Anvien`. Two test selectors were hardened
...
- Build/test ordering for this slice:
  ...
  benchmarks, packaged current-repo analyze, full Go tests, browser E2E, and `cd anvien &&
  npm test`.

| Check | Command | Elapsed / result |
| --- | --- | --- |
| Full launcher build | `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` | `34,207.9ms`, passed |
...
| Current-repo packaged analyze | `anvien-launcher\server-bundle\anvien.exe analyze --force [redacted removed argument] --no-stats --benchmark-json .tmp\fallback-failclosed-graph-parity.json --benchmark-label fallback-failclosed-graph-parity` | wall `59,937.2ms`, benchmark total `58,458.4ms` |
| Full Go tests | `go test ./cmd/... ./internal/... -count=1` | `27,772.9ms`, passed |
| Browser E2E | `cd anvien-web && E2E=1 npx playwright test --workers=1 --reporter=line` with isolated packaged Go backend | Playwright `512,685.5ms`, `32` passed / `1` skipped |
| Legacy baseline/test suite | `cd anvien && npm test` | `438,204.3ms`, passed |
...
- E2E notes:
  ...
  `http-get://127.0.0.1:4747/api/info`, an isolated `ANVIEN_HOME`, and the packaged Go backend.
...
- Build/test ordering for this slice:
  ...
  tests, browser E2E, and `cd anvien && npm test`.
...
- Test validation:
  ...
  `27,772.9ms`. Full browser E2E used an isolated `ANVIEN_HOME`, the packaged Go backend on
  `127.0.0.1:4747`, and Vite on `127.0.0.1:5173`; `cd anvien-web && E2E=1 npx playwright test
  --workers=1 --reporter=line` passed with `32` passed / `1` skipped in `512,685.5ms`.
  `cd anvien && npm test` passed in `438,204.3ms`.
...
- Build/test ordering for this slice:
  ...
  graph-parity smoke, full Go tests, browser E2E, and `cd anvien && npm test`.
...
- Test validation:
  ...
  `27,295.0ms`. Full browser E2E used an isolated `ANVIEN_HOME`, the packaged Go backend on
  `127.0.0.1:4747`, and Vite on `127.0.0.1:5173`; Playwright passed with `32` passed /
  `1` skipped in `536,712.6ms`. `cd anvien && npm test` passed in `624.8s`.
...
- Test validation:
  ...
  isolated `ANVIEN_HOME`, the packaged Go backend on `127.0.0.1:4747`, and Vite on
  `127.0.0.1:5173`; Playwright passed with `32` passed / `1` skipped in `503,825.4ms`.
  `cd anvien && npm test` passed in `374,034.3ms`.
- Recovery note:
  one earlier graph parity rerun failed because an interrupted analyze left a stale
  `.anvien/analyze.lock` and partial `lbug.shadow`/`lbug.wal.checkpoint`. The lock PID was not
...
- Test validation:
  ...
  `28,306.0ms`. Full browser E2E used isolated `ANVIEN_HOME=.tmp\phase15-p3-e2e-home-*`, the
  ...
  `cd anvien && npm test` passed in `385,336.0ms`.
...
- Build/test ordering for this slice:
  ...
  browser E2E, and `cd anvien && npm test`.
...
- Test validation:
  ...
  `ANVIEN_HOME=.tmp\phase15-graph-stream-e2e-home-*`, the packaged Go backend on
  `127.0.0.1:4747`, and Vite on `127.0.0.1:5173`; isolated analyze took `64,342.3ms`, and
  Playwright passed with `32` passed / `1` skipped in `408,728.9ms`. `cd anvien && npm test`
...
| Candidate build | `anvien-launcher\build.ps1` after changing COPY options to `PARALLEL=true` | passed in `36,663.0ms` |
...
| Candidate analyze | packaged `anvien.exe analyze --force [redacted removed argument] --no-stats --benchmark-json .tmp\phase15-dbload-parallel-true-analyze.json --benchmark-label phase15-dbload-parallel-true-analyze` | failed closed in `33,143.2ms`; no benchmark JSON was written because native DB load failed |
| Rollback build | `anvien-launcher\build.ps1` after restoring `PARALLEL=false` | passed in `33,780.9ms` |
...
- Rejection reason:
  ...
  `PARALLEL=FALSE`. Since Anvien stores code/content text in CSV fields, enabling `PARALLEL=true`
...
- Validation:
  ...
  `ANVIEN_HOME=.tmp\phase15-dbload-docs-e2e-home-*`, the packaged Go backend on
  ...
  `180,285.1ms`. `cd anvien && npm test` passed in `409,607.3ms`.
...
- Impact:
  Anvien impact reported `ScopeIR.Normalized` LOW risk, `callKey` LOW risk, and `rangeKey` MEDIUM
...
- Validation:
  ...
  used isolated `ANVIEN_HOME=.tmp\phase15-scopeir-e2e-home-*`, packaged Go backend on
  ...
  with `32` passed / `1` skipped in `405,663.7ms`. `cd anvien && npm test` passed in
  `342,850.8ms`.
...
- Phase-stay note:
  ...
  runtime. It is not provider coverage, fallback correctness, or cutover authority work.
- Impact:
  Anvien impact reported `definitionWindow` LOW risk with one direct caller (`AnnotateScopeIR`).
  Anvien impact on the caller reported CRITICAL because `AnnotateScopeIR` feeds `parseFiles` and
...
- Validation:
  ...
  isolated `ANVIEN_HOME=.tmp\phase15-framework-window-e2e-home-*`, packaged Go backend on
  ...
  `1,734.1ms` and the serial full E2E run passed. `cd anvien && npm test` passed in
  `537,613.2ms`.
...
- Phase-stay note:
  ...
  runtime. It is not provider coverage, fallback correctness, or cutover authority work.
- Impact:
  Anvien impact reported `buildWorkspace` CRITICAL because it feeds `BuildCrossFileBinding` and
...
- Validation:
  ...
  `ANVIEN_HOME=.tmp\phase15-resolution-workspace-e2e-home-*`, packaged Go backend on
  ...
  `1` skipped in `411,382.6ms` using `--workers=1`. `cd anvien && npm test` passed in
  `579,022.0ms`.
...
- Validation:
  ...
  used isolated `ANVIEN_HOME=.tmp\phase15-dbload-schema-lookup-e2e-home-final-*`, packaged Go
  ...
  `32` passed / `1` skipped in `412,198.7ms` using `--workers=1`. `cd anvien && npm test` passed
  in `424,654.6ms`.
...
- Impact:
  Anvien impact reported `loadGraph` CRITICAL because the DB load path feeds `analyze`,
  `newAnalyzeCommand`, `main`, and HTTP embed/analyze paths. Anvien context for `LoadCSVExport`
...
- Validation:
  ...
  used isolated `ANVIEN_HOME=.tmp\phase15-dbload-tx-e2e-home-*`, packaged Go backend on
  ...
  `1` skipped in `447,486.9ms` using `--workers=1`. `cd anvien && npm test` had one initial
...
- Impact:
  Anvien impact reported `resolveImportedMember` CRITICAL because it feeds
...
- Validation:
  ...
  `ANVIEN_HOME=.tmp\phase15-import-index-e2e-retry-home-*`, packaged Go backend on
  ...
  `cd anvien && npm test` passed in `378,034.1ms`.
...
- Impact:
  Anvien impact reported `countNodes` LOW risk with direct caller `Pool.Parse`. `PoolOptions` was
...
- Validation:
  ...
  isolated `ANVIEN_HOME=.tmp\phase15-nodecount-e2e-home-20260514-080427`, packaged Go backend on
  ...
  `206,434.9ms` using `--workers=1 --retries=1`. `cd anvien && npm test` passed in
  `400,152.3ms`. One earlier E2E analyze attempt is rejected from evidence because the PowerShell
  variable `$HOME` was accidentally used instead of an Anvien-specific variable, so it did not use
...
- Validation:
  ...
  `ANVIEN_HOME=.tmp\phase15-tsjs-traversal-e2e-home-20260514-082700`, packaged Go backend on
  ...
  `194,306.0ms` using `--workers=1 --retries=1`. `cd anvien && npm test` passed in
  `343,081.8ms`.
...
- Phase-stay note:
  ...
  slice.
- Impact:
  Anvien impact reported `Extract` LOW with one direct caller (`extractScopeIR`) and two affected
...
- Validation:
  ...
  `ANVIEN_HOME=F:\Anvien\.tmp\phase15-go-provider-e2e-home-20260514-084738`, packaged Go
  ...
  `32` passed / `1` skipped in `436,101.7ms` using `--workers=1 --retries=1`. `cd anvien &&
...
- Date: `2026-05-14`.
- Purpose:
  reduce the heap peak from writing `.anvien\graph.json` after the post-Go-provider heap profile
...
- Impact:
  Anvien impact reported `writeGraphSnapshot` CRITICAL because it is called from analyze `Run`
...
- Validation:
  ...
  `ANVIEN_HOME=F:\Anvien\.tmp\phase15-graph-snapshot-e2e-home-20260514-092308`, packaged Go
  ...
  `--workers=1 --retries=1`. `cd anvien && npm test` passed with exit code `0` in
  `414,123.7ms`.
...
- Impact:
  Anvien impact reported `ScopeIR.Normalized` LOW (`MarshalDeterministic`, `Unmarshal`, and the
...
- Validation:
  ...
  `35,542.3ms`; browser E2E through the packaged Go backend and isolated `ANVIEN_HOME` passed
  ...
  `cd anvien && npm test` passed in `461,316.4ms`; Anvien `detect_changes(scope=all)` reported
...
- Phase-stay note:
  ...
  authority gate.
- Impact:
  Anvien impact reported `analyze.Run` CRITICAL and `newAnalyzeCommand` HIGH because they are
...
- Validation:
  ...
  `ANVIEN_HOME=F:\Anvien\.tmp\phase15-release-scopeir-e2e-home-20260514`, packaged Go
  ...
  specs, and `cd anvien && npm test` passed in `571.4s`.
...
- Phase-stay note:
  ...
  authority.
- Impact:
  Anvien impact reported `Graph` CRITICAL (`357` impacted symbols, `376` processes),
...
- Validation:
  ...
  `ANVIEN_HOME=F:\Anvien\.tmp\phase15-graph-compact-e2e-home-20260514`, packaged Go backend
  ...
  port check found no listeners on `4747` or `5173`. `cd anvien && npm test` passed in `477.1s`.
...
- Interpretation:
  ...
  `Anvien` is proven as an independent Go MCP/tool implementation separate from
  `Anvien-main`. The attempted patch was not committed; Phase 15 micro-optimization is deferred.
- Follow-up:
...
- Impact:
  Anvien impact reported `RelationPairs` LOW. `relationPairSupported` was CRITICAL because it
...
- Purpose:
  ...
  leave `AnvienLauncher.exe`, `anvien-server.exe`, or packaged `anvien.exe serve` running and
  holding locks under `anvien-launcher\server-bundle`.
- Phase-jump note:
...
- Impact:
  ...
  `openBrowser`. The change is scoped to the Go launcher, not the direct `anvien serve` path.

| Benchmark / Validation | Result |
| --- | ---: |
| Full launcher build after lifecycle patch | `47,045.1ms` |
| `anvien-launcher/src` tests after build | `5,110.9ms` |
| `anvien-launcher/server-wrapper` tests after build | `2,052.4ms` |
| HTTP close-flow smoke | `15,647.2ms` |
| Playwright close-flow E2E | `27,569ms` |
| Post-close launcher bundle processes | `0` |
| Post-close exclusive lock check | `anvien-server.exe` and `anvien.exe` unlocked |
| `cd anvien && npm test` | `415,943.7ms` |

- Implementation:
  the launcher-served `index.html` is injected with a small launcher-only lifecycle script that
  sends heartbeat requests to `127.0.0.1:5173/__anvien_launcher/heartbeat` and sends a close
  signal to `__anvien_launcher/closed` on `pagehide`. `waitForExit` now listens for this UI-done
  ...
  `ANVIEN_LAUNCHER_NO_BROWSER=1` path suppresses real browser launch for automated smoke.
- Smoke evidence:
  the HTTP smoke started `AnvienLauncher.exe` hidden with `ANVIEN_LAUNCHER_NO_BROWSER=1`,
  ...
  `anvien-server.exe` and `anvien.exe` with exclusive file locks after close, proving the
...
- E2E evidence:
  ...
  locks remained. `cd anvien && npm test` passed with exit code `0` in `415,943.7ms`.
...
| `anvien/src` legacy implementation files | `339` |
| `anvien/test` legacy test/fixture harness files | `542` |
| `anvien/scripts` package/build scripts | `8` |
| `anvien/hooks` | `1` |
| `anvien/vitest.config.ts` | `1` |

- Interpretation:
  this is a conversion-completeness benchmark, not a performance target. `anvien-web` remains
  allowed Web UI TypeScript/React. The blocker is the non-Web inventory under `anvien/src`,
  `anvien-shared`, root/package scripts, hooks, and TypeScript test harnesses. These counts become
...
- Anvien-main refresh artifact:
  `.tmp\phase17-tsjs-classification-anvien-main-refresh.json`.

| Metric | Result |
| --- | ---: |
| Anvien-main analyze wall | `137,338.4ms` |
| Anvien-main nodes / stats edges | `29,989` / `54,524` |
| Anvien-main graph relationships | `55,624` |
| Anvien-main processes | `665` |
...
| `anvien/src` legacy implementation | `339` |
| `anvien-shared` legacy contracts | `34` |
| `anvien/test/fixtures` analyzer fixtures | `290` |
| `anvien/test` Node/Vitest harness outside fixtures | `252` |
| `anvien-web` allowed Web surface | `119` |
| `anvien/scripts` / `hooks` / `vendor` | `8` / `1` / `4` |
...
- Build prerequisite:
  ...
  command handling bug, which was fixed in `anvien-launcher/build.ps1`; the accepted rerun used
  `C:\msys64\ucrt64\bin` on PATH and passed.

| Metric | Result |
| --- | ---: |
| Pre-impact Anvien refresh wall | `29.9s` |
| Full launcher build wall | `90.3s` |
| Web build inside launcher build | `19.20s` |
| Packaged backend binary | `anvien-launcher/server-bundle/anvien.exe`, `49,444,864` bytes |
...
| Final Anvien refresh wall | `28.6s` |
...
- Purpose:
  ...
  optimization.
- Cluster:
  `anvien/hooks/claude/anvien-hook.cjs`, `anvien/hooks/claude/pre-tool-use.sh`,
  `anvien/hooks/claude/session-start.sh`, Go hidden hook command, setup hook installation, package
  metadata, and hook/setup tests.

| Metric | Result |
| --- | ---: |
| Anvien pre-impact refresh wall | `133.6s` |
| Full package build | `cd anvien && npm run build`, `18,285.7ms` |
| Go test batch | `go test ./cmd/... ./internal/... -count=1`, `24,546.2ms` |
| TypeScript check | `cd anvien && npx tsc --noEmit`, `8,745.7ms` |
| Full Anvien npm test suite | `cd anvien && npm test`, `375,380.3ms` |
| Go hook PreToolUse smoke | `anvien.exe hook claude`, `942.9ms` |
| Go hook PostToolUse smoke | `anvien.exe hook claude`, `81.2ms` |
| Go setup temp-HOME smoke | `anvien.exe setup`, `38.7ms` |
| Final Anvien refresh wall | `129,075.0ms` |
| Final Anvien graph | `30,013` nodes / `54,569` edges / `669` flows |
