# Anvien Graph Health Unknown Connectivity Separation Evidence Ledger
...
Benchmark: [2026-05-21-anvien-graph-health-unknown-connectivity-separation-benchmark.md](2026-05-21-anvien-graph-health-unknown-connectivity-separation-benchmark.md)
...
- Repository: `E:\Anvien`
- Graph refresh before inspection: `anvien analyze --force`
...
- `.anvien\graph.json` itself does not contain computed `graphHealth` fields until the API/Graph Health compute path runs.
...
Result: failed only because intentionally invalid fixture packages under `anvien/test/fixtures/...` are included by `./...`. Product/runtime packages in `cmd` and `internal` passed.
...
Result: completed. Anvien reported `risk_level: critical`, `changed_files: 17`, `changed_count: 185`, and `affected_count: 35`.
