# Anvien Multi-Language Graph Filters and Coverage Plan
...
- Evidence ledger: [2026-05-19-anvien-multilanguage-graph-filters-and-coverage-evidence.md](2026-05-19-anvien-multilanguage-graph-filters-and-coverage-evidence.md)
...
- The `Restaurant_manager` TypeScript heritage trace is optional unless `ANVIEN_RESTAURANT_MANAGER_ROOT` is set.
...
- While fixing the Web UI display for these heritage facts, the tool shell must also keep the graph workflow usable: the top bar needs a Back arrow/button beside `Anvien` to return to `Start-Anvien.html`, and the left dashboard needs drag resizing so dense node/edge/legend controls can be inspected comfortably.

These trigger findings are not the full boundary of the work. Anvien is a multi-language graph tool, so every Web UI filter and graph fact category must be checked against every code language declared by the scanner and generated Web contracts, not only TypeScript and Go and not only heritage relationships.
...
- `anvien-web/src/lib/graph-links-visibility.ts`
- `anvien-web/src/lib/graph-edge-visibility-mode.ts`
...
- launcher static asset packaging/serving paths that affect `Start-Anvien.html`
...
- The Web UI must provide a clear Back arrow/button beside the `Anvien` title that returns to `Start-Anvien.html`, with accessible naming and keyboard/click support.
...
- [x] [P0-H] Run Anvien analyze and impact/context checks for the implementation surface. Result: `FileTreePanel` impacts `App.tsx` and `FileTreePanel.dashboard-completeness.test.tsx`; `knowledgeGraphToGraphology` impacts `GraphCanvas.tsx` and `graph-adapter.edge-geometry.test.ts`; `getGraphEdgeVisibilityMode` impacts `useSigma.ts` and `graph-edge-visibility-mode.test.ts`; `WebUIContract` feeds generated Web graph/language constants; `parseFiles` routes extractor support and has tests for most provider languages.
...
- Code paths already identified by Anvien must drive implementation order: `internal/contracts/web_ui.go`, `internal/analyze/analyze.go`, `internal/providers/*`, `internal/resolution/*`, `anvien-web/src/lib/constants.ts`, `anvien-web/src/components/FileTreePanel.tsx`, `anvien-web/src/lib/graph-adapter.ts`, `anvien-web/src/lib/graph-edge-visibility-mode.ts`, `anvien-web/src/hooks/useSigma.ts`, and their tests.
...
- [x] [P2-K] Compare actual graph node labels and relationship types from `E:\Restaurant_manager` and `E:\Anvien` against Web filter rows, legend rows, color/size mappings, and adapter support. Result: benchmark ledger records final graph-present label/type inventories for both repos and Web unit tests assert graph-present dashboard completeness.
...
- [x] [P4-L] Inspect `anvien-web/src/components/Header.tsx`, `anvien-web/src/App.tsx`, launcher path handling, and `Start-Anvien.html` to confirm the correct Start-screen return target in both dev and packaged launcher modes. Result: Vite dev/build now serves/emits `Start-Anvien.html`; Header computes an absolute same-origin start-screen URL.
- [x] [P4-M] Add an icon-first Back arrow/button beside the `Anvien` top bar title with an accessible label and keyboard/click activation. Result: Header renders an `ArrowLeft` icon button labelled `Back to Start screen`.
- [x] [P4-N] Implement the return flow to `Start-Anvien.html` without showing a stale connection-loss banner during the intentional navigation transition. Result: App tracks intentional start navigation and suppresses the reconnect banner during that transition.
...
- [x] [P5-A] Analyze `E:\Anvien` and record full graph filter inventory: node label counts, relationship type counts, graph-present filter rows, adapter classification coverage, semantic unique heritage counts, and duplicate compatibility pair counts.
...
- [x] [P6-B] Run focused Go tests for extraction/resolution and graph contract behavior across every covered language/provider. Result: focused provider/resolution/contracts tests passed, including optional Restaurant_manager TS heritage trace when `ANVIEN_RESTAURANT_MANAGER_ROOT` is set.
...
- [x] [P6-D] Run full applicable Go test suite for `cmd` and `internal`. Result: `go test ./cmd/... ./internal/...` passed. Repository-wide `go test ./...` still includes intentionally non-buildable fixture folders under `anvien/test/fixtures`.
...
- [x] [P6-J] Re-run analyze on `E:\Anvien` and `E:\Restaurant_manager` and verify final graph/filter metrics match the accepted contract. Result: both repos were analyzed through `go run ./cmd/anvien analyze ... --force [redacted removed argument] --no-stats`; final counts are in the benchmark ledger.
