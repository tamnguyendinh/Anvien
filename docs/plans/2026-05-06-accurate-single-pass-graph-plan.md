...
- [x] Add per-language benchmark coverage reporting for parseable files, AST-reused scope files, compatibility/no-hook/failed extraction counts, reference-site counts, resolved/unresolved reference counts, and coverage percentages.
- [x] Make benchmark target repo git state explicit by writing `repoGitUnavailable` plus a reason when `repoGitCommit` and `repoGitDirty` cannot be read.
- [x] Expose per-language coverage deltas in `benchmark-compare` and cover them in benchmark snapshot/compare tests.
- [x] Regenerate final repeated benchmark artifacts with per-language coverage and explicit target repo git-state metadata.
...
- `reports/benchmark/2026-05-07-avmatrix-final-equivalent-accuracy-run1-gitnexus-main.json`, `run2`, and `run3` record the regenerated final repeated AVmatrix-side equivalent-accuracy benchmark point after the supervisor rereview fix. The three default runs have pairwise `graphDiffs=0`, identical `nodeDigest`, identical `relationshipDigest`, and stable counts: `19534` nodes, `30809` graph relationships, `CALLS=5127`, `IMPORTS=202`, `ACCESSES=183`, `USES=832`, `INHERITS=1`, semantic duplicate `CALLS=0`, `scopeResolutionReferenceSites=128633`, `scopeResolutionResolvedReferences=15083`, and `scopeResolutionEdgesEmitted=1470`. Wall times are `91764.5ms`, `90218.4ms`, and `91031.5ms`; median wall time is `91031.5ms`. Resolution timings are `2537ms`, `2036ms`, and `2319ms`; median resolution is `2319ms`. Each artifact now records explicit target git metadata for the non-git local copy: `repoGitUnavailable=true` and `repoGitUnavailableReason="git rev-parse HEAD unavailable for target repo; git status --porcelain unavailable for target repo"`. Each artifact also records `languageCoverageByLanguage`: JavaScript `8/8`, Python `20/20`, and TypeScript `722/722` parseable files have AST-reused scope extraction, with `0` compatibility/no-hook/failed files and `100%` AST-reused coverage for all three languages. This is the AVmatrix comparison point for the user's external GitNexus deep/scope baseline; do not reinterpret it as a local GitNexus measurement.
...
Latest validation after the supervisor rereview benchmark-protocol fix:

- Full launcher build passed with `powershell -ExecutionPolicy Bypass -File avmatrix-launcher\build.ps1` before any post-fix tests or benchmark runs.
- Targeted supervisor rereview suite passed cleanly: `typescript-single-pass-parity`, `python-single-pass-parity`, `scope-reference-resolver`, `emit-references`, `finalize-orchestrator`, `cross-file`, `analyze-benchmark-snapshot`, and `benchmark-compare-command`, `8` files / `70` tests.
- Final repeated AVmatrix benchmark artifacts were regenerated from the built CLI with per-language coverage and explicit target git unavailable metadata; `benchmark-compare` reports `graphDiffs=0` for run1 -> run2 and run2 -> run3.
- Full `cd avmatrix && npm test` passed after the launcher build and regenerated benchmarks. The accepted captured log contained no `Unhandled Errors`, `Unhandled Error`, `Worker vmForks emitted error`, `Worker forks emitted error`, `Worker exited unexpectedly`, `Test Files .*failed`, `Tests .*failed`, or `FAIL` patterns.
