# Anvien Web Graph Dynamic Scale Model And Zoom Semantics Plan
...
- Evidence ledger: [2026-05-27-anvien-web-graph-dynamic-scale-model-evidence.md](2026-05-27-anvien-web-graph-dynamic-scale-model-evidence.md)
...
3. Run the full build gate before testing. For this plan, full build means `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`, and it must rebuild `anvien-launcher\AnvienLauncher.exe`.
...
6. Only skip Anvien when running `git commit` for documentation-only staged changes. All docs technical planning, evidence, benchmark, report, and architecture work must use Anvien and read the codebase.
...
- The currently served launcher bundle at `anvien-launcher\web-dist\assets\index-B3tJBsjs.js` does not contain `graphOverview` or `graphInteraction`.
- The current Web build bundle at `anvien-web\dist\assets\index-Bxk3Ac59.js` does contain `graphOverview` and `graphInteraction`.
...
- The plan's full build gate must rebuild the launcher executable at `anvien-launcher\AnvienLauncher.exe`, not only rebuild Web assets.

## Codebase Findings

Anvien graph refresh was run on 2026-05-27 before this planning update:
...
Anvien graph refresh was run again on 2026-05-28 before reopening this plan:

```text
files: scanned=789 parsed=578 unsupported=211 failed=0
graph: nodes=90537 relationships=123975 path=E:\Anvien\.anvien\graph.json
```

Anvien query/context identified the current owners:

- `anvien-web/src/hooks/useSigma.ts`: Sigma setup, default graph load, camera reset.
- `anvien-web/src/lib/graph-readable-camera.ts`: readable camera selection and application.
- `anvien-web/src/lib/graph-adapter.ts`: graphology conversion, node color, node size, layout spacing, island geometry.
- `anvien-web/src/lib/graph-screen-spacing.ts`: viewport spacing diagnostics using `sigma.scaleSize`.
- `anvien-web/src/components/GraphCanvas.tsx`: diagnostics publication and Web graph integration.
- `anvien-web/src/lib/runtime-diagnostics.ts`: browser diagnostics object for e2e-visible runtime metrics.
...
- Anvien impact for `buildScreenNodeSpacingDiagnostics`, `buildGraphOverviewDiagnostics`, and `buildGraphOrientationLabels` is `CRITICAL`.
- Anvien impact for `applyFilterBasedClusteredLayout` is `HIGH`.
- `anvien-launcher\build.ps1` sets `$LauncherOutPath = Join-Path $LauncherRoot "AnvienLauncher.exe"`.
- `anvien-launcher\build.ps1` runs `npm run build` in `anvien-web`.
- `anvien-launcher\build.ps1` builds the canonical CLI at `anvien\bin\anvien.exe`.
- `anvien-launcher\build.ps1` builds the launcher with `go build -ldflags="-s -w -H=windowsgui" -o $LauncherOutPath .`.
- `anvien-launcher\build.ps1` builds `anvien-launcher\server-bundle\anvien-server.exe`.
- `anvien-launcher\build.ps1` deletes and recopies `anvien-web\dist` to `anvien-launcher\web-dist`.
...
- `anvien-web/src/lib/graph-adapter.ts` is `764` lines and currently owns exported Sigma node/edge types, node rendered-size helpers, app-layer ring taxonomy, island key classification, layout geometry, node mass, graph conversion, edge conversion, label filtering, and depth filtering.
...
- full build rebuilds `anvien-launcher\AnvienLauncher.exe`;
...
- Web graph layout geometry currently in `anvien-web/src/lib/graph-adapter.ts`;
- behavior-preserving code split from `graph-adapter.ts` into responsibility-owned modules before topology edits;
- a new single-owner scale model module `anvien-web/src/lib/graph-scale-model.ts`;
- Sigma/camera/render behavior in `anvien-web/src/hooks/useSigma.ts`;
- readable camera behavior in `anvien-web/src/lib/graph-readable-camera.ts`;
- graph screen diagnostics in `anvien-web/src/lib/graph-screen-spacing.ts`;
- browser runtime diagnostics in `anvien-web/src/lib/runtime-diagnostics.ts`;
- graph canvas diagnostics/test hooks in `anvien-web/src/components/GraphCanvas.tsx`;
- Web unit tests under `anvien-web/test/unit`;
- Web e2e tests under `anvien-web/e2e`;
- evidence and benchmark ledgers for this plan.
- graph diagnostics scheduling and throttling in `anvien-web/src/components/GraphCanvas.tsx`;
...
The implementation introduces one scale model module: `anvien-web/src/lib/graph-scale-model.ts`.
...
The evidence ledger must record the launcher exe `LastWriteTime`, file size, and content hash after the build. Browser validation must use the launcher-served app on `127.0.0.1:5228` after this build, not only Vite preview or `anvien-web\dist`.

## Acceptance Criteria

- Anvien refresh and impact checks are recorded before implementation edits.
- Phase 9 records Anvien/codebase evidence before code splitting and topology edits.
...
- Anvien detect-changes is run before implementation commit.
...
- Reopened Phase 8 uses `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` as the full build gate.
- The full build evidence proves `anvien-launcher\AnvienLauncher.exe` was rebuilt.
- The full build evidence proves `anvien-launcher\web-dist` was recopied from the current `anvien-web\dist`.
...
- `anvien-launcher\web-dist` is rebuilt from the current Web build and verified to contain the current diagnostics/interaction code.
...
- [x] P0-D Re-read plan, codebase, Anvien graph evidence, and revise this plan to a single concrete implementation direction.

## Phase 1 - Restore Original Color Overview Gate

- [x] P1-A Run `anvien analyze --force`.
- [x] P1-B Use Anvien context/query to trace current graph color assignment, initial camera, Sigma render, readable camera, layout, and overview diagnostics owners.
...
- [x] P1-Q Run Anvien detect-changes for the slice.
...
- [x] P2-A Use Anvien context/query to trace graph layout, Sigma render, camera, diagnostics, and e2e owners after Phase 1.
...
- [x] P3-A Add `anvien-web/src/lib/graph-scale-model.ts`.
...
- [x] P3-I Run Anvien detect-changes for the slice.
...
- [x] P4-I Run Anvien detect-changes for the slice.
...
- [x] P5-I Run Anvien detect-changes for the slice.
...
- [x] P6-I Run Anvien detect-changes for the slice.
...
- [x] P7-F Run final `anvien analyze --force`.
- [x] P7-G Run final `anvien detect-changes --repo Anvien --scope all`.
...
- [x] P8-A Run `anvien analyze --force`.
- [x] P8-B Use Anvien query/context to trace `GraphCanvas` camera updates, screen spacing diagnostics, graph overview diagnostics, orientation labels, wheel zoom, and package/launcher serving paths.
...
- [x] P8-O Run the full build gate: `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`.
- [x] P8-P Verify the full build refreshed `anvien-launcher\AnvienLauncher.exe` by recording `LastWriteTime`, file size, and hash.
- [x] P8-Q Verify `anvien-launcher\web-dist` was recopied from the current `anvien-web\dist`.
...
- [x] P8-X Run `anvien detect-changes --repo Anvien --scope all`.
...
- [x] P9-A Run `anvien analyze --force`.
- [x] P9-B Use Anvien query/context to trace current graph layout, graph conversion, hex regression, test owners, and code ownership.
...
- [x] P9-J Run `anvien detect-changes --repo Anvien --scope all` and commit the split slice.
...
- [x] P9-T Run the full build gate: `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`.
- [x] P9-U Verify the full build refreshed `anvien-launcher\AnvienLauncher.exe` by recording LastWriteTime, file size, and hash.
...
- [x] P9-X Run `anvien detect-changes --repo Anvien --scope all`.
