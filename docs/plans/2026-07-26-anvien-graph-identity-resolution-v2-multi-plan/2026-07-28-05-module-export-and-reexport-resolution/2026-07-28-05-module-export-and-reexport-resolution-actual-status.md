# Anvien Module Export Tables And Barrel/Re-Export Resolution Actual Status

Title: Anvien Module Export Tables And Barrel/Re-Export Resolution
Date: 2026-07-28
Status: P0 Complete / implementation blocked pending predecessor handoff and owner authority
Companion plan: `docs/plans/2026-07-26-anvien-graph-identity-resolution-v2-multi-plan/2026-07-28-05-module-export-and-reexport-resolution/2026-07-28-05-module-export-and-reexport-resolution-plan.md`
Companion evidence: `docs/plans/2026-07-26-anvien-graph-identity-resolution-v2-multi-plan/2026-07-28-05-module-export-and-reexport-resolution/2026-07-28-05-module-export-and-reexport-resolution-evidence.md`
Companion benchmark: `docs/plans/2026-07-26-anvien-graph-identity-resolution-v2-multi-plan/2026-07-28-05-module-export-and-reexport-resolution/2026-07-28-05-module-export-and-reexport-resolution-benchmark.md`
...
Successor child: `docs/plans/2026-07-26-anvien-graph-identity-resolution-v2-multi-plan/2026-07-28-06-ambient-external-resolution-and-diagnostics/2026-07-28-06-ambient-external-resolution-and-diagnostics-plan.md`
...
| Module/barrel resolution | import file path can resolve; imported symbol lookup stops at physical barrel definitions | export tables and proof-preserving terminal re-export resolution | wrong | 15/33/46 | `E0-P0A-SRC8`, `E0-P0A-IMPACT5..E0-P0A-IMPACT6` | P5 |
| P5 resolution vector/status contract | export/re-export outcomes lack an exhaustive named vector/status proof | phase-local vector manifest and exact tagged target/status/proof matrix | missing | 15-46 | `E0-P0A-SRC8` | P5-C |
...
| R9 | 2026-07-29 | HEAD `c637152fc211992105e4067daedf87a377da8723`; fresh `anvien analyze E:\Anvien --force --json` after correction overlay | qualified P4 handoff, derived export ownership, zero-barrel/path oracle, successor handoff | 1,557 scanned / 676 parsed / 757 documents / 85,181 nodes / 124,049 relationships / 0 failed; implementation remains pending and target untouched | `E2-PNC-REFRESH1`, `E2-PNC-MODULE1A..1E`, `E5-P5-DERIVED1`, `E5-P5-HANDOFF1` | keep P5-A blocked until exact slug-qualified Child 04 handoff is accepted |
| R10 | 2026-07-29 | HEAD `c637152fc211992105e4067daedf87a377da8723`; fresh `anvien analyze E:\Anvien --force --json` at 2026-07-29 02:01:49 +07:00 after final semantic correction edits | final P5 handoff/derived-count/path-oracle ownership freshness | 1,557 scanned / 676 parsed / 757 documents / 85,187 nodes / 124,055 relationships / 0 failed; all 4 P5 jobs remain pending; target untouched; `.anvien/meta.json` reports `indexedAt=2026-07-28T19:01:49Z` | `E2-PNC-REFRESH1`, `E2-PNC-MODULE1A..1E`, `E5-P5-DERIVED1`, `E5-P5-HANDOFF1` | keep P5-A blocked until exact slug-qualified Child 04 handoff and owner authority are accepted |
| R11 | 2026-07-29 | HEAD `c637152fc211992105e4067daedf87a377da8723`; fresh `anvien analyze E:\Anvien --force --json` at 2026-07-29 02:16:40 +07:00 after P6 capability-classification and exact catalog/transport ownership overlay | final cross-child graph/document freshness | 1,557 scanned / 676 parsed / 757 documents / 85,189 nodes / 124,057 relationships / 0 failed; all 4 P5 jobs remain pending; target untouched; `.anvien/meta.json` reports `indexedAt=2026-07-28T19:16:40Z` | `E2-PNC-REFRESH1` (pending), `E2-PNC-HANDOFF1` (pending), `E5-P5-HANDOFF1` (pending) | keep P5-A blocked until exact slug-qualified Child 04 handoff and owner authority are accepted |
| R13 | 2026-07-29 | HEAD `c637152fc211992105e4067daedf87a377da8723`; fresh `anvien analyze E:\Anvien --force --json` after P5 zero-barrel/path and write-owner hardening | current graph/document freshness and P5 derived-state boundary | 1,557 scanned / 676 parsed / 757 documents / 85,193 nodes / 124,061 relationships / 0 failed; all 4 P5 jobs remain pending; production and target remain untouched | `E2-PNC-REFRESH1` (pending), `E2-PNC-MODULE1A..1E` (pending), `E5-P5-DERIVED1` (pending), `E5-P5-HANDOFF1` (pending) | retain P5-A block until exact Child 04 handoff, three derived-count owners, zero-barrel/path oracle, and write-owner rows are accepted; graph refresh is not implementation proof |
...
| `internal/resolution/indexes.go` | export-table/re-export index registration link | current mixed coordinator | P5-B/P5-C | split then thin adapter | `E0-P0A-FD5` | no new traversal algorithm in coordinator |
| `internal/resolution/resolve.go` | terminal-result adapter link | current mixed resolver coordinator | P5-D | thin adapter | `E0-P0A-FD6` | no path resolution or derived-count ownership |
| `internal/providers/tsjs/imports.go` | syntax-fact adapter link | current TS import coordinator | P5-A/P5-B | split then thin adapter | `E0-P0A-FD9` | no terminal declaration lookup |
| `internal/resolution/import_resolution.go` | physical module-path adapter link | current module-path coordinator | P5-A | split then thin adapter | `E0-P0A-FD10` | preserve non-TS strategies; no export traversal |
| `internal/resolution/typescript_project.go` (NEW) | hash-bound TypeScript project-profile owner | immutable tsconfig/package profile | P5-A | dedicated new owner | `E2-PNC-MODULE1A` | no path/export traversal or declaration loading |
| `internal/resolution/public_api_projection.go` | sole derived public-api count owner | dedicated P5-D projection | P5-D | editable dedicated owner | `E2-PNC-MODULE1C` | no syntax-fact or path-resolution mutation |
...
### TypeScript exports and module/barrel resolution

Current state:

Export is not first-class, projection properties drift, and imported-definition lookup scans physical definitions in the barrel rather than its export surface.

Required state:

```text
ExportFacts own module export syntax and meaning.
Per-module export tables own export surfaces.
Re-export resolution returns one terminal `ResolutionTarget` + proof or a structured ambiguity/failure.
```

Evidence:

- `E0-P0A-SRC4`, `E0-P0A-SRC8`: current metadata/projection and resolver boundaries.
- `E0-P0A-IMPACT5..E0-P0A-IMPACT6`: CRITICAL import-binding paths.

Relationship and impact:

- Related file count: 15-46 across provider/resolver, plus 19/42 persistence/emission.
- Relationship summary: export syntax -> module table -> re-export proof -> terminal call/use edge.
- Impact note: CRITICAL resolver workflow; export contract must precede traversal.
...
P4 fact/projection slices, then P5 module resolution slices.

Forbidden next action:

Recursive calls inside the current physical-definition lookup, wildcarding all physical definitions, or first-candidate selection.
...
|-----------|-----------------------|--------------------------------------|
| P5 | physical barrel lookup is wrong | implement project input/table before traversal/emission |
...
P0 is complete for planning, but no production slice is authorized. The current permitted slice is P5-A only after the exact qualified Child 04 handoff, owner authority, and the P4-produced fact manifest are accepted. The Current Multi-Plan Execution Override below is operative; the copied P1-A wording is historical provenance. Scanner remediation remains excluded.
...
|---|---|---|---|
| P5-A | blocked | exact `2026-07-28-04-typescript-export-semantics::E2-PNC-HANDOFF1` plus owner authority and accepted ModuleRequestFact/ImportBindingFact producers | refresh P0 from Child 04 handoff; preserve syntactic path edges while proving zero-physical-declaration barrel surfaces, absolute before/after path/IMPORTS counts, and P5-derived count ownership |

The 98 implementation slices remain pending. No barrel/re-export resolver fix is claimed fixed. `E2-PNC-MODULE1A..1E`, `E5-P5-DERIVED1`, `E5-P5-HANDOFF1`, and `E2-PNC-NEXTSTATUS1` are required fresh evidence for the next refresh.
