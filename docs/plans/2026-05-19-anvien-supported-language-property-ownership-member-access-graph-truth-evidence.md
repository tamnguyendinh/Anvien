# Anvien Go Supported-Language Property Ownership And Member Access Graph Truth Evidence

Date: 2026-05-19

Status: complete

Companion plan: [2026-05-19-anvien-supported-language-property-ownership-member-access-graph-truth-plan.md](2026-05-19-anvien-supported-language-property-ownership-member-access-graph-truth-plan.md)

Companion benchmark: [2026-05-19-anvien-supported-language-property-ownership-member-access-graph-truth-benchmark.md](2026-05-19-anvien-supported-language-property-ownership-member-access-graph-truth-benchmark.md)
...
- For doc-only commits, do not run Anvien solely for the commit.
- For implementation slices, use Anvien for codebase analysis and impact checks.

## Plan Creation Evidence

- Created a new plan because the 2026-05-16 graph accuracy plan is scoped to Go-local measured gates on `E:\Anvien`.
- Initial follow-up scope was too narrow because it named TypeScript only.
- The corrected scope is supported-language property/access graph accuracy across every language that appears in Anvien-Go graph `Property` facts.
...
- `reports\benchmark\2026-05-16-120908-anvien-self-analyze.json`
- `reports\benchmark\2026-05-16-120908-anvien-self-analyze.md`
...
anvien resolvedAccesses:              3
anvien-main scopeResolutionResolvedAccesses: 755
```

Conclusion:

- The `3` vs `755` access number is not a final graph-edge comparison.
- `anvien-main` value `755` is an internal counter and must not be used as the final graph `ACCESSES` target.
...
| P1-C Anvien baseline gate | `.tmp\p1-property-access-anvien-baseline-20260519.json` | baseline recorded from explicit self-repo graph snapshot | done |
...
| P1-G unresolved access reasons | `.tmp\p1-access-candidates-website-20260519.json`, `.tmp\p1-access-candidates-anvien-20260519.json` | unresolved access candidates classified by reason | done |
...
Anvien use for analysis before scope correction:

```powershell
.\anvien-launcher\server-bundle\anvien.exe status
.\anvien-launcher\server-bundle\anvien.exe context Run --repo E:\Anvien --file internal\graphaccuracy\graphaccuracy.go
.\anvien-launcher\server-bundle\anvien.exe context main --repo E:\Anvien --file cmd\graph-accuracy-probe\main.go
.\anvien-launcher\server-bundle\anvien.exe context <first attempted property gate symbol> --repo E:\Anvien --file <first attempted property gate file>
```
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy
?   	github.com/tamnguyendinh/anvien/cmd/property-access-audit	[no test files]
```

Anvien refresh after the cross-language correction:
...
graph: nodes=20089 relationships=48012 path=E:\Anvien\.anvien\graph.json
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p1-property-access-anvien-graph-20260519.json -out .tmp\p1-property-access-anvien-baseline-20260519.json -max-examples 20
```
...
```text
wrote .tmp\p1-property-access-anvien-baseline-20260519.json
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/resolution
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy
?   	github.com/tamnguyendinh/anvien/cmd/access-candidate-audit	[no test files]
```
...
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\p1-access-candidates-anvien-20260519.json -max-examples 20
```
...
```text
wrote .tmp\p1-access-candidates-anvien-20260519.json
...
Anvien refresh after the P1-G implementation:
...
graph: nodes=20223 relationships=48368 path=E:\Anvien\.anvien\graph.json
```
...
Phase 2 second target: classify Anvien go_typed_property_without_owner=206 before any Go ownership expansion.
...
Phase 3 small bucket: close or reclassify Website missing_owner_link=1 and Anvien missing_owner_link=10 after ownership fixes.
```
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/lbugschema	0.938s
ok  	github.com/tamnguyendinh/anvien/internal/lbugload	1.155s
ok  	github.com/tamnguyendinh/anvien/internal/providers/tsjs	(cached)
ok  	github.com/tamnguyendinh/anvien/internal/resolution	(cached)
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy	0.528s
?   	github.com/tamnguyendinh/anvien/cmd/property-access-audit	[no test files]
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Website --force [redacted removed argument] --no-stats
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Anvien --force [redacted removed argument] --no-stats
```
...
graph: nodes=27764 relationships=54972 path=E:\Website\.anvien\graph.json
ANALYZE_MS=18,684.0
```
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20241 relationships=48415 path=E:\Anvien\.anvien\graph.json
ANALYZE_MS=15,755.6
```
...
```powershell
Copy-Item -LiteralPath 'E:\Website\.anvien\graph.json' -Destination '.tmp\p2-property-access-website-go-graph-20260519.json' -Force
Copy-Item -LiteralPath 'E:\Anvien\.anvien\graph.json' -Destination '.tmp\p2-property-access-anvien-graph-20260519.json' -Force
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p2-property-access-anvien-graph-20260519.json -out .tmp\p2-property-access-anvien-20260519.json -max-examples 20
```
...
```text
wrote .tmp\p2-property-access-anvien-20260519.json
...
- Anvien `go_typed_property_without_owner=206` was intentionally unchanged by P2-A and is classified by P2-D below.
...
Interpretation: Anvien reports low impact and no affected process traces for the staged slice. The risk is covered by full build, schema/load tests, TS/JS provider tests, analyze e2e on both workloads, and the property/access graph gate outputs recorded above.
...
Representative source samples from `.tmp\p2d-property-access-anvien-20260519.json`:

```text
anvien-launcher/src/main_test.go:16 args :: args []string
anvien-launcher/src/main_test.go:17 want :: want string
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy	0.609s
?   	github.com/tamnguyendinh/anvien/cmd/property-access-audit	[no test files]
```
...
```powershell
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p2-property-access-anvien-graph-20260519.json -out .tmp\p2d-property-access-anvien-20260519.json -max-examples 20
...
```text
wrote .tmp\p2d-property-access-anvien-20260519.json
properties.total=3089 ownerLinked=2762 standalone=327 hasPropertyEdges=2762 accessesEdges=2751 invalidHasPropertyEdges=0
language.go.properties=2508 ownerLinked=2302 standalone=206
language.typescript.properties=581 ownerLinked=460 standalone=121
orphan.false_orphan=12
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/providers/tsjs
ok  	github.com/tamnguyendinh/anvien/internal/resolution
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy
?   	github.com/tamnguyendinh/anvien/cmd/property-access-audit	[no test files]
```
...
analyzed E:\Website
files: scanned=1870 parsed=998 unsupported=872 failed=0
graph: nodes=27956 relationships=55957 path=E:\Website\.anvien\graph.json
ANALYZE_MS=17,093.2
```
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20268 relationships=48460 path=E:\Anvien\.anvien\graph.json
ANALYZE_MS=14,805.4
```
...
```powershell
Copy-Item -LiteralPath 'E:\Website\.anvien\graph.json' -Destination '.tmp\p2e-property-access-website-go-graph-20260519.json' -Force
Copy-Item -LiteralPath 'E:\Anvien\.anvien\graph.json' -Destination '.tmp\p2e-property-access-anvien-graph-20260519.json' -Force
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p2e-property-access-anvien-graph-20260519.json -out .tmp\p2e-property-access-anvien-20260519.json -max-examples 20
```
...
```text
wrote .tmp\p2e-property-access-anvien-20260519.json
...
- Anvien `real_edge_missing` moved from `12` to `0`.
- Invalid synthetic edge risk remains `0`.
- The remaining `unknown_no_edge` buckets are small and intentionally not linked without more evidence: Website `19`, Anvien `3`.
...
- `.tmp\p3a-access-candidates-anvien-20260519.json`

Commands:

```powershell
go run ./cmd/access-candidate-audit -repo E:\Website -out .tmp\p3a-access-candidates-website-20260519.json -max-examples 20
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\p3a-access-candidates-anvien-20260519.json -max-examples 20
```
...
```text
wrote .tmp\p3a-access-candidates-anvien-20260519.json
...
Anvien missing_receiver_type:
- anvien-launcher/server-wrapper/main.go:29 receiver=cmd name=Dir
- anvien-launcher/server-wrapper/main.go:54 receiver=os name=O_CREATE

Anvien missing_owner_link:
- anvien-web/src/components/FileTreePanel.tsx:291 receiver=treeNode.graphNode name=id
- anvien-web/src/core/llm/session-client.ts:18 receiver=this name=name
```
...
analyzed E:\Website
files: scanned=1870 parsed=998 unsupported=872 failed=0
graph: nodes=27956 relationships=58731 path=E:\Website\.anvien\graph.json
ANALYZE_MS=18,825.7
```
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20282 relationships=48550 path=E:\Anvien\.anvien\graph.json
ANALYZE_MS=14,784.4
```
...
```powershell
Copy-Item -LiteralPath 'E:\Website\.anvien\graph.json' -Destination '.tmp\p3b-property-access-website-go-graph-20260519.json' -Force
Copy-Item -LiteralPath 'E:\Anvien\.anvien\graph.json' -Destination '.tmp\p3b-property-access-anvien-graph-20260519.json' -Force
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p3b-property-access-anvien-graph-20260519.json -out .tmp\p3b-property-access-anvien-20260519.json -max-examples 20
```
...
Anvien property/access gate result:

```text
wrote .tmp\p3b-property-access-anvien-20260519.json
...
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\p3b-access-candidates-anvien-20260519.json -max-examples 20
```
...
Anvien access candidate result:

```text
wrote .tmp\p3b-access-candidates-anvien-20260519.json
...
- Anvien is recorded with final graph `ACCESSES=2,746` and access candidate `resolved=5,110`; this TS/JS-heavy slice primarily benefits Website.
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20318 relationships=48597 path=E:\Anvien\.anvien\graph.json
ANALYZE_MS=15,928.0
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p3e-property-access-anvien-graph-20260519.json -out .tmp\p3e-property-access-anvien-20260519.json -max-examples 20
```
...
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\p3e-access-candidates-anvien-20260519.json -max-examples 20
```
...
- P3-E closes `missing_owner_link` on both workloads: Website `768 -> 0`; Anvien `10 -> 0`.
...
- Anvien TypeScript accesses no longer resolve through unrelated Go owners with the same type name.
...
## P3-F Imported Member Receiver Cluster Evidence
...
- `resolveAccess` now falls back to imported workspace member resolution for property-like targets when the receiver is an import/package name.
- Access candidate audit classifies imported workspace member accesses as resolved and unresolved imported receivers as `external_library_type`.
- LadybugDB relation schema now accepts the real access pairs exposed by this cluster, including `Const -> Const`, `Function -> Variable`, `Method -> Const/Static/Variable`, `Package -> Const/Property/Static/Variable`, and `Const`/`Variable` to property-label targets.
- This slice does not fabricate owner links. It emits `ACCESSES` only when the imported member target resolves inside the analyzed workspace; otherwise the candidate remains a no-edge external/unmodeled case.
...
```powershell
go test ./internal/resolution ./internal/graphaccuracy ./internal/lbugschema ./cmd/access-candidate-audit
```
...
ok  	github.com/tamnguyendinh/anvien/internal/resolution
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy
ok  	github.com/tamnguyendinh/anvien/internal/lbugschema
?   	github.com/tamnguyendinh/anvien/cmd/access-candidate-audit	[no test files]
```
...
```text
TestResolveImportedWorkspaceMemberAccess
TestAuditAccessCandidatesResolvesImportedWorkspaceMember
TestAuditAccessCandidatesClassifiesUnresolvedImportReceiverAsExternal
TestSchemaQueriesPreserveDDLShape
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Website --force [redacted removed argument] --no-stats --benchmark-json .tmp\p3f-website-analyze-20260519.json --benchmark-label p3f-website
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Anvien --force [redacted removed argument] --no-stats --benchmark-json .tmp\p3f-anvien-analyze-20260519.json --benchmark-label p3f-anvien
```
...
analyzed E:\Website
files: scanned=1870 parsed=998 unsupported=872 failed=0
graph: nodes=27956 relationships=58732 path=E:\Website\.anvien\graph.json
```
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20324 relationships=50953 path=E:\Anvien\.anvien\graph.json
```

Schema regression discovered and fixed during e2e:
...
These were real graph pairs generated by imported workspace member access, so the fix was to extend the schema for those property-label target families instead of dropping the edges.

CLI/runtime property gate e2e:

```powershell
go run ./cmd/property-access-audit -repo E:\Website -graph .tmp\p3f-property-access-website-go-graph-20260519.json -out .tmp\p3f-property-access-website-20260519.json -max-examples 20
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\p3f-property-access-anvien-graph-20260519.json -out .tmp\p3f-property-access-anvien-20260519.json -max-examples 20
```
...
```text
properties.total=3096 ownerLinked=2769 standalone=327 hasPropertyEdges=2769 accessesEdges=5018 invalidHasPropertyEdges=0
graphTruth.real_edge_missing=0
graphTruth.true_no_edge=324
graphTruth.unknown_no_edge=3
```
...
```powershell
go run ./cmd/access-candidate-audit -repo E:\Website -out .tmp\p3f-access-candidates-website-20260519.json -max-examples 20
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\p3f-access-candidates-anvien-20260519.json -max-examples 20
```

Website access candidate result:

```text
accessCandidates.total=24542 resolved=4979 unresolved=19563 analyzeMillis=8037 resolvedAccesses=4979 unresolvedReferences=59288
reason.ambiguous_owner=109
...
```text
accessCandidates.total=21242 resolved=8431 unresolved=12811 analyzeMillis=6327 resolvedAccesses=8431 unresolvedReferences=47234
reason.ambiguous_owner=0
...
reason.resolved=8431
reason.unsupported_syntax=910
```

Deferred remainder:

- `missing_receiver_type`: requires broader receiver type/dataflow modeling for untyped locals, callback parameters, array elements, framework result objects, and chained receiver propagation.
- `external_library_type`: requires external package/stdlib/npm/framework models or explicit alias mapping; no local edge is defensible when the target is outside the analyzed workspace.
- `unsupported_syntax`: requires an expression evaluator for computed/index/call/new/optional-chain receiver expressions.

Interpretation:

- P3-F closes the imported member receiver cluster and records the remaining buckets as explicit future work, not hidden failures.
- The self repo is the main beneficiary: final graph `ACCESSES` increased from `2,747` to `5,018`, and candidate `resolved` increased from `5,114` to `8,431`.
- Website remains TypeScript-heavy and mostly blocked by dataflow/expression modeling; final graph `ACCESSES` increased from `2,769` to `2,770`.
...
affected process names include:
- AuditAccessCandidates -> DefRef
- ResolveAccess -> MergeRelationship
- ResolveAccess -> RelationshipTypeForReference
- ResolveAccess -> ParentScope
- ResolveAccess -> Add
- RunAccessCandidateAudit -> ParentScope
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Anvien --force [redacted removed argument] --no-stats --benchmark-json .tmp\p4-impact-default-anvien-analyze-20260519.json --benchmark-label p4-impact-default-anvien
```
...
analyzed E:\Anvien
files: scanned=682 parsed=527 unsupported=155 failed=0
graph: nodes=20351 relationships=50977 path=E:\Anvien\.anvien\graph.json
```

Context evidence:

```powershell
.\anvien-launcher\server-bundle\anvien.exe context parseFiles --repo E:\Anvien --file internal/analyze/analyze.go
.\anvien-launcher\server-bundle\anvien.exe context --uid 'Property:anvien-web/src/core/llm/types.local-runtime.ts:LLMSettings.intelligentClustering' --repo E:\Anvien
.\anvien-launcher\server-bundle\anvien.exe context RawSettingsShape --repo E:\Anvien --file anvien-web/src/core/llm/settings-service-local-runtime.ts
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe impact --uid 'Variable:internal/parser/parse.go:ErrUnsupportedLanguage' --repo E:\Anvien --direction upstream --depth 2
.\anvien-launcher\server-bundle\anvien.exe impact --uid 'Property:anvien-web/src/core/llm/types.local-runtime.ts:LLMSettings.intelligentClustering' --repo E:\Anvien --direction upstream --depth 2
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe cypher "MATCH (a)-[r:CodeRelation]->(b) WHERE a.id = 'Function:internal/analyze/analyze.go:parseFiles#4' AND b.id = 'Variable:internal/parser/parse.go:ErrUnsupportedLanguage' RETURN r.type, r.confidence, r.reason, r.resolutionSource, r.evidence LIMIT 5" --repo E:\Anvien
```
...
```text
ok  	github.com/tamnguyendinh/anvien/internal/providers
ok  	github.com/tamnguyendinh/anvien/internal/providers/astro
ok  	github.com/tamnguyendinh/anvien/internal/providers/c
ok  	github.com/tamnguyendinh/anvien/internal/providers/cpp
ok  	github.com/tamnguyendinh/anvien/internal/providers/csharp
ok  	github.com/tamnguyendinh/anvien/internal/providers/dart
ok  	github.com/tamnguyendinh/anvien/internal/providers/golang
ok  	github.com/tamnguyendinh/anvien/internal/providers/java
ok  	github.com/tamnguyendinh/anvien/internal/providers/kotlin
ok  	github.com/tamnguyendinh/anvien/internal/providers/php
ok  	github.com/tamnguyendinh/anvien/internal/providers/python
ok  	github.com/tamnguyendinh/anvien/internal/providers/ruby
ok  	github.com/tamnguyendinh/anvien/internal/providers/rust
ok  	github.com/tamnguyendinh/anvien/internal/providers/sfc
ok  	github.com/tamnguyendinh/anvien/internal/providers/svelte
ok  	github.com/tamnguyendinh/anvien/internal/providers/swift
ok  	github.com/tamnguyendinh/anvien/internal/providers/tsjs
ok  	github.com/tamnguyendinh/anvien/internal/providers/vue
ok  	github.com/tamnguyendinh/anvien/internal/mcp
ok  	github.com/tamnguyendinh/anvien/internal/resolution
ok  	github.com/tamnguyendinh/anvien/internal/graphaccuracy
ok  	github.com/tamnguyendinh/anvien/internal/lbugschema
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Website --force [redacted removed argument] --no-stats --benchmark-json .tmp\final-website-analyze-20260519.json --benchmark-label final-website
.\anvien-launcher\server-bundle\anvien.exe analyze E:\Anvien --force [redacted removed argument] --no-stats --benchmark-json .tmp\final-anvien-analyze-20260519.json --benchmark-label final-anvien
```
...
go run ./cmd/property-access-audit -repo E:\Anvien -graph .tmp\final-property-access-anvien-graph-20260519.json -out .tmp\final-property-access-anvien-20260519.json -max-examples 20
```
...
go run ./cmd/access-candidate-audit -repo E:\Anvien -out .tmp\final-access-candidates-anvien-20260519.json -max-examples 20
```
