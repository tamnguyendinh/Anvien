...
The skill says actual-status records true current state before implementation and requires phase rewrites when evidence changes scope. It does not explicitly define actual-status as a living status record after implementation starts.
...
Do not remove existing status matrix, relationship evidence, rewrite decisions, or implementation gate.

## Rewrite Decisions

| Original Plan Item | Actual Status Finding | Required Rewrite |
|--------------------|-----------------------|------------------|
| P1-A | Both target files are partial and low risk. | keep |
...
- [x] Main plan has been rewritten from this status file.
...
- [x] P0 complete. Main plan must be rewritten before implementation.
