# Anvien App Layer Resolution Gap Lens Evidence Ledger

Date: 2026-05-22

Status: Phase 9 and Phase 10 closure validation complete

Plan: [2026-05-22-anvien-app-layer-resolution-gap-lens-plan.md](2026-05-22-anvien-app-layer-resolution-gap-lens-plan.md)

Benchmark: [2026-05-22-anvien-app-layer-resolution-gap-lens-benchmark.md](2026-05-22-anvien-app-layer-resolution-gap-lens-benchmark.md)
...
- [reports/problem/2026-05-22-anvien-app-layer-resolution-gap-discussion.md](../../reports/problem/2026-05-22-anvien-app-layer-resolution-gap-discussion.md)
...
- `anvien analyze --force` on 2026-05-22 scanned `728` files, parsed `539`, reported `189` unsupported, `0` failed, and produced `22095` nodes and `54772` relationships at `.anvien/graph.json`.
- A follow-up planning review with `anvien analyze --force` on 2026-05-22 scanned `732` files, parsed `543`, reported `189` unsupported, `0` failed, and produced `22361` nodes and `55352` relationships. Baseline counts must still be refreshed at implementation start because graph counts drift as code changes.
...
- `anvien-web/src/lib/graph-adapter.ts` currently applies deterministic filter-based clustered layout during graph conversion and already caps rendered node size at `3`.
- `anvien-web/src/hooks/useSigma.ts` invokes layout work from the manual optimizer path; the current plan should change deterministic initial placement, not add automatic optimizer execution.
- `anvien-web/src/hooks/app-state/graph.tsx`, `anvien-web/src/lib/graph-health-filters.ts`, `anvien-web/src/components/FileTreePanel.tsx`, and `anvien-web/src/components/GraphCanvas.tsx` are current Web state/filter/layout surfaces and do not yet have App Layer or Resolution Health filter state.
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .tmp\2026-05-22-phase0-baseline.json --benchmark-label phase0-baseline
```
...
graph: nodes=22635 relationships=52144 path=E:\Anvien\.anvien\graph.json
```
...
| graph-health summary/report | `internal/graphhealth/compute.go`, `internal/graphhealth/policy.go`, `internal/httpapi/graph.go`, `internal/httpapi/server.go`, `internal/contracts/web_ui.go` | Topology status is computed by HTTP graph/report paths and typed in Web contracts; it is not persisted as a node property in `.anvien/graph.json`. Resolution diagnostics are persisted on source nodes under `graphHealthDiagnostics`. |
...
| generated Web contracts | `internal/contracts/web_ui.go`, `contracts/web-ui/anvien-web-contract.schema.json`, `anvien-web/src/generated/anvien-contracts.ts` | Contract manifest exposes App Layer and Functional Area enums/labels plus semantic status terms. Generated TypeScript exposes `FUNCTIONAL_AREAS`, `FUNCTIONAL_AREA_LABELS`, `FunctionalArea`, `GraphSemanticStatus.functionalArea`, `NodeProperties.functionalArea`, and `NodeProperties.functionalAreaSource` in addition to the App Layer fields. |
...
| Web graph app state | `anvien-web/src/hooks/app-state/graph.tsx`, `anvien-web/src/services/backend-client.ts`, `anvien-web/src/generated/anvien-contracts.ts` | Web state receives graph nodes, relationships, and semantic status from API/stream contracts; it must not invent missing semantic truth client-side. |
| Web graph filters/detail/layout | `anvien-web/src/lib/graph-adapter.ts`, `anvien-web/src/lib/graph-health-filters.ts`, `anvien-web/src/components/FileTreePanel.tsx`, `anvien-web/src/components/GraphCanvas.tsx`, `anvien-web/src/hooks/useSigma.ts` | Current deterministic layout and filters do not yet expose App Layer rings/Resolution Health filters. Optimizer remains manual through `useSigma`; Phase 7 must not add auto optimizer execution. |

## E4 - Baseline Unresolved And App Layer Evidence

Status: complete for Phase 0 closure audit.

Recorded during P0-D and P0-E from fresh `.anvien/graph.json`, `.tmp\2026-05-22-phase0-baseline.json`, and `.tmp\2026-05-22-phase0-source-site-accuracy.json`.
...
| Source path buckets | internal/providers 11299, anvien-web/src 5227, internal/mcp 4775, internal/httpapi 4391, anvien-web/test 3914, internal/resolution 3800, internal/cli 2996, internal/group 2175, internal/analyze 2009, internal/embeddings 1701, anvien-web/e2e 1596 |
...
| --- | --- |
| `anvien-web/src/**` | frontend, except `anvien-web/src/services/backend-client.ts` as `frontend_api_client`. |
| `anvien-web/test/**`, `anvien-web/e2e/**` | frontend_test. |
...
- `Dockerfile.cli`, `Dockerfile.web`, `anvien-web/vercel.json`, `baseline/phase-1-contract-freeze/*.json`, and several root/folder nodes remain `unknown` because the current rules do not have enough high-confidence ownership evidence.
...
| App Layer and ResolutionGap layout | `internal/semantic/app_layer.go`, `internal/semantic/functional_area.go`, `anvien-web/src/lib/graph-adapter.ts`, `anvien-web/src/lib/graph-health-filters.ts`, `anvien-web/src/hooks/useSigma.ts` | Same generic launcher/property-access/contracts/admin top processes | miss | Layout/filter terms are not matching Web graph surfaces; current retrieval is not layer-aware. |
| API contract surfaces | `internal/contracts/web_ui.go`, `contracts/web-ui/anvien-web-contract.schema.json`, `anvien-web/src/generated/anvien-contracts.ts`, `anvien-web/src/services/backend-client.ts` | Launcher flow first, then property-access audit and contract title-word process | partial | Contract file appears through `WebUIContract*`, but top result is unrelated launcher noise and frontend client/generated schema are not surfaced. |
| frontend graph filter surfaces | `anvien-web/src/lib/graph-health-filters.ts`, `anvien-web/src/hooks/app-state/graph.tsx`, `anvien-web/src/components/FileTreePanel.tsx`, `anvien-web/src/components/GraphCanvas.tsx` | Same generic launcher/property-access/contracts/admin top processes | miss | Frontend graph/filter files are absent from top results; query is dominated by unrelated process labels. |
| runtime reset hidden-terminal behavior | `anvien-launcher/src/main.go` `resetRuntime`, `stopRuntime`, `hiddenCommand`, `hiddenProcAttr` | `anvien-launcher/src/main.go` `main/resetRuntime/stopRuntime/stopPID/waitForPIDExit/processAlive/tasklistCommand/hiddenCommand/hiddenProcAttr` | hit | This intent matches because expected terms overlap the dominant launcher process. |
...
- `anvien-web/src/services/backend-client.ts` accepts the streamed `semantic_status` record and returns it as `semanticStatus` with the graph payload.
...
- Validation evidence: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused Go validation passed with `go test .\internal\semantic .\internal\httpapi .\internal\contracts`; wider Go validation passed with `go test .\internal\... .\cmd\...`; Web unit validation passed with `npm test -- --run` in `anvien-web`; whitespace validation passed with `git diff --check`.
...
- `go test ./...` still fails outside the implementation slice because fixture packages under `anvien/test/fixtures/...` are intentionally non-buildable as standalone Go packages; the prior real failure in `internal/analyze` is fixed.
...
- Accepted signals for direct node classification are deterministic high-confidence path/package rules. Examples from fresh graph output include `internal/resolution/access_audit.go` as `resolution`, `internal/graphhealth/**` and `internal/graphaccuracy/**` as `graph_health`, `internal/httpapi/**` as `api`, `anvien-web/src/lib/graph-adapter.ts` as `layout`, `internal/lbugload/**` and `internal/graph/**` as `storage`, and markdown/docs paths as `documentation`.
...
- Fresh analyze benchmark using `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .tmp\2026-05-22-functional-area-semantic-enrichment.json --benchmark-label functional-area-semantic-enrichment` produced 22358 nodes, 55349 relationships, 0 missing `functionalArea`, 0 missing `functionalAreaSource`, 20622 classified Functional Area nodes, and 1736 `unknown` nodes.
- Functional Area counts from `.anvien/graph.json`: analyzer 2426, api 1583, cli 943, configuration 37, contracts 248, documentation 1293, embeddings 701, graph_health 761, launcher 243, layout 280, mcp 1566, mixed 294, providers 4954, query 1154, reporting 308, resolution 1225, runtime 21, session 510, storage 1243, unknown 1736, web_graph_ui 832.
...
- Validation evidence for this slice: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused Go tests passed with `go test .\internal\semantic .\internal\lbugschema .\internal\lbugload .\internal\contracts .\internal\httpapi .\internal\analyze`; wider Go tests passed with `go test .\internal\... .\cmd\...`; Web unit tests passed with `npm test -- --run` in `anvien-web`. Web e2e was not run for Phase 2 because this slice changed persisted graph/contract metadata but no visible Web UI behavior.
- Anvien change detection before commit used `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it returned affected_count 24, changed_count 144, changed_files 18, risk_level `critical`. The critical scope is expected for this slice because it intentionally changes semantic enrichment, graph semantic status, LadybugDB schema/export, HTTP/Web contract surfaces, generated contracts, and plan/evidence ledgers.
...
- Impact context was checked with Anvien after `anvien analyze --force`: `resolveCall`, `resolveAccess`, `propertyLabels`, `semanticEdgeKey`, `workspace.resolveName`, and PHP import surfaces were inspected. CRITICAL/HIGH results were treated as blast-radius context requiring focused and wider validation, not as a prohibition on editing the required code.
- Full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`.
...
- Anvien change detection before commit used fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force` followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `25`, changed_files `9`, affected_count `0`, and risk_level `low`.
...
- `internal/contracts/web_ui.go` and generated `anvien-web/src/generated/anvien-contracts.ts` expose the new relationship and diagnostic fields to Web consumers without making the Web UI infer source-site truth.
...
- Validation evidence for this slice: full build passed twice with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused tests passed with `go test .\internal\resolution`, `go test .\internal\graphhealth`, `go test .\internal\lbugload .\internal\lbugschema`, and `go test .\internal\contracts`; wider backend validation passed with `go test .\internal\... .\cmd\...`.
- Fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force` passed after schema propagation and wrote `.anvien\graph.json` with scanned `733`, parsed `544`, unsupported `189`, failed `0`, graph nodes `22404`, and graph relationships `51521`.
- Anvien change detection before commit used fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force` followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `100`, changed_files `21`, affected_count `33`, and risk_level `critical`. The critical scope is expected because this slice intentionally changes graph relationship schema, graphhealth diagnostic schema, resolver edge emission metadata, strict ACCESSES target policy, LadybugDB relation schema/export/load, generated Web contracts, and plan/evidence/benchmark ledgers.
...
- `internal/cli/source_site_accuracy_command.go` adds packaged command `anvien source-site-accuracy` with `--graph`, `--out`, `--json`, and `--max-examples`. This puts the report into the built `anvien.exe` path instead of leaving it as a standalone development command.
...
- Fresh local build/analyze/report command: `.\anvien-launcher\server-bundle\anvien.exe analyze --force` produced scanned `736`, parsed `547`, unsupported `189`, failed `0`, graph nodes `22628`, graph relationships `52161`; `.\anvien-launcher\server-bundle\anvien.exe source-site-accuracy --graph .anvien\graph.json --out .tmp\2026-05-22-source-site-accuracy.json --max-examples 20` produced `84355` source-site occurrences with `84355` stable IDs and `0` missing IDs.
...
- The command found `16` graph-policy false-resolved-edge candidates, all from coarse `File -> Function` `CALLS` edges in Web unit test files such as `File:anvien-web/test/unit/constants.test.ts -> Function:anvien-web/src/lib/constants.ts:getNodeColor`. This keeps P2A-D and P2A-F open: coarse file-level call sources still need to be kept out of resolved symbol-level `CALLS` or split into a separate relation/fact role.
- Tests added: `TestRunSourceSiteAccuracyReportsProofInventory` and `TestSourceSiteAccuracyCommandOutputsJSON`.
- Validation evidence for this slice: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused tests passed with `go test .\internal\graphaccuracy` and `go test .\internal\cli`; wider backend validation passed with `go test .\internal\... .\cmd\...`.
...
- Fresh local build/analyze/report command after the gate: `.\anvien-launcher\server-bundle\anvien.exe analyze --force` produced scanned `736`, parsed `547`, unsupported `189`, failed `0`, graph nodes `22635`, graph relationships `52144`; `.\anvien-launcher\server-bundle\anvien.exe source-site-accuracy --graph .anvien\graph.json --out .tmp\2026-05-22-source-site-accuracy-after-file-source-call-gate.json --max-examples 20` produced `policy.falseResolvedEdgeCandidates=0`, `coarseFileCallEdges=0`, resolved edges without proof `0`, resolved edges without sourceSiteID `0`, low-confidence fallback resolved edges `0`, and non-property ACCESSES targets `0`.
- Validation evidence for this slice: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused tests passed with `go test .\internal\resolution` and `go test .\internal\graphaccuracy`; wider backend validation passed with `go test .\internal\... .\cmd\...`.
- Anvien change detection before commit used fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force` followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `15`, changed_files `7`, affected_count `0`, and risk_level `low`.
...
- Validation evidence for this slice: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused resolver validation passed with `go test .\internal\resolution`; focused graph/CLI/contract surfaces passed with `go test .\internal\graphaccuracy .\internal\cli .\internal\contracts .\internal\lbugload .\internal\graphhealth`; wider backend validation passed with `go test .\internal\... .\cmd\...`.
- Fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .tmp\2026-05-22-p2a-golden-postedit-analyze.json --benchmark-label p2a-golden-postedit` passed after the golden corpus slice and produced scanned `737`, parsed `548`, unsupported `189`, failed `0`, graph nodes `22710`, and graph relationships `52293`.
- Anvien change detection before commit used fresh analyze output followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; after staging the new test file and final ledger updates it reported changed_count `81`, changed_files `4`, affected_count `0`, and risk_level `low`.
...
- Validation evidence for this slice: full build passed with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; focused tests passed with `go test .\internal\graphaccuracy` and `go test .\internal\cli`; wider backend validation passed with `go test .\internal\...` and `go test .\cmd\...`.
- Fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .tmp\2026-05-22-p2a-source-site-accuracy-golden-command-postedit-analyze.json --benchmark-label p2a-source-site-accuracy-golden-command-postedit` passed before detect-changes and produced scanned `737`, parsed `548`, unsupported `189`, failed `0`, graph nodes `22751`, and graph relationships `52425`.
- Anvien change detection before commit used fresh analyze output followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `74`, changed_files `7`, affected_count `12`, and risk_level `high`. The high scope is expected because this slice extends `RunSourceSiteAccuracy`, `SourceSiteAccuracySummaryLines`, `newSourceSiteAccuracyCommand`, and new graphaccuracy fixture-validation helpers in a packaged CLI/report flow.
...
- Validation evidence for this slice: full build passed twice with `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1` because the focused test was corrected after an ordering assertion failure; final focused tests passed with `go test .\internal\graphhealth` and `go test .\internal\resolution .\internal\graphaccuracy`; wider backend validation passed with `go test .\internal\...` and `go test .\cmd\...`.
- Fresh `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .tmp\2026-05-22-p2a-gap-input-postedit-analyze.json --benchmark-label p2a-gap-input-postedit` passed before detect-changes and produced scanned `739`, parsed `550`, unsupported `189`, failed `0`, graph nodes `22806`, and graph relationships `52584`.
- Anvien change detection after staging the new source files used fresh analyze output followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `56`, changed_files `5`, affected_count `0`, and risk_level `low`.
- No visible Web UI behavior changed in this slice, so browser/e2e validation was not required for this slice.
...
- Fresh analyze command: `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p3-resolution-gap-persist-analyze.json --benchmark-label p3-resolution-gap-persist`.
...
- Field preservation inventory from `.anvien\graph.json`: missing source node ID `0`, missing sourceSiteID `0`, missing source-site status `0`, missing proof kind `0`, missing target text `0`, missing source App Layer `0`, missing actionability `0`; `7716` gaps have unknown source Functional Area because the source node was not functionally classified yet.
- Validation commands passed after full build: `go test .\internal\graphhealth .\internal\semantic .\internal\lbugschema .\internal\lbugload .\internal\contracts .\internal\scopeir`; `go test .\internal\...`; `go test .\cmd\...`; `go run .\cmd\generate-web-contracts --check`; `npm --prefix .\anvien-web run test -- --run`; `npm --prefix .\anvien-web run test:e2e` with `14` passed and `30` skipped.
- Role-validation analyze command: `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p3-role-validation-postedit-analyze.json --benchmark-label p3-role-validation-postedit`.
...
- Validation commands passed after full build for role-validation slice: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; `go test .\internal\graphhealth .\internal\semantic`; `go test .\internal\...`; `go test .\cmd\...`.
- Anvien change detection after staging the role-validation slice used fresh analyze output followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `344`, changed_files `8`, affected_count `0`, and risk_level `low`.
- Aggregate-policy analyze command: `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p3-aggregate-policy-postedit-analyze.json --benchmark-label p3-aggregate-policy-postedit`.
...
- Validation commands passed after full build for aggregate-policy slice: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; `go test .\internal\graphhealth`; `go test .\internal\...`; `go test .\cmd\...`.
- Anvien change detection after staging the aggregate-policy slice used fresh analyze output followed by `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all`; it reported changed_count `228`, changed_files `5`, affected_count `0`, and risk_level `low`.
...
- `anvien-web/src/lib/graph-health-filters.ts` maps flat graph properties into the new generated `GraphHealthNodeMetadata` fields when structured graphHealth is not present.
- `anvien resolution-inventory` reads persisted `.anvien\graph.json`, calls the same `graphhealth.ComputeSummary` full-count path as API/Web graph responses, and writes JSON or summary lines without using capped `/api/graph/report` candidates.
...
graph: nodes=82062 relationships=112614 path=E:\Anvien\.anvien\graph.json
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe resolution-inventory --graph .\.anvien\graph.json --out .\.tmp\2026-05-22-p4-resolution-inventory.json
```
...
- Full build passed after contract regeneration: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`.
...
- Web unit tests passed: `npm --prefix .\anvien-web run test -- --run` with `44` files and `358` tests passed.
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe query-health --suite .\docs\query-health\2026-05-22-anvien-app-layer-resolution-gap-suite.json --repo Anvien --out .\.tmp\2026-05-22-p5-query-health-baseline.json --limit 10
```

The command verifies the target repo is not stale by comparing indexed commit and current commit, then runs every suite intent through the same local MCP `query` path used by `anvien query`. It writes JSON with expected targets, actual top results, matched/missed targets, hit@5, hit@10, noise reason, pass/fail, and semantic fields if current query results return them. It also supports `--json` and `--fail-on-threshold`.

Suite fixture:

- `docs/query-health/2026-05-22-anvien-app-layer-resolution-gap-suite.json`
...
- graph-health separation: `internal/graphhealth/compute.go`, `internal/graphhealth/policy.go`, `anvien-web/src/lib/graph-health-filters.ts`;
- layout and optimizer surfaces: `anvien-web/src/lib/graph-adapter.ts`, `anvien-web/src/hooks/useSigma.ts`;
- runtime reset and hidden terminal behavior: `anvien-launcher/src/main.go`;
- API/contract/query surfaces: `internal/httpapi/graph.go`, `internal/contracts/web_ui.go`, `internal/mcp/tools.go`, `internal/cli/tool_command.go`, `cmd/anvien/main.go`, `anvien-web/src/generated/anvien-contracts.ts`;
- frontend graph filter/detail/layout surfaces: `anvien-web/src/hooks/app-state/graph.tsx`, `anvien-web/src/components/FileTreePanel.tsx`, `anvien-web/src/components/GraphCanvas.tsx`.

Fresh Phase 5 analyze evidence:

```text
files: scanned=748 parsed=558 unsupported=190 failed=0
graph: nodes=82538 relationships=113396 path=E:\Anvien\.anvien\graph.json
...
```text
queryHealth.suite=anvien-app-layer-resolution-gap-lens cases=7 passed=1 failed=6
...
- Most failing cases still return launcher `anvien-launcher/src/main.go`, `internal/graphhealth/diagnostics.go`, `internal/graphhealth/resolution_gap_inputs.go`, `internal/semantic/app_layer.go`, and `anvien-web/src/components/Header.tsx` as top files.
...
- `query-implementation-surfaces` misses `internal/mcp/tools.go`, `internal/cli/tool_command.go`, `cmd/anvien/main.go`, `queryTool`, `rankedProcessMatches`, `matchingDefinitionRows`, and `newQueryCommand`.
...
- Full build passed before final tests: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`.
...
graph: nodes=82935 relationships=113953 path=E:\Anvien\.anvien\graph.json
...
queryHealth.suite=anvien-app-layer-resolution-gap-lens cases=7 passed=7 failed=0
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe query "query ranking process matching definitions CLI query command implementation" --repo Anvien --limit 10
```
...
Pre-commit Anvien scope check:

```text
.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all
summary: affected_count=17, changed_count=432, changed_files=5, risk_level=critical
```

The critical scope is expected for this slice because it intentionally changes the shared `queryTool`, `rankedProcessMatches`, and `matchingDefinitionRows` retrieval/output path and adds tests plus plan ledgers. The affected processes reported by Anvien are query-tool and matching/ranking flows such as `QueryTool -> MinInt`, `RankedProcessMatches -> QueryTokenStem`, and `MatchingDefinitionRows -> MinInt`.
...
graph: nodes=83205 relationships=114335 path=E:\Anvien\.anvien\graph.json
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe context --repo Anvien --uid "Function:internal/mcp/context.go:contextSymbolPayload#2" > .\.tmp\2026-05-22-p6b-context-symbol-output.txt
.\anvien-launcher\server-bundle\anvien.exe context --repo Anvien --uid "ResolutionGap:SourceSite:internal/mcp/context.go#call#any#197#12#204#2" > .\.tmp\2026-05-22-p6b-context-resolution-gap-output.txt
```
...
graph: nodes=83519 relationships=114735 path=E:\Anvien\.anvien\graph.json
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe impact --repo Anvien --uid "Function:internal/mcp/context.go:contextSymbolPayload#2" --direction upstream > .\.tmp\2026-05-22-p6c-impact-context-symbol-output.txt
.\anvien-launcher\server-bundle\anvien.exe impact --repo Anvien --uid "Function:internal/mcp/impact.go:runImpactBFSProfiled#4" --direction upstream > .\.tmp\2026-05-22-p6c-impact-critical-warning-output.txt
```
...
graph: nodes=83783 relationships=115138 path=E:\Anvien\.anvien\graph.json
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all > .\.tmp\2026-05-22-p6d-detect-changes-semantic-output.txt
```
...
```text
files: scanned=752 parsed=562 unsupported=190 failed=0
graph: nodes=83982 relationships=115558 path=E:\Anvien\.anvien\graph.json
...
Pre-commit Anvien scope check after staging the P6-F/P6-G slice:
...
- The current Anvien graph has no persisted route nodes, so live `route_map`, `shape_check`, and `api_impact` artifacts show semantic status on empty/error route payloads; fixture tests prove populated route, consumer, flow-detail, and impact-summary semantic fields.
...
- `anvien-web/src/lib/semantic-filters.ts` centralizes App Layer labels, Resolution Health filter defaults, filterable node extraction, count builders, semantic predicates, and the required Resolution Health lens rows.
...
```text
npm --prefix .\anvien-web run test -- --run test/unit/semantic-filters.test.ts test/unit/graph-adapter.edge-geometry.test.ts test/unit/FileTreePanel.dashboard-completeness.test.tsx
```
...
```text
npm --prefix .\anvien-web run test -- --run test/unit/GraphCanvas.selection-performance.test.tsx
npm --prefix .\anvien-web run test -- --run
```
...
```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7a-p7b-web-semantic-filters-analyze.json --benchmark-label p7a-p7b-web-semantic-filters
files: scanned=755 parsed=565 unsupported=190 failed=0
graph: nodes=84880 relationships=116668 path=E:\Anvien\.anvien\graph.json
```

Impact and graph freshness evidence before the Web filter slice:

```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7-start-analyze.json --benchmark-label p7-start
...
Pre-commit Anvien scope check after staging the P7-A/P7-A2/P7-B Web filter slice:
...
```text
npm --prefix .\anvien-web run test -- --run test/unit/CodeReferencesPanel.graph-health.test.tsx
```
...
```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7c-detail-lens-start-analyze.json --benchmark-label p7c-detail-lens-start
files: scanned=755 parsed=565 unsupported=190 failed=0
graph: nodes=84880 relationships=116668 path=E:\Anvien\.anvien\graph.json

.\anvien-launcher\server-bundle\anvien.exe impact "CodeReferencesPanel" --repo Anvien --direction upstream
risk: LOW
...
Pre-commit Anvien scope check after staging the P7-C detail-panel slice:

```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7c-detail-lens-precommit-analyze.json --benchmark-label p7c-detail-lens-precommit
files: scanned=755 parsed=565 unsupported=190 failed=0
graph: nodes=84982 relationships=116778 path=E:\Anvien\.anvien\graph.json
...
The low scope is expected for this slice because it changes one Web detail panel, its focused unit test, and plan ledgers. Anvien reported no affected execution flows.
...
Browser runtime ring diagnostic captured against `Anvien` on `127.0.0.1:5228` with `anvien serve` on `127.0.0.1:4848`:
...
```text
anvien-web\test-results\server-connect-Graph-Dashb-1289f-ode-type-islands-in-browser-chromium\app-layer-rings-visible.png
```

Impact evidence before the layout/diagnostic edits:

```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7-layout-start-analyze.json --benchmark-label p7-layout-start
files: scanned=755 parsed=565 unsupported=190 failed=0
...
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7-layout-docs-anchor-preedit-analyze.json --benchmark-label p7-layout-docs-anchor-preedit
files: scanned=755 parsed=565 unsupported=190 failed=0
...
```text
powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1
npm --prefix .\anvien-web run test -- --run test/unit/graph-adapter.edge-geometry.test.ts test/unit/GraphCanvas.selection-performance.test.tsx test/unit/runtime-diagnostics.test.ts
npm --prefix .\anvien-web run test:e2e -- e2e/server-connect.spec.ts
```

Result:

```text
Full build passed after stopping the temporary `anvien serve` process that was locking anvien.exe.
...
Pre-commit Anvien scope check after staging the P7-D through P7-J layout slice:

```text
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p7-layout-final2-precommit-analyze.json --benchmark-label p7-layout-final2-precommit
files: scanned=755 parsed=565 unsupported=190 failed=0
...
| Spec | Result |
| --- | --- |
| `npm --prefix .\anvien-web run test:e2e -- e2e/server-connect.spec.ts` | 10 passed |
| `npm --prefix .\anvien-web run test:e2e -- e2e/graph-health-ui.spec.ts` | 4 passed |
| `npm --prefix .\anvien-web run test:e2e -- e2e/heartbeat-reconnect.spec.ts` | 2 passed |
| `npm --prefix .\anvien-web run test:e2e -- e2e/multi-repo-scoping.spec.ts` | 3 passed |
| `npm --prefix .\anvien-web run test:e2e -- e2e/repo-switching.spec.ts` | 6 passed |
| `npm --prefix .\anvien-web run test:e2e -- e2e/onboarding.spec.ts` | 12 passed, 1 packaged-launcher case skipped by `PACKAGED_LAUNCHER_E2E` flag |
| `npm --prefix .\anvien-web run test:e2e -- e2e/shell-interactions.spec.ts` | 7 passed |
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p8-final-analyze.json --benchmark-label p8-final
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe query-health --suite .\docs\query-health\2026-05-22-anvien-app-layer-resolution-gap-suite.json --repo Anvien --out .\.tmp\2026-05-22-p8-query-health-final.json --limit 10
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe resolution-inventory --graph .\.anvien\graph.json --out .\.tmp\2026-05-22-p8-resolution-inventory-final.json
```
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-22-p8-command-examples-analyze.json --benchmark-label p8-command-examples
```
...
| --- | --- | --- |
| `query "layout optimizer app layer ring graph filters" --repo Anvien --limit 10` | `.tmp\2026-05-22-p8-query-output.txt` | returned layout/frontend definitions including `knowledgeGraphToGraphology`, `compareAppLayerRingKeys`, `getAppLayerRingAngle`, and `useSigma`, with App Layer and Functional Area fields |
| `context "applyFilterBasedClusteredLayout" --repo Anvien` | `.tmp\2026-05-22-p8-context-output.txt` | found the function with `appLayer=frontend`, `functionalArea=layout`, incoming proof-backed calls/import uses, source-site proof/status metadata, and complete semantic status |
| `impact "applyFilterBasedClusteredLayout" --repo Anvien --direction upstream` | `.tmp\2026-05-22-p8-impact-output.txt` | reported `risk=CRITICAL`, `workflowWarningBlocksOutput=false`, `impactedCount=6`, affected App Layer `frontend=6`, affected Functional Areas `layout=3` and `web_graph_ui=3` |
| `detect-changes --repo Anvien --scope all` | `.tmp\2026-05-22-p8-detect-output.txt` | reported `risk_level=low`, `changed_count=13`, `changed_files=3`, changed App Layer `docs=13`, changed Functional Area `documentation=13`, no affected processes, and complete semantic status |
...
Because the remaining closure commit is doc-only, the repository rule's doc-only exception applies and no extra Anvien detect-changes gate is required solely for that final commit.
...
- Anvien detect-changes output for implementation commits;
- final commit hashes;
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-23-p9-non-actionable-breakdown-start-analyze.json --benchmark-label p9-non-actionable-breakdown-start
```
...
| Web lens rows | `query "resolution inventory non actionable builtin standard library test framework web graph shape size square"` found `anvien-web/src/lib/semantic-filters.ts`, `FileTreePanel`, and graph-health filter surfaces. `getResolutionLensRows` impact risk `LOW`. |
...
| Web lens rows for builtin / standard library / test framework | implemented in `anvien-web/src/lib/semantic-filters.ts`; the collapsed `Builtin/Test/Stdlib non-actionable` row is replaced by separate `builtin-non-actionable`, `standard-library-non-actionable`, and `test-framework-non-actionable` rows. |
| ResolutionGap classification island keys | implemented in `anvien-web/src/lib/graph-adapter.ts`; non-actionable ResolutionGap nodes with `builtin`, `standard_library`, or `test_framework` classification use `ResolutionGap:<classification>` island keys. |
| ResolutionGap square renderer and baseline size `1` | implemented with `NodeSquareProgram` in `anvien-web/src/lib/sigma-node-square-program.ts`, registered from `useSigma`, and assigned by `knowledgeGraphToGraphology` for `ResolutionGap` nodes with baseline size `1`. |
| Focused backend/Web tests | passed: `go test .\internal\cli -run "TestResolutionInventoryCommandOutputsJSON" -count=1`; `npm --prefix .\anvien-web run test -- --run test/unit/semantic-filters.test.ts test/unit/graph-adapter.edge-geometry.test.ts test/unit/FileTreePanel.dashboard-completeness.test.tsx` passed `3` files and `33` tests. |
| Full build and validation | passed: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; `go test .\internal\... .\cmd\...`; `go run .\cmd\generate-web-contracts --check`; `npm --prefix .\anvien-web run test -- --run` passed `45` files and `369` tests. |
| Browser/e2e validation | passed targeted browser coverage: `npm --prefix .\anvien-web run test:e2e -- e2e/server-connect.spec.ts -g "reports Backend API Frontend rings"` passed `1` test, and `npm --prefix .\anvien-web run test:e2e -- e2e/graph-health-ui.spec.ts` passed `4` tests. Full e2e and full `server-connect.spec.ts` timed out during validation and are not recorded as passed. |
| P9-I full e2e closure validation | passed after fresh full build gate. The all-spec command `npm --prefix .\anvien-web run test:e2e` was attempted and timed out at `1804044 ms`, so it is not counted as passing evidence. Per-spec reruns covered the full non-ignored suite: `server-connect.spec.ts` `10/10` passed in `11.1m`; `graph-health-ui.spec.ts` `4/4` passed in `20.9s`; `heartbeat-reconnect.spec.ts` `2/2` passed in `1.9m`; `multi-repo-scoping.spec.ts` `3/3` passed in `3.6m`; `repo-switching.spec.ts` `6/6` passed in `8.8m`; `onboarding.spec.ts` `12` passed and `1` expected packaged-launcher case skipped in `2.4m`; `shell-interactions.spec.ts` `7/7` passed in `6.3m`. Total closure evidence: `44` passed and `1` expected skip. |
| Final analyze | passed: `.\anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json .\.tmp\2026-05-23-p9-non-actionable-breakdown-final-analyze.json --benchmark-label p9-non-actionable-breakdown-final`; result `files scanned=756`, `parsed=566`, `unsupported=190`, `failed=0`, `nodes=85732`, `relationships=117678`. |
| Final resolution inventory | passed: `.\anvien-launcher\server-bundle\anvien.exe resolution-inventory --graph .\.anvien\graph.json --out .\.tmp\2026-05-23-p9-resolution-inventory-final.json`; result `unresolvedNonActionable=25841`, breakdown `builtin=10725`, `standard_library=7677`, `test_framework=7439`. |
| Detect-changes and commit | pre-commit `.\anvien-launcher\server-bundle\anvien.exe detect-changes --repo Anvien --scope all` passed after staging the implementation slice: `changed_count=163`, `changed_files=13`, `affected_count=4`, `risk_level=medium`. Affected scope is the expected frontend graph rendering path through `useSigma`; changed scope covers backend CLI output, Web lens/layout/filter code, tests, and plan ledgers. |
...
| Documentation | root README, packaged README, Anvien CLI/guide skill docs, and the generated AGENTS/CLAUDE context block describe threshold pass and exact pass as separate outcomes. |

Validation evidence to fill after P10-E/P10-F:

| Item | Result |
| --- | --- |
| Full build before tests | passed: `powershell -ExecutionPolicy Bypass -File .\anvien-launcher\build.ps1`; Vite emitted existing chunk-size/dynamic-import warnings only. |
...
| Source hygiene check | passed: `rg -n "gitnexus\|GitNexus\|git-nexus" internal cmd anvien README.md AGENTS.md CLAUDE.md` returned no source/context matches after replacing the old test fixture with an Anvien-managed block. |
| Live query-health command | passed: `.\anvien-launcher\server-bundle\anvien.exe query-health --suite .\docs\query-health\2026-05-22-anvien-app-layer-resolution-gap-suite.json --repo Anvien --out .\.tmp\2026-05-23-p10-query-health-threshold-exact.json --limit 10`. |
| Query-health threshold/exact counts | `thresholdPassed=7`, `thresholdFailed=0`, `exactPassed=2`, `exactFailed=5`, `matchedTargets=33/39`, `missedTargets=6`. Exact misses are `resolveCall`, `AppendDiagnosticToNode`, `startRuntime`, `anvien-web/src/generated/anvien-contracts.ts`, `cmd/anvien/main.go`, and `GraphCanvas`. |
| Pre-commit analyze/detect-changes | passed: fresh `analyze --force` produced `nodes=85822`, `relationships=117824`; `detect-changes --repo Anvien --scope all` was saved to `.tmp\2026-05-23-p10-detect-changes.txt` and reported `changed_count=152`, `changed_files=12`, `affected_count=12`, `risk_level=high`, changed App Layers `backend=78`, `backend_test=51`, `docs=23`, and affected App Layer `backend=12`. The high scope matches root CLI `query-health` output/scoring and generated context-copy changes. |
