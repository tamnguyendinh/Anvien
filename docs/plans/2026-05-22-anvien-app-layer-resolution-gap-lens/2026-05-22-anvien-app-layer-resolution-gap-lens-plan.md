# Anvien App Layer Resolution Gap Lens Plan
...
- Benchmark ledger: [2026-05-22-anvien-app-layer-resolution-gap-lens-benchmark.md](2026-05-22-anvien-app-layer-resolution-gap-lens-benchmark.md)
- Evidence ledger: [2026-05-22-anvien-app-layer-resolution-gap-lens-evidence.md](2026-05-22-anvien-app-layer-resolution-gap-lens-evidence.md)
...
4. Run a full build before testing. For this plan the full build gate is `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`.
...
10. Do not implement stale graph fallback. Graph-based work starts from `anvien analyze --force`, and fresh analyze output is the source of truth.
...
- persisted graph snapshot shape under `.anvien`;
...
- Web graph filter state is managed through `anvien-web/src/hooks/app-state/graph.tsx` in addition to panel and adapter files;
- Web graph conversion currently applies deterministic filter-based clustered layout in `anvien-web/src/lib/graph-adapter.ts`, and the manual optimizer button calls layout work through `anvien-web/src/hooks/useSigma.ts`;
...
- independent benchmark evidence shows Anvien ACCESSES is much cleaner than GitNexus because all sampled Anvien ACCESSES targets are Property-like while GitNexus over-emits function/method/const targets and duplicates heavily; this should be preserved with a strict ACCESSES contract rather than relaxed.
- independent benchmark evidence shows Anvien CALLS has zero duplicate pairs in the sampled comparison but still has a real false resolved edge where Go `main.stop()` is connected to TypeScript `SSEListener.stop`; this requires removing or gating dangerous simple-name/global fallback.
- GitNexus emits more CALLS but includes coarse `File -> Function` edges; Anvien should keep symbol-level source precision and should not use file-level source edges as proof of a resolved call.
...
- [x] [P0-E] Measure a provisional path/package-derived App Layer inventory without changing product behavior. Seed the audit with the discussion examples `anvien-web/src/**`, `anvien-web/test/**`, `anvien-web/e2e/**`, `internal/**`, `cmd/**`, `internal/httpapi/**`, `anvien-web/src/services/backend-client.ts`, `anvien-launcher/**`, `contracts/**`, `internal/contracts/**`, `cmd/generate-web-contracts/**`, `docs/**`, `reports/**`, `*.md`, `*_test.go`, `test/fixtures/**`, config files, package files, and build scripts. The evidence must show candidate counts for backend, api, frontend, cli_launcher, shared_contract, api_contract, api_shared_contract, frontend_api_client, backend_test, frontend_test, api_test, generated_contract, docs, config, generated, mixed, and unknown, plus examples explaining every uncertain bucket.
...
- [x] [P4-D] Add or extend a CLI inventory command for resolution gaps and semantic graph health. `anvien resolution-inventory --graph .anvien\graph.json --out .tmp\2026-05-22-p4-resolution-inventory.json` reads persisted analyze output and prints full counts for ResolutionGap nodes/relationships, occurrence counts, resolved references, App Layer, Functional Area, fact family, target role, classification, actionability, Resolution Health bucket, Resolution Confidence, and topology overlay without applying UI/report candidate caps.
...
- [x] [P5-B] Add a CLI command for query health so retrieval accuracy can be checked by running one command. `anvien query-health --suite <suite.json> --repo <repo> --out <report.json> --limit 10` verifies the indexed repo is not stale by commit, calls the same local MCP `query` implementation path as `anvien query`, scores results, writes JSON reports, and prints readable summary lines; `--json` and `--fail-on-threshold` are available for automation. Phase 10 adds `--fail-on-exact` for strict expected-target coverage.

- [x] [P5-C] Add initial suite entries for unresolved reference diagnostic generation, graph health unknown-connectivity separation, App Layer/resolution-gap layout, runtime reset hidden-terminal behavior, API contract surfaces, query implementation surfaces, and frontend graph filter surfaces. The suite at `docs/query-health/2026-05-22-anvien-app-layer-resolution-gap-suite.json` includes the required resolver, graph-health, query, CLI, API, contract, Web graph filter/layout/optimizer, and launcher runtime files.
...
- [x] [P7-C] Add node/detail-panel explanations for App Layer, Functional Area, Topology Health, Resolution Health, and related ResolutionGaps. `CodeReferencesPanel` now renders persisted App Layer, Functional Area, topology counters/status, Resolution Confidence, ResolutionGap counts, Resolution Health bucket counts, and attached persisted `ResolutionGap` entities through `HAS_RESOLUTION_GAP` relationships. The detail copy explicitly says degraded resolution confidence is unresolved-reference evidence and not a dead-code verdict; the focused graph-health panel test covers backend/resolution metadata, degraded confidence, unresolved call buckets, and related gap details.

- [x] [P7-D] Replace or extend the current deterministic filter-based clustered placement with deterministic multi-ring placement where App Layer is the macro position and node type or ResolutionGap kind is the micro island inside that ring. `applyFilterBasedClusteredLayout` now groups nodes first by persisted App Layer ring and then by node type or ResolutionGap kind island. Backend, API, and Frontend are distinct rings when present, API is placed on the bridge axis between Backend and Frontend, docs are anchored at the center, and contract/client/test/config/generated/mixed/unknown rings appear when the graph contains those categories.

- [x] [P7-E] Update Web graph adapter color and island assignment so existing node type/filter colors stay grouped inside each App Layer ring. `SigmaNodeAttributes` now carries `appLayerRing`, `islandKey`, and ring-center anchors; node color remains the existing node-type/filter color. Unit tests assert Function/Class islands stay separated inside a Backend ring, ResolutionGap kinds split into separate islands, and the browser diagnostic reports `sameColorIslandViolations=0`.

- [x] [P7-F] Add layout optimizer invocation guards and tests around render, data load, filter changes, and ring placement so optimizer execution is only triggered by the manual Web UI control. `GraphCanvas.selection-performance.test.tsx` verifies graph load/filter paths do not invoke manual optimization, and `server-connect.spec.ts` verifies `layout.starts=0`, `layout.stops=0`, and `manualOptimizerInvocations` changes only after clicking `Optimize Layout`. No product/runtime timeout, delayed refresh, or elapsed-time budget path was added.

- [x] [P7-G] Define ring size, spacing, ordering, and default visibility rules before coding layout behavior. The layout now uses a deterministic App Layer angle policy, adaptive island radius, island gap, ring gap, center clearance for docs, and unbounded additional rings when categories exist. Default visibility stays controlled by existing node type/App Layer/Resolution Health filters; node size remains capped at `3`, while readability is handled by island spacing, ring spacing, and screenshot-backed browser diagnostics.

- [x] [P7-H] Add Web unit tests for App Layer filters, Resolution Health filters, detail-panel fields, ring grouping, color/type island behavior, ring size/default visibility policy, graph-health composition, and no auto optimizer invocation. The final Phase 7 Web unit run passed `45` files and `368` tests, including `semantic-filters`, `FileTreePanel.dashboard-completeness`, `CodeReferencesPanel.graph-health`, `graph-adapter.edge-geometry`, `GraphCanvas.selection-performance`, and `runtime-diagnostics`.

- [x] [P7-I] Add Web e2e tests in a real browser proving Backend/API/Frontend rings are visible with API between Backend and Frontend, additional rings appear when data exists, ResolutionGaps are visible/filterable, filters do not collapse topology and resolution health, node colors remain grouped by type, default visibility behaves as specified, and the optimizer only runs from manual user action. Playwright against the local Vite UI and `anvien serve` covered all non-manual e2e specs: `server-connect` 10 passed, `graph-health-ui` 4 passed, `heartbeat-reconnect` 2 passed, `multi-repo-scoping` 3 passed, `repo-switching` 6 passed, `onboarding` 12 passed and 1 packaged-launcher case skipped by flag, and `shell-interactions` 7 passed.

- [x] [P7-J] Record screenshots, Playwright artifacts, visible ring/filter counts, ring placement evidence, default visibility evidence, and UI behavior evidence in the evidence and benchmark ledgers. Evidence E12 and benchmark B10/B11 now record the App Layer ring screenshot path, browser runtime diagnostic counts, API-between-Backend-and-Frontend proof, docs-centered proof, same-color island violation count, optimizer manual-only evidence, full Web unit results, and per-spec e2e results.

## Phase 8 - Full Validation And Closure

- [x] [P8-A] Run the full build gate before tests: `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`. Full build passed after the Phase 7 commit; the launcher build used Go `go1.26.3`, rebuilt `anvien-web/dist`, and produced the launcher/server bundle artifacts before any P8 tests were run.
...
- [x] [P8-D] Run Web unit tests for filters, detail panels, graph layout, manual optimizer behavior, and generated contract usage. `npm --prefix .\anvien-web run test -- --run` passed with `45` files and `368` tests.

- [x] [P8-E] Run Web e2e tests covering multi-ring layout, App Layer filters, Resolution Health filters, persisted ResolutionGap visibility, node type island grouping, and manual-only optimizer behavior. Playwright passed the full non-manual spec set against Vite on `127.0.0.1:5228` and `anvien serve` on `127.0.0.1:4848`: `44` passed and `1` packaged-launcher case skipped by flag.
...
- [x] [P8-I] Run Anvien detect-changes according to repository rules before implementation commits and record the affected symbols/flows in evidence, with the doc-only commit exception handled by the repository rules. All implementation slices recorded pre-commit detect-changes evidence; the final closure changes are docs-only, so no extra Anvien gate is required solely for the final doc commit.
...
- [x] [P10-D] Add or update tests and user-facing documentation for the separated query-health semantics. Focused CLI tests cover exact pass, exact fail with threshold pass, JSON fields, table fields, `--fail-on-threshold`, and `--fail-on-exact`; README, packaged README, Anvien CLI/guide skill docs, and the AGENTS/CLAUDE generated context block describe threshold and exact results separately.
...
- [x] [P10-F] Run fresh `anvien analyze --force` and `detect-changes` before the implementation commit, record the affected scope, then commit the slice. Fresh analyze passed with `nodes=85822` and `relationships=117824`; pre-commit `detect-changes --repo Anvien --scope all` reported `changed_count=152`, `changed_files=12`, `affected_count=12`, and `risk_level=high`. The high scope is expected for this slice because it changes root CLI `query-health` scoring/output behavior and AGENTS/CLAUDE generated context copy through `renderAnvienBlock`.
