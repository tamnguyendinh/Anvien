# Anvien Graph Health Unknown Connectivity Separation Plan
...
- Benchmark ledger: [2026-05-21-anvien-graph-health-unknown-connectivity-separation-benchmark.md](2026-05-21-anvien-graph-health-unknown-connectivity-separation-benchmark.md)
- Evidence ledger: [2026-05-21-anvien-graph-health-unknown-connectivity-separation-evidence.md](2026-05-21-anvien-graph-health-unknown-connectivity-separation-evidence.md)
...
4. Run the full build before testing. For this plan the full build gate is `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`.
...
The Web UI Graph Health filter currently shows a very large `Unknown` bucket. On the current `E:\Anvien` graph, investigation found `4345` nodes in that bucket. Turning off this bucket visually removes a large part of the graph, including many connected "islands".
...
- Current `E:\Anvien` `unknown_connectivity` count is no longer equal to the `graphHealthDiagnostics` node count.
- Current `E:\Anvien` connected-with-diagnostics nodes are still visible when only `Unknown` topology is hidden.
...
- `anvien-web/src/lib/graph-health-filters.ts` labels `unknown_connectivity` as `Unknown` and uses topology filters independently from diagnostic kind filters, but backend data currently conflates the two.
- `anvien-web/src/components/FileTreePanel.tsx` exposes Graph Health filters and counts from graph payload node metadata.
- Existing evidence shows `4345` diagnostic nodes on `E:\Anvien`, with `2036` still connected by counted edges.

## Implementation Slices

### P0 - Baseline and Reproduction

- [x] [P0-A] Reproduce current `/api/graph` Graph Health summary on a freshly analyzed `E:\Anvien` graph.
- [x] [P0-B] Record raw `.anvien\graph.json` diagnostic-node inventory and counted-edge degree buckets.
...
- [x] [P5-A] Run full build first: `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1`.
...
- [x] [P5-H] Re-run `anvien analyze --force` and record final inventory counts.
