# Anvien Runtime Lock And Process Lifecycle Hardening Evidence Ledger
...
- Plan: [2026-05-26-anvien-runtime-lock-process-lifecycle-hardening-plan.md](2026-05-26-anvien-runtime-lock-process-lifecycle-hardening-plan.md)
...
- `docs/plans/2026-05-26-anvien-runtime-lock-process-lifecycle-hardening-plan.md`
- `docs/plans/2026-05-26-anvien-runtime-lock-process-lifecycle-hardening-evidence.md`
- `docs/plans/2026-05-26-anvien-runtime-lock-process-lifecycle-hardening-benchmark.md`

Plan creation scope:

- Address stale `.anvien/analyze.lock` behavior after crash, killed process, or reboot.
...
- User reported that after analyze runs, Anvien appears to keep a process alive even after browser close and even after PC restart.
...
```powershell
Get-CimInstance Win32_Process | Where-Object { $_.Name -like '*anvien*' } | Select-Object ProcessId,Name,CommandLine,CreationDate | Format-List
```
...
|---:|---|---:|---|
| 14008 | `anvien.exe` | 912 | `anvien.exe mcp` from npm package path. |
| 912 | `cmd.exe` | 532 | Runs `anvien.cmd mcp`. |
| 532 | `codex.exe` | 1616 | Codex session process. |

Conclusion:

- The currently observed live process is editor/agent-owned `anvien mcp`, not an `analyze` process.
- Closing a browser is not expected to stop this process because it is owned by Codex through MCP setup.
- UX still needs improvement because users can reasonably misinterpret any long-running `anvien.exe` as a stuck analyze process.
...
- Removing the stale lock allowed `.\anvien\bin\anvien.exe analyze --force` to run successfully.
...
```powershell
rg -n "analyze\.lock|index lock|lock is already held|acquiredAt|repository index lock|With.*Lock|lock file|stale lock|Remove.*lock|pid=" internal cmd anvien-launcher -g "*.go" -g "*.ps1"
```
...
| Repo paths | `internal/repo/paths.go` | Lock path is `.anvien/analyze.lock`. |
| Setup MCP config | `internal/cli/setup_command.go` | Setup writes editor MCP entries with command `anvien` and args `["mcp"]`. |
| Launcher cleanup | `anvien-launcher/src/main.go` | Cleanup targets launcher-owned `serve --port 4848` and support processes, not editor-owned MCP. |
...
foreach ($pathItem in $paths) {
  if (Test-Path $pathItem) {
    Select-String -LiteralPath $pathItem -Pattern "anvien|mcp_servers|mcpServers|command|args" -Context 1,2
  }
}
```
...
- `anvien mcp` is intentionally configured as an editor/agent MCP server.
...
```powershell
.\anvien\bin\anvien.exe impact AcquireStorageLock --repo Anvien --direction upstream
```
...
```powershell
.\anvien\bin\anvien.exe impact AcquireStorageLock --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact ensureAnalyzeLockAvailable --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact handleEmbed --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact NewRootCommand --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact printSetupResult --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact collectAnvienProcesses --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact collectWindowsAnvienProcesses --repo Anvien --direction upstream
```
...
| `handleEmbed` | LOW | 0 | 0 | No upstream callers were reported by Anvien for the handler method. |
| `NewRootCommand` | CRITICAL | 1 | 11 | Root CLI command surface; direct external entry is `cmd/anvien/main.go:main`. |
| `printSetupResult` | CRITICAL | 3 | 11 | Setup output path through root command; wording-only change. |
| `collectAnvienProcesses` | CRITICAL | 3 | 11 | Diagnostics process collection path through `doctor processes`. |
| `collectWindowsAnvienProcesses` | LOW | 3 | 0 | Windows diagnostics helper under `doctor processes`. |
...
```powershell
.\anvien\bin\anvien.exe doctor locks --repo .
.\anvien\bin\anvien.exe doctor locks --repo . --json
.\anvien\bin\anvien.exe doctor processes --json
```
...
Anvien analyze lock
Repo: E:\Anvien
Storage: E:\Anvien\.anvien
Lock: E:\Anvien\.anvien\analyze.lock
Status: free
```
...
{
  "repoPath": "E:\\Anvien",
  "storagePath": "E:\\Anvien\\.anvien",
  "lockPath": "E:\\Anvien\\.anvien\\analyze.lock",
  "status": "free",
  "diagnosis": {
    "exists": false,
    "alive": false,
...
- Included `anvien mcp` PID `14008`, parent `cmd.exe` PID `912`, parent chain rooted under `codex.exe`.
...
```text
MCP lifecycle: anvien mcp is editor-owned and may stay running while the editor or agent session is active.
Diagnostics: anvien doctor locks, anvien doctor processes
```
...
```text
ok github.com/tamnguyendinh/anvien/internal/repo
ok github.com/tamnguyendinh/anvien/internal/cli 9.586s
ok github.com/tamnguyendinh/anvien/internal/httpapi
ok github.com/tamnguyendinh/anvien/internal/analyze
```
...
```text
ok github.com/tamnguyendinh/anvien/internal/repo 3.603s
ok github.com/tamnguyendinh/anvien/internal/analyze 1.599s
ok github.com/tamnguyendinh/anvien/internal/httpapi 2.633s
ok github.com/tamnguyendinh/anvien/internal/cli 8.674s
```
...
- Failed because it tries to build intentionally invalid fixture packages under `anvien/test/fixtures` and `node_modules`.
...
```powershell
$lock = Join-Path (Get-Location) '.anvien\analyze.lock'
...
graph: nodes=86682 relationships=118981 path=E:\Anvien\.anvien\graph.json
```
...
- `doctor locks --repo .` reported repo `E:\Anvien`, lock `E:\Anvien\.anvien\analyze.lock`, and status `free`.
- `doctor locks --repo . --json` reported status `free` with `reason: lock file does not exist`.
- `doctor processes --json` reported the active Playwright `anvien-web` test server and editor-owned MCP processes.
...
```powershell
.\anvien\bin\anvien.exe analyze --force
.\anvien\bin\anvien.exe impact LockHeldError --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact lockHeldMessage --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact classifyDoctorProcess --repo Anvien --direction upstream
.\anvien\bin\anvien.exe impact renderAnvienBlock --repo Anvien --direction upstream
```
...
| `renderAnvienBlock` | CRITICAL | 4 | 13 | Not edited after scope correction; generated AI context is out of scope for this lock bug. |
...
- The next action tells the user to wait for the owner, stop the PID only if stale, and inspect with `anvien doctor locks --repo <repo>`.
...
```text
ok github.com/tamnguyendinh/anvien/internal/repo 3.037s
ok github.com/tamnguyendinh/anvien/internal/analyze 5.070s
ok github.com/tamnguyendinh/anvien/internal/httpapi 6.110s
ok github.com/tamnguyendinh/anvien/internal/cli 9.767s
```
...
```text
exit=1
repository index lock is already held (path=E:\Anvien\.anvien\analyze.lock, pid=8636, host=TAM-PC, acquiredAt=2026-05-26T07:50:03Z, age=1h9m54s, command=manual live lock smoke, reason=owning process is still running, next=wait for the owning process to finish or stop pid 8636 if it is stale; inspect with anvien doctor locks --repo E:\Anvien)
```
...
exit=1
lockExists=True
repository index lock is already held (path=E:\Anvien\.anvien\analyze.lock, age=72ms, malformed=true, reason=lock metadata is malformed but recent, next=wait for the owning process to finish or stop the owning process if it is stale; inspect with anvien doctor locks --repo E:\Anvien)
```
...
- `doctor processes --json` reported the Playwright `anvien-web` test server as `anvien-runtime` and the active MCP command/wrapper as `editor-owned`.
...
analyzed E:\Anvien
files: scanned=766 parsed=569 unsupported=197 failed=0
graph: nodes=86778 relationships=119083 path=E:\Anvien\.anvien\graph.json
```
