# Anvien Multi-Language Graph Filters and Coverage Evidence Ledger
...
- Plan: [2026-05-19-anvien-multilanguage-graph-filters-and-coverage-plan.md](2026-05-19-anvien-multilanguage-graph-filters-and-coverage-plan.md)
- Benchmark ledger: [2026-05-19-anvien-multilanguage-graph-filters-and-coverage-benchmark.md](2026-05-19-anvien-multilanguage-graph-filters-and-coverage-benchmark.md)
...
- original file set: `docs/plans/2026-05-19-anvien-heritage-edge-semantics-and-coverage-{plan,benchmark,evidence}.md`
- current file set after scope correction: `docs/plans/2026-05-19-anvien-multilanguage-graph-filters-and-coverage-{plan,benchmark,evidence}.md`
...
- This plan file creation did not run Anvien. It records evidence already gathered during the preceding investigation.
...
```powershell
$g = Get-Content -Raw -LiteralPath 'E:\Restaurant_manager\.anvien\graph.json' | ConvertFrom-Json
...
```powershell
.\anvien-launcher\server-bundle\anvien.exe context RelExtends --repo Anvien
.\anvien-launcher\server-bundle\anvien.exe context HeritageExtends --repo Anvien
...
```powershell
rg -n --glob '*.ts' --glob '*.tsx' --glob '!node_modules/**' --glob '!.git/**' --glob '!.anvien/**' --glob '!reports/**' --glob '!Docs/**' --glob '!dist/**' --glob '!build/**' "^\s*(export\s+)?interface\s+\w+\s+extends\s+" E:\Restaurant_manager

rg -n --glob '*.ts' --glob '*.tsx' --glob '!node_modules/**' --glob '!.git/**' --glob '!.anvien/**' --glob '!reports/**' --glob '!Docs/**' --glob '!dist/**' --glob '!build/**' "^\s*(export\s+)?class\s+\w+\s+extends\s+" E:\Restaurant_manager
```
...
### Anvien-Assisted Checks

Commands:

```powershell
anvien analyze --force
anvien context FileTreePanel --repo Anvien
anvien impact FileTreePanel --repo Anvien --direction upstream --depth 2 --include-tests
...
anvien context Header --repo Anvien
anvien impact Header --repo Anvien --direction upstream --depth 2 --include-tests
anvien context resolveHeritage --repo Anvien
anvien impact resolveHeritage --repo Anvien --direction upstream --depth 2 --include-tests
anvien context emitReferenceKind --repo Anvien
anvien impact emitReferenceKind --repo Anvien --direction upstream --depth 2 --include-tests
```
...
- `anvien-web/src/lib/lucide-icons.tsx`
...
- Back target resolves to `/Start-Anvien.html` on the current origin.
- Vite dev/build serves or emits `Start-Anvien.html`.
...
go run ./cmd/anvien analyze E:\Restaurant_manager --force [redacted removed argument] --no-stats
```

Results:

```text
Anvien analyze runtime: 17.89s
Anvien graph: nodes=20,771 relationships=51,854
Anvien graph-present labels=16 relationship types=11
Anvien heritage: raw=0 uniquePairs=0 duplicateCompatibilityPairs=0
...
npm --prefix anvien-web test -- --run
npm --prefix anvien-web run test:e2e -- shell-interactions.spec.ts -g "back button|resizes the left dashboard" --workers=1 --timeout=120000
```
...
- Web production build passed and emitted `dist/Start-Anvien.html`.
- Full Vitest suite passed: `41` files, `322` tests.
- Focused Playwright e2e passed: `2/2`.

Additional note:

- `go test ./...` was also attempted. It fails on pre-existing non-buildable fixture packages under `anvien/test/fixtures` such as fixture imports from `models`/`animal`, C source files without cgo, and intentionally invalid Go examples. The applicable Go validation scope for this slice is `./cmd/... ./internal/...`.
...
- add a Back arrow/button beside the `Anvien` top bar title to return to `Start-Anvien.html`;
...
- no Anvien analysis was run for this documentation correction.
...
- the graph filter/fact fix cannot be bounded to TypeScript, Go, or heritage relationships because Anvien declares a multi-language code surface;
...
## E10 - Anvien-Assisted Codebase Audit For Multi-Language Graph Filters
...
- `anvien status` reported the index was stale.
- `anvien analyze` first failed during native LadybugDB schema initialization with `File already exists in catalog`.
- `anvien analyze --force` succeeded:

```text
files: scanned=691 parsed=530 unsupported=161 failed=0
graph: nodes=19714 relationships=47169 path=E:\Anvien\.anvien\graph.json
```

Anvien context/impact checks:

- `FileTreePanel` is used by `anvien-web/src/App.tsx` and `anvien-web/test/unit/FileTreePanel.dashboard-completeness.test.tsx`.
- `knowledgeGraphToGraphology` is used by `anvien-web/src/components/GraphCanvas.tsx` and `anvien-web/test/unit/graph-adapter.edge-geometry.test.ts`.
- `getGraphEdgeVisibilityMode` is used by `anvien-web/src/hooks/useSigma.ts` and `anvien-web/test/unit/graph-edge-visibility-mode.test.ts`.
...
- `anvien-web/src/lib/constants.ts` imports generated graph contracts and defines display behavior: default visible labels, colors, sizes, edge labels/colors, sort order, and unknown fallbacks.
- `anvien-web/src/components/FileTreePanel.tsx` builds loaded-graph node/edge filter rows from graph-present labels/types, not from every contract row; no-graph fallback uses the full generated lists.
- `anvien-web/src/lib/graph-edge-visibility-mode.ts` and `anvien-web/src/hooks/useSigma.ts` do apply `visibleEdgeTypes` to rendered edges through the Sigma edge reducer.
- `anvien-web/src/lib/graph-adapter.ts` still contains hand-classified graph behavior: `structuralTypes`, `symbolTypes`, hierarchy relationship priorities, node mass rules, edge size multipliers, node size caps, and community-color symbol handling. Those sets are not proven complete for all generated node labels and relationship types.
...
- scope now includes `anvien-web/src/lib/graph-edge-visibility-mode.ts` and `anvien-web/src/hooks/useSigma.ts`, because edge visibility is enforced through the graph edge visibility mode and Sigma reducer, not only through dashboard row generation;
...
- Phase 5 and Phase 6 now require full graph filter inventory and final graph/filter verification for both `E:\Anvien` and `E:\Restaurant_manager`.

Doc-only note:

- no Anvien analysis was run for this documentation correction; it uses the Anvien-assisted audit evidence recorded in E10.
...
```powershell
go run ./cmd/anvien context schemaResource --repo Anvien
go run ./cmd/anvien impact schemaResource --repo Anvien --direction upstream --depth 2 --include-tests
```
...
- `docs/plans/2026-05-19-anvien-multilanguage-graph-filters-and-coverage-plan.md`
- `docs/plans/2026-05-19-anvien-multilanguage-graph-filters-and-coverage-benchmark.md`
- `docs/plans/2026-05-19-anvien-multilanguage-graph-filters-and-coverage-evidence.md`
...
- `FileTreePanel` is used by `anvien-web/src/App.tsx` and `anvien-web/test/unit/FileTreePanel.dashboard-completeness.test.tsx`.
- upstream depth-2 impact also reaches `anvien-web/src/main.tsx` through `App.tsx`;
...
Anvien: nodes=20,771 edges=51,854
```
...
```powershell
npm --prefix anvien-web test -- --run test/unit/constants.test.ts test/unit/FileTreePanel.dashboard-completeness.test.tsx
npm --prefix anvien-web run test:e2e -- shell-interactions.spec.ts -g "displays graph filters" --workers=1 --timeout=120000
npm --prefix anvien-web run build
npm --prefix anvien-web test -- --run
npm --prefix anvien-web run test:e2e -- shell-interactions.spec.ts -g "back button|resizes the left dashboard|displays graph filters" --workers=1 --timeout=120000
```
...
```powershell
rg -n --glob '*.ts' --glob '*.tsx' --glob '!node_modules/**' --glob '!.git/**' --glob '!.anvien/**' --glob '!reports/**' --glob '!Docs/**' --glob '!dist/**' --glob '!build/**' "^\s*(export\s+)?interface\s+\w+\s+extends\s+|^\s*(export\s+)?class\s+\w+\s+extends\s+" E:\Restaurant_manager
```

ScopeIR trace validation:

```powershell
$env:ANVIEN_RESTAURANT_MANAGER_ROOT='E:\Restaurant_manager'
...
```powershell
Remove-Item Env:\ANVIEN_RESTAURANT_MANAGER_ROOT -ErrorAction SilentlyContinue
go test ./internal/providers/tsjs -run TestExtractRestaurantManagerTypeScriptHeritageSites -count=1 -v
...
- optional Restaurant_manager TS ScopeIR trace passed when `ANVIEN_RESTAURANT_MANAGER_ROOT` was set;
...
- no Anvien analysis was run for this review/update, per plan rule 6;
...
- The `Restaurant_manager` large-graph e2e smoke is not deterministic. `anvien-web/e2e/shell-interactions.spec.ts` chooses the first repository returned by `/api/repos`; the previous evidence only proves that `Restaurant_manager` happened to be first in that environment.
- `TestExtractRestaurantManagerTypeScriptHeritageSites` is useful audit evidence, but it is optional unless `ANVIEN_RESTAURANT_MANAGER_ROOT` is set. It is not currently a default regression gate.
...
```powershell
go run .\cmd\anvien status
go run .\cmd\anvien analyze --force [redacted removed argument] --no-stats
go run .\cmd\anvien context providerCoverage --repo Anvien
go run .\cmd\anvien impact providerCoverage --repo Anvien --direction upstream --depth 3 --include-tests
go run .\cmd\anvien context WebUIContract --repo Anvien
go run .\cmd\anvien impact WebUIContract --repo Anvien --direction upstream --depth 2 --include-tests
```

Result summary:

- initial Anvien status was stale after the doc-only reopen commit;
...
- The external `ANVIEN_RESTAURANT_MANAGER_ROOT` trace remains available as audit evidence and passed in this environment.
...
go run .\cmd\generate-web-contracts
npm --prefix anvien-web test -- --run test/unit/constants.test.ts
go build ./cmd/... ./internal/...
go test ./cmd/... ./internal/...
npm --prefix anvien-web run build
npm --prefix anvien-web test -- --run
npm --prefix anvien-web run test:e2e -- shell-interactions.spec.ts -g "back button|resizes the left dashboard|displays graph filters" --workers=1 --timeout=120000
$env:ANVIEN_RESTAURANT_MANAGER_ROOT='E:\Restaurant_manager'; go test ./internal/providers/tsjs -run TestExtractRestaurantManagerTypeScriptHeritageSites -count=1 -v
```
