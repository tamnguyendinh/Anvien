...
| Related-file metadata | Relationship groups expose related file path/counts/samples, but not file metadata | `relatedFiles` table with file metadata, direction flags, counts, kind counts, and stable refs | partial | 42 related files for builder source | `E0-P0A-DISCUSS1`, `E0-P0A-SRC1`, `E0-P0A-FD1` | implement P1-B |
...
| Generated contract artifacts | Generated TS/schema reflect expanded-only response | Generated artifacts reflect updated source contract | partial | 0 related files for schema artifact | `E0-P0A-SRC4`, `E0-P0A-FD7` | regenerate in P2-C; do not edit manually |
| MCP/agent file-context surface | MCP `context file`, file layer, and file impact flows call `BuildFileContext` and expose expanded file-context data | Preserve expanded MCP compatibility or add an explicit compact/expanded MCP contract with tests/smoke evidence | partial | 21/28/40 related files for target dispatch/context/impact sources | `E0-P0A-SRC6`, `E0-P0A-FD8`, `E0-P0A-FD9`, `E0-P0A-FD10`, `E0-P0A-IMPACT5` | implement P2-D before Web runtime validation |
| User-facing command docs | README/RUNBOOK document `file-detail`, `context file`, and `/api/file-detail` examples without compact/expanded limit semantics | Update docs or record no-change decision based on final default behavior | partial | N/A | `E0-P0A-SRC7`, `E0-P0A-REVIEW1` | implement P2-E after CLI/API/MCP decisions |
| Web client | `fetchFileContext` requests `/api/file-detail` with sample limits and returns expanded type | Client requests compact format and adapts compact payload without hidden fallback | partial | 22 related files for client source | `E0-P0A-SRC5`, `E0-P0A-FD5` | edit P3-A |
...
| P1-B | Related-file groups exist but related-file metadata is partial | keep P1-B and add `relatedFiles` metadata table |
...
| P2-E | README/RUNBOOK docs contain file-detail examples but docs scope was missing | add P2-E after final CLI/API/MCP behavior decisions |
| P3-A | Web client is bound to expanded type | keep P3-A after generated types exist |
