...
- `E2-P2D-IMPACT1`: Before P2-D edits, `anvien analyze --force` was rerun. `anvien impact file internal/mcp/target_dispatch.go --repo Anvien --direction upstream` showed 9 affected MCP files and `ContextToolInternal -> ContextCandidate`; `anvien impact file internal/mcp/context.go --repo Anvien --direction upstream` showed 12 affected files including CLI/HTTP MCP wrappers; `anvien impact file internal/mcp/impact.go --repo Anvien --direction upstream` showed 9 affected MCP files and `ImpactAutoPayload -> ContextCandidate`. This confirmed MCP file-context and file-impact share the expanded `BuildFileContext` path.
- `E2-P2D-SRC1`: P2-D preserves MCP/agent compatibility instead of introducing compact MCP payloads. `mcpBuildRepoFileContext` still returns expanded `filecontext.FileContext` with sample limits `5/5/5`; `context file` continues returning `fileContext` as expanded shape, and `impact` file target continues returning expanded `fileLayer`.
- `E2-P2D-TEST1`: Added assertions to `TestServeCallToolContextAndImpactResolveAbsoluteFileTargets` proving MCP `context` file payload has expanded `fileContext.relationships` and no compact `format`/`tables`, and MCP file impact payload has expanded `fileLayer.relationshipCounts` and no compact `tables`. `go test ./internal/mcp -run TestServeCallToolContextAndImpactResolveAbsoluteFileTargets -count=1` passed, then `go test ./internal/mcp -count=1` passed.
- `E2-P2D-BUILD1`: `go build ./cmd/... ./internal/...` passed after P2-D test changes.
- `E2-P2D-DETECT1`: `anvien detect-changes --repo Anvien --scope all` ran before the P2-D commit. It reported `5` changed files, `5` affected files, changed-file risk `low`, overall risk `low`, and the changed test symbol `TestServeCallToolContextAndImpactResolveAbsoluteFileTargets`.
- `E2-P2E-DOC1`: Inspected the user-facing docs surface for final `file-detail` behavior. `README.md` owns the command/API overview, and `RUNBOOK.md` owns operator smoke/troubleshooting examples. No generated `AGENTS.md`, `CLAUDE.md`, schema, or generated command-guide artifact was edited as source of truth for this docs slice.
- `E2-P2E-DOC2`: Updated README/RUNBOOK docs to state that CLI `file-detail --json` and `/api/file-detail` default to compact full-detail output, `--format expanded` / `format=expanded` preserve the legacy expanded shape, omitted compact limits mean full rows, explicit limits expose total/returned/omitted counts, and MCP `context file` remains expanded for agent compatibility.
- `E2-P2E-SMOKE1`: After `go run .\cmd\anvien analyze --force`, `go run .\cmd\anvien file-detail internal/httpapi/file_context.go --repo Anvien --json` returned `format=file-detail.compact`, `relationshipSamplesPerGroup=-1`, `relatedFiles=21`, and `60962` characters. `go run .\cmd\anvien file-detail internal/httpapi/file_context.go --repo Anvien --json --format expanded` returned the legacy expanded shape with `relationships` present and `3312` characters.
- `E2-P2E-DETECT1`: `go run .\cmd\anvien detect-changes --repo Anvien --scope all` ran before the P2-E commit. It reported `6` changed files, `7` affected files, changed-file risk `low`, overall risk `low`, docs-only changed app layer, and no affected processes.

## E3 - P3 Evidence

Matching plan item(s): `P3-A`, `P3-B`, `P3-C`

- `E3-P3A-IMPACT1`: Before P3-A edits, `go run .\cmd\anvien analyze --force` refreshed the graph. `go run .\cmd\anvien impact file anvien-web/src/services/backend-client.ts --repo Anvien --direction upstream --json` reported broad file-level blast radius: `29` affected files, `1` affected process, and `197` contained symbols. `go run .\cmd\anvien impact symbol "fetchFileContext" --uid "Function:anvien-web/src/services/backend-client.ts:fetchFileContext" --repo Anvien --direction upstream --json` narrowed the edited function to LOW risk with `4` impacted symbols, `3` affected files, and `0` affected processes. `go run .\cmd\anvien file-detail anvien-web/src/services/backend-client.ts --repo Anvien --json --relationships 1 --unresolved 1 --linked 1` reported `risk=high`, `symbols=197`, `relatedFiles=22`, `local=132`, `outbound=9`, `inbound=177`, `unresolved=392`, `linkedFlows=31`, and `linkedTests=6`. The broad file-level blast radius was treated as a scope warning; edits were limited to `fetchFileContext` wiring plus a new adapter.
- `E3-P3A-SRC1`: Added `anvien-web/src/services/file-detail-adapter.ts` to expand compact file-detail row tables into the existing expanded-compatible render model and preserve compact `relatedFiles` metadata. Updated `fetchFileContext` to request `format=compact`, adapt the response through the compact adapter, return `FileDetailContext`, and throw a visible malformed compact response error instead of falling back silently.
- `E3-P3A-TEST1`: `npm run test -- test/unit/server-connection.test.ts` passed with `22` tests. `npm run test -- test/unit/FileDetailPanel.test.tsx test/unit/server-connection.test.ts` passed with `2` files and `26` tests. Coverage includes compact request params, symbol tree expansion, relationship/unresolved/linked row expansion, related-file preservation, malformed compact row failure, and the existing panel major-section render.
- `E3-P3A-BUILD1`: `npm run build` in `anvien-web` passed after P3-A. Vite emitted existing chunk-size and mixed dynamic/static import warnings for large chunks; the build completed successfully.
- `E3-P3A-DETECT1`: After staging the new adapter file, `go run .\cmd\anvien detect-changes --repo Anvien --scope all` reported `7` changed files, `7` affected files, changed-file risk `high`, overall risk `low`, and no affected processes. The high changed-file risk came from frontend API client/adapter file scope; focused unit tests and Web build passed.
- `E3-P3B-IMPACT1`: Before P3-B edits, `go run .\cmd\anvien analyze --force` refreshed the graph. `go run .\cmd\anvien impact file anvien-web/src/components/FileDetailPanel.tsx --repo Anvien --direction upstream --json` reported `4` affected files, `0` affected processes, and `70` contained symbols. `go run .\cmd\anvien impact symbol "FileDetailPanel" --repo Anvien --direction upstream --json` reported LOW symbol risk with `3` affected files and `0` affected processes. `go run .\cmd\anvien file-detail anvien-web/src/components/FileDetailPanel.tsx --repo Anvien --json --relationships 1 --unresolved 1 --linked 1` reported `risk=high`, `symbols=70`, `relatedFiles=4`, `local=20`, `outbound=5`, `inbound=4`, `unresolved=298`, `linkedFlows=0`, and `linkedTests=1`.
- `E3-P3B-SRC1`: Updated `FileDetailPanel` to consume `FileDetailContext`, render a new `Related Files` section from compact adapter `relatedFiles`, and show per-file path, direction, risk, role, layer, symbol count, unresolved count, total relationships, and relationship-kind counts. Existing summary, quality, symbol tree, relationships, unresolved, and linked sections remain rendered.
- `E3-P3B-TEST1`: `npm run test -- test/unit/FileDetailPanel.test.tsx` passed with `4` tests. `npm run test -- test/unit/FileDetailPanel.test.tsx test/unit/server-connection.test.ts` passed with `2` files and `26` tests. The panel test asserts related-file metadata including path, direction, risk, and `USES` count while preserving the prior major-section assertions.
- `E3-P3B-BUILD1`: `npm run build` in `anvien-web` passed after P3-B. Vite emitted existing chunk-size and mixed dynamic/static import warnings; the build completed successfully.
- `E3-P3B-DETECT1`: After refreshing the graph, `go run .\cmd\anvien detect-changes --repo Anvien --scope all` reported `6` changed files, `6` affected files, changed-file risk `low`, overall risk `low`, and `0` affected processes.
- `E3-P3C-IMPACT1`: P3-C Docker server image build initially reached Linux `go build` and failed on `internal/cli/package_runtime.go` with duplicate `case "linux"` in `platformMatches`. Before editing, `go run .\cmd\anvien analyze --force` refreshed the graph. `go run .\cmd\anvien impact file internal/cli/package_runtime.go --repo Anvien --direction upstream --json` reported `4` affected files and package-runtime flow impact. `go run .\cmd\anvien impact symbol "platformMatches" --repo Anvien --direction upstream --json` and `go run .\cmd\anvien impact symbol "archMatches" --repo Anvien --direction upstream --json` each reported CRITICAL blast radius with `3` affected files, `4` impacted symbols, and `17` affected processes. `go run .\cmd\anvien file-detail internal/cli/package_runtime.go --repo Anvien --json --relationships 1 --unresolved 1 --linked 1` reported `risk=high`, `symbols=72`, `relatedFiles=4`, `unresolved=235`, `linkedFlows=6`, and `linkedTests=2`. The CRITICAL warning was treated as build-flow scope risk; the edit was limited to the two matching helpers.
- `E3-P3C-SRC1`: Updated `platformMatches` and `archMatches` to normalize aliases (`win32 -> windows`, `x64 -> amd64`) before comparing to `runtime.GOOS`/`runtime.GOARCH`. This preserves existing metadata compatibility while avoiding duplicate compile-time switch cases on Linux/Darwin/arm64 builds.
- `E3-P3C-TEST1`: `go test ./internal/cli` passed after the build-blocker fix. Added `TestPackageRuntimePlatformAndArchAliases` to cover current platform/arch matches plus alias semantics.
- `E3-P3C-BUILD1`: Docker Desktop runtime was started locally after the Docker CLI reported the daemon was unavailable. `docker build -f Dockerfile.cli -t anvien-server:file-detail-compact .` then passed, producing `anvien-server:file-detail-compact`. `docker build -f Dockerfile.web -t anvien-web:file-detail-compact .` passed, producing `anvien-web:file-detail-compact`; the web build emitted existing Vite chunk-size/mixed import warnings and npm audit vulnerability notices, but completed successfully.
- `E3-P3C-DETECT1`: After refreshing the graph, `go run .\cmd\anvien detect-changes --repo Anvien --scope all` reported `6` changed files, `6` affected files, changed-file risk `high`, overall risk `medium`, and `2` affected processes. The affected process risk is from the CLI package/runtime build path fixed for Docker validation.
