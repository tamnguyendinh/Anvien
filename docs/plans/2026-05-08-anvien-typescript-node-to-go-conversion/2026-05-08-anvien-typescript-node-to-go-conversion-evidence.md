# anvien-GO TypeScript/Node To Go Conversion Evidence

Source plan: [2026-05-08-anvien-typescript-node-to-go-conversion-plan.md](2026-05-08-anvien-typescript-node-to-go-conversion-plan.md)

This file contains execution evidence moved out of the checklist plan. Benchmark gates and benchmark result numbers live in [2026-05-08-anvien-typescript-node-to-go-conversion-benchmark.md](2026-05-08-anvien-typescript-node-to-go-conversion-benchmark.md).

## Phase 4 - HTTP API Shell

### Evidence

- `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed before tests.

- `go test ./cmd/... ./internal/... -count=1` passed.

- Runtime smoke passed: `go run ./cmd/anvien serve --host 127.0.0.1 --port 48747`
...
- `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed before tests using
  Go `1.26.3`.

- Package contents verified: `server-bundle\anvien-server.exe` exists,
  `server-bundle\anvien.exe` exists, and `server-bundle\node.exe` is absent.

- Runtime smoke passed: `server-bundle\anvien-server.exe` started the packaged Go backend and
...
- `go test ./cmd/... ./internal/... -count=1` passed, including scanner/ignore tests for
  built-in ignores, `.gitignore`, `.anvienignore`, `ANVIEN_NO_GITIGNORE`, include/exclude
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before loader
  tests.
...
- Attempted `go test ./... -count=1`; current full-module glob still enters baseline sample
  fixture directories under `anvien\test\fixtures` and fails on intentionally non-buildable
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before runtime
  tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before stdio
  guard tests.
...
- Added `.anvien\graph.json` as the current readable Go graph snapshot artifact for CLI analyze;
...
- Wired the Go analyze path to the embedding runtime behind `--embeddings`. The CLI now exposes the
  flag, resolves HTTP-mode dimensions from `ANVIEN_EMBEDDING_DIMS`, runs an `embeddings` phase
...
- Added a read-only native LadybugDB runner for server search. It opens the repo `.anvien/lbug`
...
- Wired the Go server embedding service to graph snapshot + native DB runtime. It loads
  `.anvien/graph.json`, opens `.anvien/lbug` through the native write runner, reuses existing
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before lock/force
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before HTTP search
  tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before HTTP embed
  job tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before CLI
  status/wiki tests.
...
- Added `internal/aicontext` for Go-owned AI context generation: repo-specific generated skills
  from graph `Community`/`MEMBER_OF`/`Process` data, base Anvien skill installation, and
  managed Anvien block upsert for `AGENTS.md` and `CLAUDE.md`.

- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before AI
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before
...
- Added Go local embedding model mode with Hugot `v0.7.2` and `onnxruntime_go v1.30.0`. HTTP
  ...
  Anvien-Go HuggingFace cache without Hub symlink/lock paths, normalize embeddings, and validate
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before local
  embedding tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before HTTP tests.
...
- Web UI runtime validation passed against the packaged Go backend. The test used isolated
  `ANVIEN_HOME` registries under `.tmp/`, a deterministic fixture with `package.json` plus the
  resolution/process fixture files, `anvien-launcher\server-bundle\anvien.exe serve --host
...
- Added CLI `anvien mcp`, routed through the Go stdio server. The command writes protocol frames
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP tests.

- Re-ran `go test ./internal/mcp ./internal/cli -count=1`.

- Re-ran `go test ./cmd/... ./internal/... -count=1`.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe mcp` with a real stdio
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before HTTP MCP
  tests.
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48713`
...
- Added Go MCP context resource support through `resources/templates/list` and `resources/read` for
  `anvien://repo/{name}/context`. The resource reads the Go registry/meta snapshot, emits
  text/yaml project stats, available MCP tools, and related resource URIs.

- Added `TestServeReadsRepoContextResource`.

- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before context
  resource tests.
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48714`
  with HTTP MCP `resources/read` for `anvien://repo/Anvien/context`; response contained
  `tools_available` and `project: Anvien`.

- Added Go MCP `resources/read` support for `anvien://repo/{name}/clusters` and
  `anvien://repo/{name}/processes`, backed by the Go `graph.json` snapshot and YAML summaries.
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48715`
  with HTTP MCP `resources/read` for `anvien://repo/Anvien/clusters` and
  `anvien://repo/Anvien/processes`; responses contained `modules:` and `processes:`.

- Added Go MCP static resources `anvien://repos` and `anvien://setup`.

- Added schema/detail resource support for `anvien://repo/{name}/schema`,
  `anvien://repo/{name}/cluster/{clusterName}`, and
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48716`
  with HTTP MCP `resources/list`, `resources/templates/list`, `resources/read` for schema,
  `prompts/list`, and `prompts/get`; responses contained `anvien://repos`,
  `anvien://repo/{name}/schema`, `INHERITS`, `detect_impact`, and the generated map prompt.
...
- Re-ran `anvien analyze --force` before MCP smoke and staged-scope detection.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48718`
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP context
  tests.
...
- Re-ran `anvien analyze --force` before MCP context smoke and staged-scope detection.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48719`
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP impact
  tests.
...
- Re-ran `anvien analyze --force` before MCP impact smoke and staged-scope detection.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48720`
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP
...
- Re-ran `anvien analyze --force` before MCP `detect_changes` smoke and staged-scope detection.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48721`
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP rename
  tests.
...
- Re-ran `anvien analyze --force` before MCP rename smoke and staged-scope detection.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48722`
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48724`
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe mcp` over stdio with
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48726`
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before group MCP
  tests.
...
- Smoke-tested `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 48727`
  with temporary `ANVIEN_HOME` and HTTP MCP `tools/call group_list` plus
  `tools/call group_status`; responses contained group list/detail/status payloads and next-step
  hints.

- Smoke-tested `anvien-launcher\server-bundle\anvien.exe mcp` over stdio with temporary
  `ANVIEN_HOME` and `tools/call group_status`; response was framed with `Content-Length` and
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before group
...
- Smoke-tested `go run ./cmd/anvien serve --host 127.0.0.1 --port <ephemeral>` with temporary
  `ANVIEN_HOME` and HTTP MCP `tools/call group_contracts` plus `tools/call group_query`;
  responses contained contract registry payloads and merged query results.

- Smoke-tested `go run ./cmd/anvien mcp` over stdio with temporary `ANVIEN_HOME` and
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before group sync
  tests.
...
- Smoke-tested `go run ./cmd/anvien serve --host 127.0.0.1 --port <ephemeral>` with temporary
  `ANVIEN_HOME` and HTTP MCP `tools/call group_sync` followed by `group_contracts`; responses
...
- Smoke-tested `go run ./cmd/anvien serve --host 127.0.0.1 --port <ephemeral>` with temporary
  `ANVIEN_HOME` and HTTP MCP StreamableHTTP initialize/tools-list calls; responses used
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before stale-index
  tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before stdout-safety
  tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before MCP snapshot
  tests.
...
- Rebuilt the local TypeScript runtime with `cd anvien && npm run build` before comparing MCP
  behavior.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before reject
  regression tests.
...
- Re-ran `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` before the second
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-python-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force` under isolated
    `ANVIEN_HOME=.tmp\phase14-python-full-e2e-home`.
  - CLI observable graph result: `files=3`, `nodes=19`, `relationships=39`, `communities=3`,
    ...
    `ENTRY_POINT_OF`, and `STEP_IN_PROCESS` present in `.anvien\graph.json`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-java-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-java-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-kotlin-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-kotlin-full-e2e-home`.
  - CLI observable graph result: `files=2`, `parsed=1`, `unsupported=1`, `nodes=23`,
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-c-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-c-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-csharp-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-csharp-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-cpp-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-cpp-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-rust-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-rust-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-php-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-php-full-e2e-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-dart-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-dart-full-e2e-home`.
  - CLI observable graph result: `files=2`, `parsed=1`, `unsupported=1`, `nodes=21`,
...
- `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed before full tests.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-vue-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-vue-full-e2e\.anvien-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-swift-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-swift-full-e2e\.anvien-home`.
  - CLI observable graph result: `files=2`, `parsed=1`, `unsupported=1`, `nodes=18`,
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-ruby-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-ruby-full-e2e\.anvien-home`.
  - CLI observable graph result: `files=2`, `parsed=1`, `unsupported=1`, `nodes=18`,
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-cobol-full-e2e`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-cobol-full-e2e\.anvien-home`.
...
- Full runtime E2E:
  - Fixture: `.tmp\phase14-framework-facts-full-e2e-clean`, indexed with launcher-built
    `anvien-launcher\server-bundle\anvien.exe analyze --force --benchmark-json` under isolated
    `ANVIEN_HOME=.tmp\phase14-framework-facts-anvien-home`.
...
- Runtime decision:
  the packaged launcher runtime is Go-owned. `AnvienLauncher.exe` is built from
  `anvien-launcher/src`, `anvien-server.exe` is built from `anvien-launcher/server-wrapper`,
  and the backend CLI/server binary is `anvien-launcher/server-bundle/anvien.exe`.

- Server-wrapper runtime:
  `anvien-launcher/server-wrapper/main.go` starts
  `anvien-launcher/server-bundle/anvien.exe serve --host 127.0.0.1 --port 4747`; it no longer
  starts a bundled `node.exe` plus `anvien/dist/cli/index.js serve`.

- Direct launcher tests:
  - `cd anvien-launcher/src && go test ./...` passed, covering action parsing, Web UI static
    serving/fallback, web-dist validation, and launcher state read/write.
  - `cd anvien-launcher/server-wrapper && go test ./...` passed, covering hidden process
    attributes for the packaged backend wrapper.

- Full launcher build:
  `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed before launcher
  tests and smoke checks.

- Package contents:
  `anvien-launcher/server-bundle` contains `anvien.exe` and `anvien-server.exe`;
  `anvien-launcher/server-bundle/node.exe` is absent.

- Packaged runtime smoke:
  - Started `AnvienLauncher.exe`.
  - Backend readiness passed at `http://127.0.0.1:4747/api/info`.
  - Web UI readiness passed at `http://127.0.0.1:5173`.
  - Playwright smoke loaded the packaged UI, clicked the `Anvien` repo card, and reached
    status `READY` with `31807 nodes` and `55775 edges`.
  - `AnvienLauncher.exe stop` left no packaged `AnvienLauncher`, `anvien-server`, or
    `anvien` processes running.
...
- Full launcher build first:
  `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed before Phase 17
...
- Web unit gate:
  `cd anvien-web && npm test` passed with `39` test files and `296` tests after fixing
...
- Browser E2E gate:
  - Packaged launcher started backend `http://127.0.0.1:4747` and Web UI
    `http://127.0.0.1:5173`.
  - `cd anvien-web && npx playwright test --workers=1` passed with `33/33` tests against the
    packaged launcher.
...
- Phase 17 runtime fixes discovered by cutover validation:
  ...
  - Go CLI now includes `anvien list`, backed by the repo registry store. Packaged command
    evidence: `anvien.exe --help`, `anvien.exe status`, and `anvien.exe list` all ran from
...
- Cutover test target note:
  ...
  `anvien/test/fixtures` that intentionally include non-buildable Go/C examples. The cutover Go
  runtime gate is `go test ./cmd/... ./internal/... -count=1`.

- Container and CI runtime cutover:
  - `Dockerfile.cli` now builds `cmd/anvien` with Go `1.26.3` on Debian slim, auto-resolves the
    latest stable LadybugDB native release, builds with `-tags ladybugdb`, copies the Go `anvien`
    binary plus native LadybugDB SO into the runtime image, and contains no Node runtime.
  - `docker build -f Dockerfile.cli -t anvien-cli-cutover .` passed.
  - Container smoke passed: `docker run --rm anvien-cli-cutover anvien version` returned
  ...
  - `.github/workflows/ci-e2e.yml` has been ported to the Go backend path: setup Go from `go.mod`,
    prepare latest-stable LadybugDB native runtime, build `.tmp/anvien` with `-tags ladybugdb`,
  ...
  - `README.md`, `TESTING.md`, `docs/local-usage.md`, and `.env.example` local runtime
    instructions now run the Go backend directly from `cmd/anvien` on `127.0.0.1`; Web UI remains
...
- Phase 17 current-repo large benchmark/parity probe at commit `5d64ece`:
  ...
  - TypeScript baseline command:
    `node anvien\dist\cli\index.js analyze . --force [redacted removed argument] --no-stats --benchmark-json
    .tmp\phase17-cutover-ts-anvien.json --benchmark-label ts-phase17-current-repo` passed with
    `28,731` nodes, `51,603` CLI-reported edges, `52,686` graph-snapshot relationships, and
    `150,292.7ms` wall time.
  - Go packaged command:
    `anvien-launcher\server-bundle\anvien.exe analyze . --force [redacted removed argument] --no-stats
    --benchmark-json .tmp\phase17-cutover-go-anvien.json` passed with `31,829` nodes,
  ...
  - Native DB build probe:
    ...
    `go build -tags ladybugdb -trimpath -o .tmp\anvien-ladybugdb-smoke.exe .\cmd\anvien` both
...
- Phase 17 native LadybugDB packaging fix:
  - Added `scripts/ensure-ladybug-native.ps1` and `scripts/ensure-ladybug-native.sh`. Default
    ...
    assets, and still allow explicit pinning through `ANVIEN_LADYBUGDB_VERSION` or Docker build
    ARG for rollback/debug.
  - Full launcher build command:
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed after rollback
    ...
    `.tmp\ladybug-native\v0.16.1\windows-x86_64`, built `server-bundle\anvien.exe` with
  ...
  - Packaged current-repo analyze command:
    `anvien-launcher\server-bundle\anvien.exe analyze . --force [redacted removed argument] --no-stats
    --benchmark-json .tmp\phase17-cutover-go-native-anvien.json` passed with active DB load:
    `nodeRows=31,549`, `relationshipRows=55,824`, `fallbackInsertCount=0`, and no
    `dbLoad.skipped=true`.
  - Docker command:
    `docker build -f Dockerfile.cli -t anvien-cli-cutover .` passed after switching the image
    to Debian slim and copying native LadybugDB SO files into `/usr/local/lib`.
  - Container smoke command:
    `docker run --rm anvien-cli-cutover sh -c 'anvien version; ... analyze ...'` passed:
...
- Phase 17 process parity fix:
  - Impact precheck:
    `anvien impact --uid Function:internal/processes/processes.go:Apply#2 --repo Anvien
...
- Phase 17 import parity fix:
  - Impact prechecks:
    `anvien impact "resolveImports" --repo Anvien --direction upstream --depth 2` and
    `anvien impact "emitImportEdges" --repo Anvien --direction upstream --depth 2` both
    returned `LOW` risk.
  - Runtime change:
  ...
  - Auto-update proof was reconfirmed during this batch: the local LadybugDB latest cache was
    ...
    and rebuilt `server-bundle\anvien.exe` with `lbug_shared.dll` beside the binary.
  - Packaged current-repo rerun:
    `anvien-launcher\server-bundle\anvien.exe analyze . --force [redacted removed argument] --no-stats
...
- Phase 17 CALLS compatibility and DB schema reconciliation:
  - Impact prechecks:
    `anvien impact "semanticEdgeKey" --repo Anvien --direction upstream --depth 2`
    returned `LOW`; `anvien impact "emitRelationship" --repo Anvien --direction upstream
    --depth 2` returned `MEDIUM`; `anvien impact "resolveCall" --repo Anvien --direction
    upstream --depth 2` returned `LOW`; `anvien impact "RelationPairs" --repo Anvien
    --direction upstream --depth 2` returned `LOW`; `anvien impact "NodeTables" --repo
    Anvien --direction upstream --depth 2` returned `LOW`.
  - Runtime change:
  ...
  - Full build and test:
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` passed, then
    `go test ./cmd/... ./internal/... -count=1` passed.
  - Auto-update proof:
    ...
    `anvien-launcher\server-bundle\lbug_shared.dll`.
  - Packaged current-repo rerun:
    `anvien-launcher\server-bundle\anvien.exe analyze . --force [redacted removed argument] --no-stats
...
- Phase 17 call-return type binding enrichment:
  - Impact prechecks:
    `anvien impact "buildWorkspace" --repo Anvien --direction upstream --depth 2`
    returned `HIGH`; `anvien impact "resolveMember" --repo Anvien --direction upstream
    --depth 2` returned `LOW`; `anvien impact "lookupTypeBinding" --repo Anvien
    --direction upstream --depth 2` returned `LOW`; `anvien impact "baseTypeName" --repo
    Anvien --direction upstream --depth 2` returned `LOW`.
  - Runtime change:
...
- Phase 17 TypeScript/Node runtime-authority audit (2026-05-13):
  ...
  - Graph-tool freshness:
    `.anvien/meta.json` showed `embeddings=0`, so the repo graph was refreshed with
    `anvien analyze --force` before Anvien graph queries. The refresh completed with `28,828`
    nodes and `51,858` edges.
  - Graph exploration:
    `query` for TypeScript shared contract/runtime terms surfaced the legacy TypeScript MCP/backend
    class `anvien/src/mcp/local/local-backend.ts:LocalBackend`, the Phase 1 contract snapshot
  ...
  - Filesystem/package audit:
    `Dockerfile.cli` builds `/usr/local/bin/anvien` from `./cmd/anvien` and contains no Node
    ...
    build compiles `cmd/anvien` into `anvien-launcher/server-bundle/anvien.exe`, compiles Go
    launcher/server-wrapper binaries, copies `lbug_shared.dll`, and uses Node only to build
    `anvien-web`.
  - Blocker found:
    `Get-Command anvien` on the current machine resolves to
    `C:\Users\TAM PC\AppData\Roaming\npm\anvien.ps1`, whose script invokes `node` with
    `node_modules/anvien/dist/cli/index.js`. `anvien/package.json` still declares
    `"bin": { "anvien": "dist/cli/index.js" }`, Node/tsx build scripts, TypeScript CLI/server/MCP
    dependencies, and publish/release workflows still publish the TypeScript npm package.
  - Contract-authority blocker found:
    `anvien-shared` remains a TypeScript package with source contracts and generated dist output,
    and the legacy `anvien/` TypeScript runtime still imports it. This has not yet been reduced to
...
- Phase 17 local npm/PATH CLI distribution cutover (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien-launcher\server-bundle\anvien.exe analyze --force`; the Go analyzer completed for
    `F:\Anvien` with `scanned=1208`, `parsed=1036`, `unsupported=172`, `failed=0`,
    ...
    `resolveCliPath` and `runAnvienCli` each had `handlePreToolUse` as the direct caller.
  - Implementation:
    `anvien/package.json` now maps `bin.anvien` to `bin/anvien.exe` and includes `bin` in the
    package files. `anvien/scripts/build.js` still builds the legacy TypeScript baseline for
    comparison/tests, then runs `scripts/build-go-runtime.cjs`. The new Go runtime build script
    builds `cmd/anvien` with `-tags ladybugdb`, resolves/copies the LadybugDB native runtime next
    to the package binary, and writes `bin/anvien-runtime.json` with the platform metadata.
  ...
  - Validation order and results:
    ...
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed. Then
    `cd anvien && npm run build` passed, `go test ./cmd/... ./internal/... -count=1` passed, and
    `cd anvien && npx vitest run test/unit/package-bin.test.ts test/unit/hooks.test.ts` passed
    `74` tests across `2` files. Browser E2E was run against
    `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747`; `/api/info` returned
    `nodeVersion="go1.26.3"` and `cd anvien-web && npx playwright test --workers=1` passed
    `33/33`.
  - Package/runtime verification:
    `npm link` in `anvien/` rewrote the global PATH shim so
    `C:\Users\TAM PC\AppData\Roaming\npm\anvien.ps1` invokes
    `node_modules/anvien/bin/anvien.exe` instead of
    `node_modules/anvien/dist/cli/index.js`. `anvien --help` from PATH prints the Go CLI help.
    `npm publish --dry-run` passed and the dry-run tarball included `bin/anvien.exe`,
    `bin/lbug_shared.dll`, and `bin/anvien-runtime.json`.
  - Benchmark:
    old PATH npm shim `anvien --help` median was `133.7ms` and average was `150.8ms`; direct
    packaged Go `anvien.exe --help` median was `25.7ms`. After cutover and `npm link`, PATH
    `anvien --help` median was `46.5ms` and average was `57.5ms`; direct package
    `anvien/bin/anvien.exe --help` median was `29.6ms`. The PATH command is now about `2.9x`
...
- Phase 17 portable npm source-build distribution cutover (2026-05-13):
  ...
  - Implementation:
    `anvien/scripts/prepare-go-source-package.cjs` copies a minimal Go source package into
    `anvien/go-src` during `prepack`: `go.mod`, `go.sum`, non-test `.go` files under `cmd/` and
    ...
    source. `anvien/package.json` now includes `go-src` in package files and runs
    ...
    builds `cmd/anvien` from `node_modules/anvien/go-src` when installed outside the repo.
  - Native resolver fix:
  ...
  - Validation order and results:
    ...
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in about
    `39.9s`. Then `cd anvien && npm run build` passed, `go test ./cmd/... ./internal/... -count=1`
    passed, and `cd anvien && npx vitest run test/unit/package-bin.test.ts test/unit/hooks.test.ts`
  ...
  - Portable tarball install test:
    ...
    F:\Anvien\.tmp\npm-portable-install\node_modules\anvien\go-src`, resolved LadybugDB
    native under that packaged source tree, wrote the installed package binary, and the installed
    `bin/anvien-runtime.json` reported `source="go-src"`, `platform="win32"`, `arch="x64"`.
    `node_modules\.bin\anvien.cmd --help` returned
    `Anvien local CLI and MCP server`. Optional `tree-sitter-swift` native build still emitted the
  ...
  - Browser E2E:
    ...
    started from `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747` and Vite on
    `127.0.0.1:5173`. The run exited `0`; `anvien-web/test-results/.last-run.json` reported
    `status="passed"`, and `npx playwright test --list` listed `33` tests in `6` files.
  - Publish dry-run:
    `cd anvien && npm publish --dry-run --loglevel=error` passed. The lifecycle ran `prepack`,
    copied `211` files to `go-src`, ran `prepare`, then `postpack` removed
    `F:\Anvien\anvien\go-src`.
  - Cutover conclusion:
...
- Phase 17 direct graph-tool CLI command surface batch (2026-05-13):
  ...
  - Graph freshness and impact:
    the graph was refreshed before graph-based work with `anvien analyze --force`; the run
    ...
    `cmd/anvien/main.go:main`. The graph reported HIGH risk for existing `newAnalyzeCommand` and
  ...
  - Validation order and results:
    ...
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    `38,947.9ms`. Then `cd anvien && npm run build` passed in `32,498.7ms`, and
    `go test ./cmd/... ./internal/... -count=1` passed in `30,142.1ms`.
  - Direct CLI smoke/benchmark:
    using `anvien\bin\anvien.exe`, all new direct graph-tool commands exited `0`:
    `query "CLI command surface" --repo F:\Anvien --limit 1`,
    `context NewRootCommand --repo F:\Anvien --file internal/cli/command.go`,
    `impact NewRootCommand --repo F:\Anvien --direction upstream --depth 1`,
    `cypher "MATCH (n:Function) RETURN n.id AS id, n.name AS name, n.filePath AS path LIMIT 1"
    --repo F:\Anvien`, `detect-changes --scope unstaged --repo F:\Anvien`, and
  ...
  - Browser E2E:
    ...
    `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747` and the Web UI was served on
    `127.0.0.1:5173`. `cd anvien-web && E2E=1 npx playwright test --workers=1` exited `0` in
    `219,001.4ms`; `anvien-web/test-results/.last-run.json` reported `status="passed"`, and
...
- Phase 17 local admin/analyze CLI command surface batch (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien\bin\anvien.exe analyze --force [redacted removed argument] --no-stats` in `54,730.6ms`
  ...
  - Implementation:
    ...
    `--all`, unregisters repos, deletes `.anvien`, and preserves `.anvien/settings.json`.
    `index` registers an existing `.anvien`/LadybugDB index, supports `--force` and
    `--allow-non-git`, writes a minimal meta file when forced, and appends `.anvien/` to
  ...
  - Validation order and results:
    full launcher build was run first with
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    `37,419.6ms`. Then `cd anvien && npm run build` passed in `31,215ms`, and
    `go test ./cmd/... ./internal/... -count=1` passed in `29,775.5ms`.
  - Admin/analyze CLI smoke/benchmark:
    using `anvien\bin\anvien.exe` with a temp `ANVIEN_HOME` and temp repos, all commands
    ...
    `index <temp git repo with .anvien/lbug>` `164ms`; `clean --force` on a temp indexed repo
    `33.2ms`; `benchmark-compare` `44.8ms`; and `benchmark-compare --json` `35.9ms`.
  - Browser E2E:
    after the full build and command validation, the Go backend was started from
    `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747`. `cd anvien-web && E2E=1
    npx playwright test --workers=1` exited `0` in `198,151.4ms`;
    `anvien-web/test-results/.last-run.json` reported `status="passed"`, and
    `npx playwright test --list` listed `33` tests in `6` files.
  - Cutover conclusion:
...
- Phase 17 group CLI command surface batch (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien\bin\anvien.exe analyze --force [redacted removed argument] --no-stats` in `58,846.9ms`.
  ...
  - Validation order and results:
    full launcher build was run first with
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    `56,230ms`. Then `cd anvien && npm run build` passed in `34,282.8ms`, and
    `go test ./cmd/... ./internal/... -count=1` passed in `32,754.3ms`.
  - Group CLI smoke/benchmark:
    using `anvien\bin\anvien.exe` with a temp `ANVIEN_HOME` and temp indexed repos, all group
...
- Phase 17 setup/quarantine CLI command surface batch (2026-05-13):
  - Phase-jump note:
    this work stayed in Phase 17 because `anvien setup`, editor MCP config, global skill
  ...
  - Graph freshness and impact:
    ...
    `anvien\bin\anvien.exe analyze --force [redacted removed argument] --no-stats` in `83,500.2ms`.
    `NewRootCommand` impact remained CRITICAL because the root CLI fans out to normal runtime
    ...
    processes.
  - Implementation:
    Go now exposes `anvien setup`. The command creates the global Anvien directory, writes MCP
  ...
  - Regression tests:
    ...
    content, installing Claude hooks/settings, creating the configured global Anvien directory,
  ...
  - Validation order and results:
    ...
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    `33,954.6ms`. Then `cd anvien && npm run build` passed in `14,970.8ms`, and
    `go test ./cmd/... ./internal/... -count=1` passed in `23,659.4ms`.
  - Setup CLI smoke/benchmark:
    using `anvien\bin\anvien.exe` with temp `HOME`/`USERPROFILE`/`ANVIEN_HOME` and an empty
    ...
    The Codex fallback contained exactly one `[mcp_servers.anvien]` section after two runs.
  - Browser E2E:
    after the full build and command validation, the Go backend was started from
    `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747`. `cd anvien-web && E2E=1
...
- Phase 17 post-command-surface TypeScript/Node runtime audit (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien\bin\anvien.exe analyze --force [redacted removed argument] --no-stats` in `55,369.8ms`.
  ...
  - Implementation:
    `anvien/package.json` now routes `npm run serve` to `bin/anvien.exe serve` and `npm run
    dev` to `go run ../cmd/anvien`. The old TypeScript watch entrypoint is retained only as
    `dev:ts-baseline`, making it explicit baseline/dev material rather than normal runtime
    authority. The Web onboarding command now shows `anvien serve` and no longer shows
    `cd anvien && npm run serve` or global npm install fallbacks.
  - Runtime audit:
    the local PowerShell PATH shim invokes `node_modules/anvien/bin/anvien.exe`. Runtime-file
    search found no `tsx src/cli/index.ts serve`, no `"serve": "tsx`, no default `"dev": "tsx
    watch src/cli/index.ts`, and no Web onboarding runtime command `cd anvien && npm run serve`;
    the only remaining literal old onboarding command is a negative unit-test assertion.
  - Validation order and results:
    full launcher build was run first with
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    `35,880.6ms`. Then `cd anvien && npm run build` passed in `16,488.3ms`;
    `go test ./cmd/... ./internal/... -count=1` passed in `26,452.4ms`; and
    `cd anvien-web && npm test -- OnboardingGuide.local-only.test.tsx` passed in `4,364.1ms`.
  - Runtime benchmark:
    all audited entrypoints exited `0`: PATH `anvien --help` `75.7ms`; direct
    `anvien\bin\anvien.exe --help` `36.6ms`; `cd anvien && npm run --silent serve -- --help`
    `369.2ms`; and `cd anvien && npm run --silent dev -- --help` `411.9ms`.
  - Browser E2E:
    after the full build and command validation, the Go backend was started from
    `anvien\bin\anvien.exe serve --host 127.0.0.1 --port 4747`. `cd anvien-web && E2E=1
  ...
  - Cutover conclusion:
    ...
    local runtime. Remaining TypeScript under `anvien/src`, `anvien-shared`, and legacy tests is
...
- Phase 17 Go-owned Web contract authority cutover (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien\bin\anvien.exe analyze --force [redacted removed argument] --no-stats` in `66,617.2ms`,
    ...
    `anvien-shared` to Go-generated browser glue.
  - Implementation:
    ...
    `anvien-web/src/generated/anvien-contracts.ts` from Go runtime constants for graph labels,
    ...
    adapter, and the Web package no longer depends on or aliases `anvien-shared`.
  - Web/runtime audit:
    `rg -n "anvien-shared" anvien-web` returned zero hits in `55.8ms`. The Web package
    lockfile, Vite config, Vitest config, Vercel install command, source imports, and test imports
    no longer point at `anvien-shared`.
  - Validation order and results:
    full launcher build was run first with
    `powershell -ExecutionPolicy Bypass -File anvien-launcher\build.ps1` and passed in
    ...
    `go run ./cmd/generate-web-contracts --check` passed in `559.9ms`; `cd anvien-web && npm ci`
    passed in `50,004.7ms`; a post-install full launcher build passed in `33,223.8ms`; and
    `cd anvien && npm test` passed in `338,908.6ms`.
  - Browser E2E:
    after the post-install full build, the packaged Go backend was started from
    `anvien-launcher\server-bundle\anvien.exe serve --host 127.0.0.1 --port 4747`. `cd
    anvien-web && E2E=1 npx playwright test --workers=1` exited `0` in `205,536.5ms` with
    `33/33` tests passed.
  - Cutover conclusion:
    ...
    `anvien-shared` and `anvien/src` TypeScript remain quarantined as baseline/dev/test
...
- Phase 15 MCP route_map hot-path optimization (2026-05-13):
  ...
  - Graph freshness and impact:
    ...
    `anvien-launcher\server-bundle\anvien.exe analyze --force [redacted removed argument] --no-stats` in
  ...
  - Validation order and results:
    ...
    `ANVIEN_HOME` containing only `Anvien`. `cd anvien && npm test` passed in
    `373,303.6ms`.
...
- Phase 14 frontend/mobile app coverage addendum (2026-05-13):
  ...
  - Validation order and results:
    ...
    `ANVIEN_HOME` and a verified packaged Go backend whose `/api/repos` returned only
    `Anvien`; the final accepted full run passed with `32` passed / `1` skipped in
    `494,207.2ms`. `cd anvien && npm test` passed in `335,723.4ms`.
  - Test hardening note:
...
- Phase 10 LadybugDB fallback fail-closed correctness reassessment (2026-05-13):
  ...
  - Anvien/impact evidence:
    `context LoadCSVExport --repo Anvien --file internal/lbugload/load.go` showed the normal DB
  ...
  - Validation order and results:
    ...
    `27,772.9ms`; full browser E2E through the packaged Go backend and isolated `ANVIEN_HOME`
    passed with `32` passed / `1` skipped in `512,685.5ms`; and `cd anvien && npm test` passed in
    `438,204.3ms`.
  - Conclusion:
...
- Phase 15 MCP context one-pass neighborhood optimization (2026-05-13):
  ...
  - Validation order and results:
    ...
    packaged Go backend and isolated `ANVIEN_HOME` passed with `32` passed / `1` skipped in
    `512,685.5ms`; and `cd anvien && npm test` passed in `438,204.3ms`.
  - Optimization conclusion:
...
- Phase 15 MCP impact timing-profile slice (2026-05-13):
  ...
  - Validation order and results:
    ...
    packaged Go backend and isolated `ANVIEN_HOME` passed with `32` passed / `1` skipped in
    `536,712.6ms`; and `cd anvien && npm test` passed in `624.8s`.
  - Optimization conclusion:
...
- Phase 15 MCP group_sync cold/warm timing split (2026-05-14):
  ...
  - Graph parity/analyze smoke:
    ...
    `.anvien/analyze.lock` and partial `lbug.shadow`/`lbug.wal.checkpoint`; the recorded lock PID
  ...
  - Validation order and results:
    ...
    `ANVIEN_HOME` passed with `32` passed / `1` skipped in `503,825.4ms`; and `cd anvien &&
...
- Phase 15 MCP P3 startup/query/tools-list/noise preservation slice (2026-05-14):
  ...
  - Graph freshness and impact:
    before editing, the packaged Go runtime refreshed the graph with
    `anvien-launcher\server-bundle\anvien.exe analyze --force [redacted removed argument] --no-stats` in
    `59,746.3ms`, producing `33,644` nodes and `66,800` relationships. Anvien impact checks were
  ...
  - Validation order and results:
    ...
    isolated `ANVIEN_HOME` passed with `32` passed / `1` skipped after isolated analyze
    `57,281.5ms` and Playwright `488,397.8ms`; and `cd anvien && npm test` passed in
    `385,336.0ms`.
  - Optimization conclusion:
...
- Phase 15 large-repo graph stream and pprof slice (2026-05-14):
  ...
  - Graph freshness and impact:
    ...
    `33,661` nodes and `66,883` relationships. Anvien impact for `newAnalyzeCommand` was
    CRITICAL because it is CLI root/analyze wiring; that risk was accepted only for narrow opt-in
    profiling flags that do not change default analyze behavior. Anvien impact for
    `streamGraphNDJSON` was LOW, with direct impact limited to `handleGraph`.
  - Implementation:
    `anvien analyze` now accepts `--cpuprofile` and `--memprofile`, writing Go pprof CPU and heap
  ...
  - Graph stream benchmark:
    before stream batching, Go `/api/graph?stream=true&repo=Anvien` took `2,915.0ms`; final
  ...
  - Validation order and results:
    ...
    `ANVIEN_HOME` passed with isolated analyze `64,342.3ms`, Playwright `32` passed / `1` skipped
    in `408,728.9ms`; and `cd anvien && npm test` passed in `419,432.8ms`.
  - Optimization conclusion:
...
- Phase 15 DB-load parallel COPY rejection slice (2026-05-14):
  ...
  - Anvien usage:
    packaged Go analyze refreshed the Anvien graph before graph-based work. Anvien context for
  ...
  - Candidate result:
    ...
    `method.csv` at line `315`. Because Anvien stores source/content text in CSV fields, this is a
  ...
  - Validation:
    ...
    (`29,349.2ms`), browser E2E through the packaged Go backend and isolated `ANVIEN_HOME`
    (`33/33` passed; isolated analyze `63,905.4ms`; Playwright `180,285.1ms`), and
    `cd anvien && npm test` (`409,607.3ms`). The accepted E2E analyze artifact
  ...
  - Conclusion:
    ...
    remains open because global parallel COPY is unsafe for current Anvien CSV content. The useful
...
- Phase 15 ScopeIR normalization allocation optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    packaged Go analyze refreshed the Anvien graph before graph-based work. Anvien context on
    `ScopeIR.Normalized` showed it owns the ScopeIR fact sorting path. Anvien impact reported
  ...
  - Validation order and results:
    ...
    `57,957.6ms`; Playwright `405,663.7ms`), and `cd anvien && npm test` (`342,850.8ms`).
  - Optimization conclusion:
...
- Phase 15 framework definition-window allocation optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    packaged Go analyze refreshed the Anvien graph before graph-based work. Anvien context on
    `definitionWindow` showed one direct caller, `AnnotateScopeIR`. Anvien impact reported
  ...
  - Validation order and results:
    ...
    packaged Go backend and isolated `ANVIEN_HOME` (`32` passed / `1` skipped with
    `--workers=1`; isolated analyze `55,728.6ms`; graph stream smoke `1,734.1ms`; Playwright
    `397,958.2ms`), and `cd anvien && npm test` (`537,613.2ms`). A 4-worker Playwright run failed
...
- Phase 15 resolution workspace allocation optimization (2026-05-14):
  - Phase-stay note:
    ...
    optimization on the already cut-over Go runtime.
  - Anvien usage and impact:
    `anvien analyze --force [redacted removed argument]` refreshed the graph before graph-based work.
    Anvien impact reported `buildWorkspace` CRITICAL because it feeds `BuildCrossFileBinding`,
  ...
  - Validation order and results:
    ...
    Go backend and isolated `ANVIEN_HOME` (`32` passed / `1` skipped with `--workers=1`;
...
- Phase 15 DB-load schema lookup allocation optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    `anvien analyze --force [redacted removed argument]` refreshed the graph before graph-based work.
    Anvien impact reported `validNodeTables`, `nodeColumns`, and `relationPairSupported` CRITICAL
  ...
  - Validation order and results:
    ...
    (`31,352.4ms`), browser E2E through the packaged Go backend and isolated `ANVIEN_HOME`
    (`32` passed / `1` skipped with `--workers=1`; isolated analyze `57,103.4ms`; Playwright
    `412,198.7ms`), and `cd anvien && npm test` (`424,654.6ms`).
  - Optimization conclusion:
...
- Phase 15 native DB-load transaction optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    the packaged Go binary refreshed the Anvien graph before graph-based work. Anvien impact
    reported `loadGraph` CRITICAL because it feeds `analyze`, `newAnalyzeCommand`, `main`, and HTTP
    embed/analyze paths. Anvien context for `LoadCSVExport` showed analyze, native integration,
  ...
  - E2E and smoke evidence:
    ...
    isolated home confirmed `/api/info` and `/api/repos` served the indexed `Anvien` repo.
  - Validation order and results:
    ...
    through the packaged Go backend (`32` passed / `1` skipped). `cd anvien && npm test` had one
...
- Phase 15 imported-member resolution index optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    ...
    changed. Anvien impact reported `resolveImportedMember` CRITICAL because it feeds
  ...
  - E2E validation note:
    ...
    same highlight check. The accepted full run used a fresh isolated `ANVIEN_HOME`, packaged Go
  ...
  - Validation order and results:
    ...
    Playwright `428,776.4ms`). `cd anvien && npm test` passed in `378,034.1ms`.
  - Optimization conclusion:
...
- Phase 15 parser node-count diagnostic optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    the packaged Go binary refreshed the Anvien graph before graph-based work. Anvien impact
    reported `countNodes` LOW risk with direct caller `Pool.Parse`. Anvien impact reported
  ...
  - Validation order and results:
    ...
    browser E2E through packaged Go backend and isolated `ANVIEN_HOME` (`33/33` passed; isolated
    analyze `29,087.9ms`; Playwright `206,434.9ms`), and `cd anvien && npm test`
...
- Phase 15 TS/JS provider traversal optimization (2026-05-14):
  - Phase-stay note:
    ...
    cut-over Go runtime.
  - Anvien usage and impact:
    the packaged Go binary refreshed the Anvien graph before graph-based work. Anvien context
    showed `tsjs.Extract` is called by `extractScopeIR`, SFC extraction, and resolution tests.
    Anvien impact reported `Extract` CRITICAL because it feeds analyze/runtime/test flows; the
  ...
  - Validation order and results:
    ...
    tests (`29,375.2ms`), browser E2E through packaged Go backend and isolated `ANVIEN_HOME`
    (`33/33` passed; isolated analyze `25,467.0ms`; Playwright `194,306.0ms`), and
    `cd anvien && npm test` (`343,081.8ms`).
  - Optimization conclusion:
...
- Phase 15 Go provider traversal optimization (2026-05-14):
  ...
  - Validation order and results:
    ...
    tests (`34,494.9ms`), browser E2E through packaged Go backend and isolated `ANVIEN_HOME`
    (`32` passed / `1` skipped; isolated analyze `34,598.5ms`; Playwright `436,101.7ms`), and
    `cd anvien && npm test` (`404,491.2ms`, exit code `0`).
  - Optimization conclusion:
...
- Phase 15 graph snapshot stream memory optimization (2026-05-14):
  ...
  - Validation order and results:
    ...
    (`31,950.0ms`), browser E2E through packaged Go backend and isolated `ANVIEN_HOME`
    (`31` passed / `1` skipped / `1` flaky recovered on retry; isolated analyze `26,815.2ms`;
    Playwright `528,159.0ms`), and `cd anvien && npm test` (`414,123.7ms`, exit code `0`).
  - Optimization conclusion:
...
- Phase 10 TypeAlias->Method LadybugDB schema gap fix (2026-05-14):
  ...
  - Validation order and results:
    ...
    `ANVIEN_HOME=F:\Anvien\.tmp\typealias-method-e2e-home-20260514-unique`, packaged Go
    ...
    passed with `32` passed / `1` skipped in `475,727.0ms`. `cd anvien && npm test` passed with
...
- Phase 16 launcher UI-close process lifecycle fix (2026-05-14):
  - Phase-jump note:
    ...
    running and holding locks under `anvien-launcher\server-bundle`. That breaks build/update
    behavior and must be treated as a real lifecycle bug, not a manual-test artifact.
  - Anvien usage and impact:
    Anvien impact reported LOW risk for `startRuntime`, `staticHandler`, `waitForExit`, and
    `openBrowser`. The changed surface is the Go launcher process that serves the packaged Web UI
    and starts the packaged backend; direct user-started `anvien serve` sessions are not killed.
  - Implementation:
    ...
    `/__anvien_launcher/heartbeat` and sends `/__anvien_launcher/closed` on `pagehide`.
    ...
    Web server and stopping only the backend PID it started. `ANVIEN_LAUNCHER_NO_BROWSER=1` is
    test-only and leaves normal launcher UX unchanged.
  - Unit/focused validation:
    after the final full launcher build, `go test ./... -count=1` in `anvien-launcher/src` passed
    in `5,110.9ms`, and `go test ./... -count=1` in `anvien-launcher/server-wrapper` passed in
  ...
  - Build and smoke validation:
    ...
    `AnvienLauncher.exe` hidden with `ANVIEN_LAUNCHER_NO_BROWSER=1`, verified the backend and
    launcher-served Web UI became ready, verified the served index contained
    `data-anvien-launcher-lifecycle`, posted heartbeat and closed events, and completed in
    `15,647.2ms`. After close, no `AnvienLauncher.exe`, `anvien-server.exe`, or packaged
    `anvien.exe` process remained under `anvien-launcher`, and both packaged binaries opened
  ...
  - Browser E2E validation:
    ...
    `27,569ms`. `cd anvien && npm test` passed with exit code `0` in `415,943.7ms`.
  - Conclusion:
...
- Phase 15 ScopeIR owned normalization allocation optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    packaged Go Anvien refreshed the graph before graph-based work. Anvien impact reported
  ...
  - Validation status:
    ...
    isolated `ANVIEN_HOME=F:\Anvien\.tmp\phase15-owned-normalize-e2e-home-20260514`, the
    ...
    listed `33` tests in `6` files, and the E2E command wall was `590.4s`. `cd anvien &&
    npm test` passed in `461,316.4ms`. Anvien `detect_changes(scope=all)` reported MEDIUM risk
...
- Phase 15 ScopeIR release-after-resolution retained-heap optimization (2026-05-14):
  ...
  - Anvien usage and impact:
    Anvien impact reported `analyze.Run` CRITICAL and `newAnalyzeCommand` HIGH because these are
  ...
  - Validation status:
    ...
    and isolated `ANVIEN_HOME` (`32` passed / `1` skipped; Playwright `441,444.3ms`; isolated
    analyze total `29,141.3ms`, graph rows `33,928` / `67,617`, fallback/skipped `0`), Prettier
    check for the touched E2E specs, and `cd anvien && npm test` (`571.4s`).
...
- Phase 15 graph compact-after-processes retained-heap optimization (2026-05-14):
  - Phase-stay note:
    ...
    coverage, or Phase 17 runtime authority.
  - Anvien usage and impact:
    after the previous commit, packaged Go Anvien refreshed the graph. Anvien impact reported
  ...
  - Validation status:
    ...
    backend and isolated `ANVIEN_HOME` (`32` passed / `1` skipped; Playwright `434,795.2ms`;
    isolated analyze total `25,136.7ms`, graph rows `33,982` / `67,657`, fallback/skipped `0`),
    and `cd anvien && npm test` (`477.1s`). The E2E wrapper had a cleanup exit-code quirk after
...
- Phase 15 TS/JS fact-kind dispatch deferred evidence (2026-05-14):
  - Deferral note:
    ...
    readiness is proven.
  - Impact:
    Anvien reported `internal/providers/tsjs.Extract` as CRITICAL (`8` impacted symbols,
  ...
  - Decision:
    ...
    the plan returns to Phase 17 to prove `Anvien` is an independent Go MCP/tool implementation
    separate from the existing `Anvien-main` MCP.
...
- Audit command:
  ...
  `node_modules`, `dist`, `build`, `vendor`, `anvien-web/dist`, and
  `anvien-launcher/server-bundle`.
- Audit result:
  `1051` TypeScript/JavaScript-family files remain in source areas. Split by top-level path:
  `anvien=895`, `anvien-web=119`, `anvien-shared=34`, root Docker/web-server scripts `2`,
  and root ESLint config `1`. Inside `anvien`, the split is `test=542`, `src=339`, `scripts=8`,
...
- Interpretation:
  ...
  acceptable. `anvien/src` still contains legacy CLI/server/MCP/analyzer/core TypeScript
  implementation; `anvien-shared` remains a TypeScript package; root and package scripts still
...
- Date: `2026-05-14`.
- Reason:
  the plan priority was clarified: `Anvien` must become a correct, accurate, usable Go tool,
...
- Goal interpretation:
  ...
  to run independently from `Anvien-main`. The target is for `Anvien` to be correct and
  accurate enough to use for real work, and to be more accurate than the currently used
  `Anvien-main` where the conversion has identified legacy weaknesses; compatibility parity
...
- Reopened Phase 1:
  ...
  `anvien-shared`, legacy `anvien/src` contract consumers, package scripts, and Node/Vitest
...
- Date: `2026-05-14`.
- Anvien usage:
  confirmed Codex MCP `anvien` points at `F:\Anvien-main\anvien\dist\cli\index.js`, then used
  Anvien-main `status`, `list`, `analyze --force`, `query`, and `context` against `Anvien`.
  The refresh artifact is `.tmp\phase17-tsjs-classification-anvien-main-refresh.json`.
- Phase-jump note:
...
| --- | ---: | --- | --- |
| `anvien-web` Web UI | `119` | Allowed TypeScript/React surface | Keep as Web UI display/build/test surface; ensure it calls Go backend contracts only. |
| Go-generated browser glue | `1` inside `anvien-web/src/generated` | Allowed generated TypeScript | Keep generated from Go; never make it backend/CLI/MCP/analyzer authority. |
| `anvien/src` legacy implementation | `339` | Non-Web TypeScript implementation | Remove from runtime/package authority after Go equivalents are proven; do not treat as completed conversion. |
| `anvien-shared` | `34` | Legacy TypeScript contract authority | Replace with Go-owned contracts/generated Web glue or exclude after all consumers are removed. |
| Analyzer fixture source files | `290` under `anvien/test/fixtures` | Source-language input data | May remain as analyzer fixtures; must be excluded from runtime/package authority. |
| Node/Vitest harness | `252` under `anvien/test` outside fixtures | Heavy legacy test runner surface | Convert runtime/analyzer coverage to Go tests where practical; keep only Web/browser harness where justified. |
| `anvien/scripts` | `8` | Package/build/test glue | Per-file decision: port to Go/native, remove with legacy TS package, or justify as minimal npm/Web ecosystem glue. |
| `anvien/hooks` | `1` | Non-Web support runtime | Port or prove it only delegates to Go and is required by editor ecosystem. |
| `anvien/vendor` JS/CJS | `4` | Legacy Node/vendor artifacts | Remove or exclude unless a remaining allowed Web/npm path proves it is required. |
...
- Anvien usage:
  refreshed `Anvien` with
  `.\\anvien\\bin\\anvien.exe analyze --force [redacted removed argument] --no-stats --benchmark-json .tmp\\phase17-web-docker-nginx-preimpact-refresh.json`,
  then ran `detect_changes(scope=all)`, `query` for Docker/static-server concepts, and attempted
  `impact(target="anvien-launcher/build.ps1")`. The graph mapped no code execution flow for this
...
- Build-gate hardening:
  the required full launcher build exposed that `anvien-launcher/build.ps1` could return success
...
- Remaining blocker:
  `[P17-NON-WEB-TSJS-CONVERSION-BLOCKER]` stays open for `anvien/src`, `anvien-shared`,
...
- Anvien usage:
  refreshed `Anvien` before graph-based work, then ran impact checks for
...
- Translation/cutover:
  the legacy Claude hook behavior from `anvien/hooks/claude/anvien-hook.cjs` was translated
  into hidden Go command `anvien hook claude`. The Go command reads hook JSON from stdin,
  silently ignores invalid input, finds `.anvien` by walking up from `cwd`, handles PreToolUse
  ...
  `.anvien/meta.json` and emitting stale-index guidance with `--embeddings` when previous stats
  include embeddings.
- Setup/package retirement:
  Go setup and the legacy TypeScript setup shim now write `anvien hook claude` directly into
  Claude Code settings, replace old copied `anvien-hook.cjs` entries, and preserve user-owned
  settings. `anvien/package.json` no longer ships the `hooks` directory. The legacy
  `anvien/hooks/claude/anvien-hook.cjs`, `pre-tool-use.sh`, and `session-start.sh` files were
  deleted after the Go hook path was proven.
- Validation:
  full package build ran before tests and passed in `18,285.7ms`; Go tests
  `go test ./cmd/... ./internal/... -count=1` passed in `24,546.2ms`; `cd anvien && npx tsc
  --noEmit` passed in `8,745.7ms`; `cd anvien && npm test` passed in `375,380.3ms` and included
  ...
  `anvien hook claude` hook entries and no `anvien-hook.cjs` reference.
- Checklist/evidence result:
  `docs/plans/2026-05-08-anvien-typescript-node-to-go-conversion-remaining-files.md` now ticks
  `anvien/hooks/claude/anvien-hook.cjs`. The broader
  `[P17-NON-WEB-TSJS-CONVERSION-BLOCKER]` remains open for the remaining `anvien/src`,
  `anvien-shared`, Node/Vitest harnesses, package scripts, vendor JS/CJS, config, and final
